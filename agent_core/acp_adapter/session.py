...
Sessions are persisted to the shared SessionDB (``~/.devil/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    ...
    """
    from devil_constants import is_wsl
...
class SessionManager:
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            ...
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.devil/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``DEVIL_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_devil_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=devil_home / "state.db")
            return self._db_instance
        except Exception:
