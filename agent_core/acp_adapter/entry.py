...
Loads environment variables from ``~/.devil/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from DEVIL_HOME (default ``~/.devil``)."""
    from devil_cli.env_loader import load_devil_dotenv

    devil_home = get_devil_home()
    loaded = load_devil_dotenv(devil_home=devil_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", devil_home / ".env"
        )
...
def _print_version() -> None:
    from devil_cli import __version__ as hermes_version
...
def _run_setup() -> None:
    from devil_cli.main import main as hermes_main
