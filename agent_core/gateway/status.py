...
The PID file lives at ``{DEVIL_HOME}/gateway.pid``.  DEVIL_HOME defaults to
``~/.devil`` but can be overridden via the environment variable.  This means
separate DEVIL_HOME directories naturally get separate PID files — a property
...
def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting DEVIL_HOME."""
    home = get_devil_home()
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("DEVIL_GATEWAY_LOCK_DIR")
    if override:
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different DEVIL_HOME dirs).
    """
