...
- GET  /v1/models                  — lists devil-agent as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to devil-agent
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_devil_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
        ...
        # gracefully on NFS/SMB/FUSE-mounted DEVIL_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # devil_state._WAL_INCOMPAT_MARKERS).
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through devil-agent's AIAgent.
    """
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "devil-agent"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return devil-agent as an available model."""
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from devil_cli.auth import has_usable_secret
