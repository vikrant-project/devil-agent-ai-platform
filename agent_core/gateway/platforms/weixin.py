...
Connects Devil Agent to WeChat personal accounts via Tencent's iLink Bot API.
...
def _account_dir(devil_home: str) -> Path:
    path = Path(devil_home) / "weixin" / "accounts"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _account_file(devil_home: str, account_id: str) -> Path:
    return _account_dir(devil_home) / f"{account_id}.json"
...
def save_weixin_account(
    ...
    *,
    ...
    path = _account_file(devil_home, account_id)
...
def load_weixin_account(devil_home: str, account_id: str) -> Optional[Dict[str, Any]]:
...
class ContextTokenStore:
    ...
    def __init__(self, devil_home: str):
        self._root = _account_dir(devil_home)
...
def _sync_buf_path(devil_home: str, account_id: str) -> Path:
    return _account_dir(devil_home) / f"{account_id}.sync.json"


def _load_sync_buf(devil_home: str, account_id: str) -> str:
    path = _sync_buf_path(devil_home, account_id)
    if not path.exists():
        return ""
    try:
...
def _save_sync_buf(devil_home: str, account_id: str, sync_buf: str) -> None:
    path = _sync_buf_path(devil_home, account_id)
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._devil_home = devil_home
        self._token_store = ContextTokenStore(devil_home)
        ...
        if self._account_id and not self._token:
            persisted = load_weixin_account(devil_home, self._account_id)
            if persisted:
    ...
    async def _poll_loop(self) -> None:
        ...
        sync_buf = _load_sync_buf(self._devil_home, self._account_id)
        ...
        while self._running:
            try:
                ...
                if new_sync_buf:
                    ...
                    _save_sync_buf(self._devil_home, self._account_id, sync_buf)
...
async def send_weixin_direct(
    *,
    ...
    token_store = ContextTokenStore(str(get_devil_home()))
