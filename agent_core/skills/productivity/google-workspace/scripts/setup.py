#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Devil Agent.
...
# Ensure sibling modules (_devil_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _devil_home import display_devil_home, get_devil_home

DEVIL_HOME = get_devil_home()
TOKEN_PATH = DEVIL_HOME / "google_token.json"
CLIENT_SECRET_PATH = DEVIL_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = DEVIL_HOME / "google_oauth_pending.json"
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_devil_home()}/google_token.json")
