...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("DEVIL_HOME", os.path.expanduser("~/.devil")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("DEVIL_HOME", Path.home() / ".devil")) / "skills" / "red-teaming" / "godmode"
...
DEVIL_HOME = Path(os.getenv("DEVIL_HOME", Path.home() / ".devil"))
CONFIG_PATH = DEVIL_HOME / "config.yaml"
PREFILL_PATH = DEVIL_HOME / "prefill.json"
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.devil/prefill.json."""
