...
~30 Hz (matching xterm key-repeat), summarizes ~/.devil/perf.log on exit.
...
bypasses the devil_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  HERMES_PERF_LOG     (default ~/.devil/perf.log)
...
DEFAULT_LOG = Path(os.environ.get("HERMES_PERF_LOG", str(get_devil_home() / "perf.log")))
DEFAULT_STATE_DB = get_devil_home() / "state.db"
