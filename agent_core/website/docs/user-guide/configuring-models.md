...
Devil Agent uses two kinds of model slots:
...
- **Right** — the curated model list for the selected provider. These are the agentic models Devil Agent recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Devil Agent writes it to `~/.hermes/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning Devil Agent uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
When you save via the dashboard, Devil Agent writes to `~/.hermes/config.yaml`:
...
`provider: auto` with `model: ''` tells Devil Agent to use the main model for that task.
...
Devil Agent lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `hermes setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Devil Agent switched providers on me
...
curl -H "X-Devil Agent-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Devil Agent-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Devil Agent-Session-Token: $TOKEN" \
