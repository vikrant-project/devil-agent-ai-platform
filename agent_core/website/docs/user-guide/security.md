...
Devil Agent Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Devil Agent checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Devil Agent refuses to run them **regardless** of:
...
When running the messaging gateway, Devil Agent controls who can interact with the bot through a layered authorization system.
...
For more flexible authorization, Devil Agent includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.
...
When using the `docker` terminal backend, Devil Agent applies strict security hardening to every container.
...
When loaded, Devil Agent checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Devil Agent infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Devil Agent infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
mcp_servers:
  github:
    ...
    env:
      GITHUB_PERSONAL_ACCESS_TOKEN: "YOUR_GITHUB_TOKEN..."  # Only this is passed
```
...
Devil Agent integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Devil Agent under WSL.
