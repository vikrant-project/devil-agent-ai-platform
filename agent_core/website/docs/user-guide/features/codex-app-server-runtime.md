...
Devil Agent can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Devil Agent becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default Devil Agent behavior is unchanged unless you flip the flag. Devil Agent never auto-routes you onto this runtime.
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your Devil Agent session.
- **Devil Agent' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Devil Agent for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Devil Agent' message shape so the self-improvement loop sees a normal-looking transcript.
...
These ship with `codex app-server` itself — no Devil Agent involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Devil Agent' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Devil Agent also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Devil Agent queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Devil Agent tool callback (MCP server, registered in `~/.codex/config.toml`)

Devil Agent registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Devil Agent' image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Devil Agent' skill library.
- **`text_to_speech`** — TTS through Devil Agent' configured provider.

When the model wants one of these, codex spawns the `hermes_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Devil Agent' default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four Devil Agent tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Devil Agent' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Devil Agent' todo store (codex's `update_plan` is the in-runtime equivalent)
...
- The Devil Agent tool callback for browser_*, vision, image_gen, skills, TTS
...
|  | Devil Agent default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Devil Agent' own `hermes auth login codex` writes to `~/.hermes/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Devil Agent auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   Devil Agent will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.

## Enabling

In a Devil Agent session:
...
- **Registers Devil Agent' own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Devil Agent runtime)
```

## Self-improvement loop (memory + skill nudges)

Devil Agent' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Devil Agent runtime.
...
One detail: the review fork itself needs to call Devil Agent' agent-loop tools (`memory`, `skill_manage`), which require Devil Agent' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Devil Agent owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Devil Agent' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Devil Agent shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Devil Agent' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Devil Agent' managed block:

```toml
default_permissions = ":read-only"
```

(Devil Agent will preserve your override on re-migration as long as it lives outside the `# managed by hermes-agent` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, session search summarization, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Devil Agent' auxiliary client uses the main provider/model when no per-task override is set.
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Devil Agent downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Devil Agent' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

Devil Agent wraps everything it manages between two marker comments:
...
- Add your own MCP servers Devil Agent doesn't know about
...
By default, Devil Agent points the codex subprocess at `~/.codex/` regardless of which Devil Agent profile is active. This means `hermes -p work` and `hermes -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.
...
Devil Agent does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Devil Agent' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Devil Agent (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- Devil Agent-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Devil Agent writes a `[plugins."<name>@openai-curated"]` block enabling it in your Devil Agent session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Devil Agent' codex runtime, Devil Agent activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Devil Agent doesn't touch credentials.

## Devil Agent tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Devil Agent registers itself as an MCP server in `~/.codex/config.toml`:
...
When the model calls `web_search` (or another exposed Devil Agent tool), codex spawns the `hermes_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Devil Agent runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Devil Agent Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Devil Agent UI
...
- Devil Agent web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **Devil Agent auth and codex auth are separate sessions.** You need both `codex login` AND `hermes auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Devil Agent' `_import_codex_cli_tokens` — Devil Agent won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
```
                ┌─── Devil Agent shell (CLI / TUI / gateway) ───┐
