...
Devil Agent Agent includes two model-callable web tools backed by multiple providers:
...
The summary keeps quotes, code blocks, and key facts in their original formatting — it's a content compressor, not a paraphraser. If summarization fails or times out, Devil Agent falls back to the first ~5 000 chars of raw content rather than a useless error.
...
SearXNG is a privacy-respecting, open-source metasearch engine that aggregates results from 70+ search engines. **No API key required** — just point Devil Agent at a running SearXNG instance.
...
# After (enable JSON for Devil Agent):
formats:
  - html
  - json
```
...
With this config, Devil Agent uses SearXNG for all search queries and Firecrawl for URL extraction — combining free search with high-quality extraction.
...
If no backend is explicitly configured, Devil Agent picks the first available one based on which credentials are set:
