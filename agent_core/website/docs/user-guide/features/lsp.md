...
Devil Agent runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Devil Agent
...
1. Devil Agent captures a baseline of current diagnostics for the file.
...
…). Devil Agent auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — Devil Agent installs both packages together when you
...
When `install_strategy: auto`, Devil Agent installs binaries into
...
shared location — the staging dir is fully Devil Agent-owned and is
...
Servers are kept alive for the life of the Devil Agent process. There's
