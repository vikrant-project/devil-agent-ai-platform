...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Devil Agent automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Devil Agent auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:
...
Devil Agent automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Devil Agent PKCE OAuth | `~/.hermes/auth.json` | Yes (Anthropic) |
