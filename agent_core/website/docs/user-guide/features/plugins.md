...
title: "Plugins"
description: "Extend Devil Agent with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Devil Agent has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in Devil Agent
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Devil Agent Plugin](/docs/guides/build-a-hermes-plugin)** — step-by-step guide with a complete working example.
...
Start Devil Agent — your tools appear alongside built-in tools. The model can call them immediately.
...
```python
"""Minimal Devil Agent plugin — registers a tool and a hook."""
...
Drop both files into `~/.hermes/plugins/hello-world/`, restart Devil Agent, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.hermes/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Devil Agent.
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Devil Agent — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
...
Within each source, Devil Agent also recognizes sub-category directories that route plugins to specialized discovery systems:
...
Several categories of plugin bypass `plugins.enabled` — they're part of Devil Agent' built-in surface and would break basic functionality if gated off by default:
...
Devil Agent has four kinds of plugins:
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Devil Agent also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Devil Agent Plugin](/docs/guides/build-a-hermes-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a Devil Agent Plugin](/docs/guides/build-a-hermes-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Devil Agent Plugin](/docs/guides/build-a-hermes-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Devil Agent auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
