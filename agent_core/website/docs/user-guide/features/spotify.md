...
Devil Agent can control Spotify directly — playback, queue, search, playlists, saved tracks/albums, and listening history — using Spotify's official Web API with PKCE OAuth. Tokens are stored in `~/.hermes/auth.json` and refreshed automatically on 401; you only log in once per machine.

Unlike Devil Agent' built-in OAuth integrations (Google, GitHub Copilot, Codex), Spotify requires every user to register their own lightweight developer app. Spotify does not let third parties ship a public OAuth app that anyone can use. It takes about two minutes and `hermes auth spotify` walks you through it.
...
- Devil Agent Agent installed and running.
...
Scroll to `🎵 Spotify`, press space to toggle it on, then `s` to save. Devil Agent drops you straight into the OAuth flow — if you don't have a Spotify app yet, it walks you through creating one inline. Once you finish, the toolset is enabled AND authenticated in one pass.
...
If no `HERMES_SPOTIFY_CLIENT_ID` is set, Devil Agent walks you through the app registration inline:
...
| App description | anything (e.g. `personal Devil Agent integration`) |
| Website | leave blank |
| Redirect URI | `http://127.0.0.1:43827/spotify/callback` |
| Which API/SDKs? | check **Web API** |

Agree to the terms and click **Save**. On the next page click **Settings** → copy the **Client ID** and paste it into the Devil Agent prompt. That's the only value Devil Agent needs — PKCE doesn't use a client secret.

### Running over SSH / in a headless environment

If `SSH_CLIENT` or `SSH_TTY` is set, Devil Agent skips the automatic browser open during both the wizard and the OAuth step. Copy the dashboard URL and the authorization URL Devil Agent prints, open them in a browser on your local machine, and proceed normally — the local HTTP listener still runs on the remote host on port `43827`. Your laptop's browser can't reach the remote loopback without an SSH local-forward:
...
Shows whether tokens are present and when the access token expires. Refresh is automatic: when any Spotify API call returns 401, the client exchanges the refresh token and retries once. Refresh tokens persist across Devil Agent restarts, so you only re-auth if you revoke the app in your Spotify account settings or run `hermes auth logout spotify`.
...
Because Spotify tools are regular Devil Agent tools, a cron job running in a Devil Agent session can trigger playback on any schedule. No new code needed.
...
1. Cron spins up a headless Devil Agent session.
...
**`204 No Content` on `get_currently_playing`** — nothing is currently playing on any device. This is Spotify's normal response, not an error; Devil Agent surfaces it as an explanatory empty result (`is_playing: false`).

**`INVALID_CLIENT: Invalid redirect URI`** — the redirect URI in your Spotify app settings doesn't match what Devil Agent is using. The default is `http://127.0.0.1:43827/spotify/callback`. Either add that to your app's allowed redirect URIs, or set `HERMES_SPOTIFY_REDIRECT_URI` in `~/.hermes/.env` to whatever you registered.

**`429 Too Many Requests`** — Spotify's rate limit. Devil Agent returns a friendly error; wait a minute and retry. If this persists, you're probably running a tight loop in a script — Spotify's quota resets roughly every 30 seconds.
...
**Wizard doesn't open the browser** — If you're over SSH or in a container without a display, Devil Agent detects it and skips the auto-open. Copy the dashboard URL it prints and open it manually.

## Advanced: custom scopes

By default Devil Agent requests the scopes needed for every shipped tool. Override if you want to restrict access:
