...
chat completions — use your Devil Agent-managed provider subscription as their
...
| Use case | "Use Devil Agent as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Devil Agent stores
the refresh token in `~/.hermes/auth.json` — the same place all Devil Agent
...
```
Starting Devil Agent proxy for Nous Portal
...
Model:      Devil Agent-4-70B    # or Devil Agent-4.3-36B, Devil Agent-4-405B
```
...
```
Devil Agent proxy upstream adapters
...
INFERENCE_TEXT_MODEL=Devil Agent-4-70B
```
