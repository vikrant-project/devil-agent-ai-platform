...
You can also point Devil Agent at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Devil Agent to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.hermes/plans/` relative to the active workspace/backend working directory.
...
When a missing value is encountered, Devil Agent asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `hermes setup` or `~/.hermes/.env` locally instead.
...
If you maintain skills outside of Devil Agent — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Devil Agent to scan those directories too.
...
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Devil Agent ignores it without errors. Useful for optional shared directories that may not be present on every machine.
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Devil Agent. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `hermes skills search <query> --source skills-sh`. Devil Agent resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Devil Agent currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Devil Agent repository itself and install with builtin trust.
...
This is Vercel's public skills directory. Devil Agent can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
Devil Agent can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
- Devil Agent source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Devil Agent supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Devil Agent source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Devil Agent can search and convert agent entries from LobeHub's public catalog into installable Devil Agent skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Devil Agent source id: `lobehub`

#### 8. Direct URL (`url`)

Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Devil Agent fetches the URL, parses the YAML frontmatter, security-scans it, and installs.

- Devil Agent source id: `url`
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Devil Agent | Always trusted |
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Devil Agent users add with `hermes skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
Devil Agent discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Devil Agent user can subscribe and install:
...
Devil Agent ships with a set of bundled skills in `skills/` inside the repo. On install and on every `hermes update`, a sync pass copies those into `~/.hermes/skills/` and records a manifest at `~/.hermes/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Devil Agent recomputes the hash of your local copy and compares it to the origin hash:
