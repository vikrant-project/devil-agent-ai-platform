...
```
Devil Agent Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
3. Add `twozero_td` MCP server to Devil Agent config (if missing)
...
3. **Restart Devil Agent session** to pick up the new MCP server
