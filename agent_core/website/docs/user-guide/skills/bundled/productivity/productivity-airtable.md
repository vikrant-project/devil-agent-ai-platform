...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Devil Agent. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Important Notes for Devil Agent
