...
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Devil Agent Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Devil Agent terminal calls (desktop app flow only)
...
### Service Account (recommended for Devil Agent)
...
## Devil Agent Execution Pattern (desktop app flow)

Devil Agent terminal commands are non-interactive by default and can lose auth context between calls.
