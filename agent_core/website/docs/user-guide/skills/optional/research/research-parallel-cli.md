...
This is an optional third-party workflow, not a Devil Agent core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Devil Agent native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Devil Agent native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Devil Agent native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Devil Agent workflow:
...
## Recommended Devil Agent usage patterns
...
- Do not silently choose Parallel when Devil Agent native tools are already sufficient.
