---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Devil Agent Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Devil Agent Agent"
---
...
Migrate a user's OpenClaw customization footprint into Devil Agent Agent. Imports Devil Agent-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
...
| Version | `1.0.0` |
| Author | Devil Agent Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Devil Agent`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Devil Agent Migration

Use this skill when a user wants to move their OpenClaw setup into Devil Agent Agent with minimal manual cleanup.
...
- import `SOUL.md` into the Devil Agent home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Devil Agent memory entries
- merge OpenClaw command approval patterns into Devil Agent `command_allowlist`
- migrate Devil Agent-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.hermes/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Devil Agent workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hermes/tts/`
- archive non-secret docs that do not have a direct Devil Agent destination
...
With `--migrate-secrets`, it will also import a small allowlisted set of Devil Agent-compatible secrets, currently:
...
Devil Agent CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Devil Agent. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Devil Agent workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Devil Agent skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Devil Agent unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Devil Agent targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Devil Agent destination. Unsupported auth blobs must still be reported as skipped.
...
- Devil Agent persona state imported
- Devil Agent memory files populated with converted OpenClaw knowledge
