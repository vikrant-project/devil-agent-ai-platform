...
| Version | `0.1.0` |
| Author | Teknium (teknium1), Devil Agent Agent |
| License | MIT |
...
The remote client connects to `$URL` with `Authorization: Bearer $TOKEN`. Devil Agent' own native MCP client config: `{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`.
