...
sidebar_position: 2
title: "Configuration"
description: "Configure Devil Agent Agent — config.yaml, providers, models, API keys, and more"
---
...
For cloud sandboxes such as Modal, Daytona, and Vercel Sandbox, `container_persistent: true` means Devil Agent will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
**Single persistent container, not per-command.** Devil Agent starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container — across sessions, `/new`, `/reset`, and `delegate_task` subagents — for the lifetime of the Devil Agent process. Working-directory changes, installed packages, and files in `/workspace` carry over from one tool call to the next, just like a local shell. The container is stopped and removed on shutdown. See **Container lifecycle** below for details.
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Devil Agent probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `HERMES_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.

**Container lifecycle:** Devil Agent reuses a single long-lived container (`docker run -d ... sleep 2h`) for every terminal and file-tool call, across sessions, `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the Devil Agent process. Commands run via `docker exec` with a login shell, so working-directory changes, installed packages, and files in `/workspace` all persist from one tool call to the next. The container is stopped and removed on Devil Agent shutdown (or when the idle-sweep reclaims it).

Parallel subagents spawned via `delegate_task(tasks=[...])` share this one container — concurrent `cd`, env mutations, and writes to the same path will collide. If a subagent needs an isolated sandbox, it must register a per-task image override via `register_task_env_overrides()`, which RL and benchmark environments (TerminalBench2, Devil AgentSweEnv, etc.) do automatically for their per-task Docker images.
...
Runs commands in a [Vercel Sandbox](https://vercel.com/docs/vercel-sandbox) cloud microVM. Devil Agent uses the normal terminal and file tool surfaces; there are no Vercel-specific model-facing tools.
...
**Required authentication:** Configure access-token auth with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This is the supported setup for deployments and normal long-running Devil Agent processes on Render, Railway, Docker, and similar hosts.

For one-off local development, Devil Agent also accepts short-lived Vercel OIDC tokens:
...
**Runtime:** `terminal.vercel_runtime` supports `node24`, `node22`, and `python3.13`. If unset, Devil Agent defaults to `node24`.

**Persistence:** When `container_persistent: true`, Devil Agent snapshots the sandbox filesystem during cleanup and restores a later sandbox for the same task from that snapshot. Snapshot contents can include Devil Agent-synced credentials, skills, and cache files that were copied into the sandbox. This preserves filesystem state only; it does not preserve live sandbox identity, PID space, shell state, or running background processes.

**Background commands:** `terminal(background=true)` uses Devil Agent' generic non-local background process flow. You can spawn, poll, wait, view logs, and kill processes through the normal process tool while the sandbox is alive. Devil Agent does not provide native Vercel detached-process recovery after cleanup or restart.

**Disk sizing:** Vercel Sandbox does not currently support Devil Agent' `container_disk` resource knob. Leave `container_disk` unset or at the shared default `51200`; non-default values fail diagnostics and backend creation instead of being silently ignored.
...
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Devil Agent logs a clear error if either is missing.
...
For the **SSH**, **Modal**, and **Daytona** backends (anywhere the agent's working tree lives on a different machine than the host running Devil Agent), Devil Agent tracks files the agent touched inside the remote sandbox and, on session teardown / sandbox cleanup, **syncs the modified files back to the host** under `~/.hermes/cache/remote-syncs/<session-id>/`.
...
Devil Agent resolves each listed variable from your current shell first, then falls back to `~/.hermes/.env` if it was saved with `hermes config set`.
...
When enabled, Devil Agent appends `--user $(id -u):$(id -g)` to the `docker run` command so files written into bind-mounted directories (`/workspace`, `/root`, anything in `docker_volumes`) are owned by your host user, not root. The trade-off: the container can no longer `apt install` or write to root-owned paths like `/root/.npm` — use a base image whose `HOME` is owned by a non-root user (or add your required tooling at image build time) if you need both.
...
Docker sandboxes stay isolated by default. Devil Agent does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Devil Agent from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Devil Agent from
...
When the agent uses `skill_manage` to create, edit, patch, or delete a skill, Devil Agent can optionally scan the new/updated content for dangerous keyword patterns (credential harvesting, obvious prompt injection, exfil instructions). The scanner is **off by default** — real agent workflows that legitimately touch `~/.ssh/` or mention `$OPENAI_API_KEY` were tripping the heuristic too often. Turn it back on if you want the scanner to prompt you before the agent's skill writes land:
...
Three related caps control how much raw output a tool can return before Devil Agent truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, Devil Agent keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
Devil Agent automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
`hygiene_hard_message_limit` is a gateway-only **pre-compression safety valve**. Runaway sessions with thousands of messages can hit model context limits before the normal percent-of-context threshold fires; when message count crosses this ceiling, Devil Agent forces compression regardless of token usage. Default `400` — raise it for platforms where very long sessions are normal, lower it to force more aggressive compression. Editing this value on a running gateway takes effect on the next message (see below).
...
`agent.api_max_retries` controls how many times Devil Agent retries a provider API call on transient errors (rate limits, connection drops, 5xx) **before** fallback-provider switching engages. The default is `3` — four attempts total. If you have [fallback providers](/docs/user-guide/features/fallback-providers) configured and want to fail over faster, drop this to `0` so the first transient error on your primary immediately hands off to the fallback instead of churning retries against the flaky endpoint.

### API Timeouts

Devil Agent has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.
...
The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Devil Agent raises this to 30 minutes when it detects a local endpoint. If you explicitly set `HERMES_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Devil Agent disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `HERMES_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Devil Agent Agent — Auxiliary Models Tutorial"
...
Every model slot in Devil Agent — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Devil Agent ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Devil Agent uses that provider's built-in auth and base URL.
...
If you use a reasoning-heavy model for `auxiliary.session_search`, Devil Agent now gives you two built-in controls:

- `auxiliary.session_search.max_concurrency`: limits how many matched sessions Devil Agent summarizes at once
...
Use `extra_body` only when your provider documents OpenAI-compatible request-body fields you want Devil Agent to pass through for that task. Devil Agent forwards the object as-is.

:::warning
`extra_body` is only effective when your provider actually supports the field you send. If the provider does not expose a native OpenAI-compatible reasoning-off flag, Devil Agent cannot synthesize one on its behalf.
:::
...
The shape mirrors what OpenRouter accepts in the chat completions request body. Devil Agent forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Devil Agent uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Devil Agent uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
When `display.file_mutation_verifier` is `true` (default), Devil Agent appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and was never superseded by a successful write to the same path. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
When `display.runtime_footer.enabled: true`, Devil Agent appends a small runtime-context footer to the **final** message of each gateway turn — same info the CLI shows in its status bar (model, context %, cwd, session duration, tokens, cost). Off by default; opt in per-gateway if your team wants every reply to include the provenance.
...
`interim_assistant_messages` is gateway-only. When enabled, Devil Agent sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Devil Agent falls back automatically in this order: `local` → `groq` → `openai`.
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Devil Agent to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Devil Agent still keys DMs by chat/DM ID as usual.
...
Control what Devil Agent does when an unknown user sends a direct message:
...
- `pair` is the default. Devil Agent denies access, but replies with a one-time pairing code in DMs.
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Devil Agent's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
browser:
  ...
  # Optional CDP override — when set, Devil Agent attaches directly to your own
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when Devil Agent creates a tab
...
Control how Devil Agent handles potentially dangerous commands:
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Devil Agent falls back to `OPENAI_API_KEY` only.
...
Devil Agent uses two different context scopes:
...
- If SOUL.md is missing, empty, or cannot be loaded, Devil Agent falls back to a built-in default identity.
...
- Devil Agent automatically seeds a default `SOUL.md` if one does not already exist.
