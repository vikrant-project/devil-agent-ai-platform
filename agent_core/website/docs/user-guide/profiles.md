...
Run multiple independent Devil Agent agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Devil Agent home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Devil Agent state.
...
That's it. `coder` is now its own Devil Agent profile with its own config, memory, and state.
...
- A **profile** gives Devil Agent its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Devil Agent was launched from", not "the profile directory".
