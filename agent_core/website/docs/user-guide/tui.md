...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for Devil Agent — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for Devil Agent — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run Devil Agent interactively.
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while Devil Agent is starting.
...
On first launch Devil Agent installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new Devil Agent version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point Devil Agent at it:
...
The status-bar busy indicator is pluggable — the default rotates Devil Agent' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
The TUI respects all standard Devil Agent config: `~/.hermes/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
If the TUI fails to launch (no Node, missing bundle, TTY issue), Devil Agent prints a diagnostic and falls back — rather than leaving you stuck.
