...
sidebar_position: 4
title: "Slack"
description: "Set up Devil Agent Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Devil Agent Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Devil Agent instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Devil Agent uses the modern
...
The fastest path is to paste a manifest Devil Agent generates for you. It
...
### Option A: From a Devil Agent-generated manifest (recommended)
...
4. Enter an app name (e.g., "Devil Agent Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Devil Agent can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Devil Agent configuration issue.
:::
...
Devil Agent uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
Every Devil Agent command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Devil Agent command with its description.

Under the hood: Devil Agent ships with a generated Slack app manifest (see
...
When Devil Agent adds new commands (e.g. after `hermes update`), regenerate
...
`/hermes btw run the tests` — Devil Agent routes it the same way as `/btw
...
Slack never delivers them to Devil Agent.

As a workaround, Devil Agent recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Devil Agent treats it identically to the slash form and replies in the same
thread.
...
Understanding how Devil Agent behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Devil Agent Agent what time is it?`). In channels, Devil Agent replies in a thread attached to that message. |
| **Threads** | If you @mention Devil Agent inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Devil Agent in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Devil Agent will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Devil Agent will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Devil Agent Agent`).

---

## Multi-Workspace Support

Devil Agent can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Devil Agent also loads tokens from an **OAuth token file** at:
...
- When a message arrives, Devil Agent uses the correct workspace-specific client to respond.
...
Devil Agent supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Devil Agent Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Devil Agent Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Devil Agent now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Devil Agent Agent`)
...
- Audit who has access to your Devil Agent config directory
