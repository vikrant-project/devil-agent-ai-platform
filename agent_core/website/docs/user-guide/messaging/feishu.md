...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Devil Agent Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Devil Agent Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Devil Agent opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Devil Agent responds to every message. |
| Group chats | Devil Agent responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Devil Agent will automatically create a bot application with the correct permissions and save the credentials.
...
Use WebSocket mode when Devil Agent runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Devil Agent behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Devil Agent starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Devil Agent responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Devil Agent responds to @mentions from any user in any group. |
| `allowlist` | Devil Agent only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Devil Agent ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Devil Agent read all group traffic without requiring an @mention:
...
Devil Agent auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Devil Agent ignores messages sent by other bots. Enable bot-to-bot messaging when you want Devil Agent to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Devil Agent. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `hermes update --gateway` needs confirmation, the adapter records the selected answer in Devil Agent's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Devil Agent reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
| `Another local Devil Agent gateway is already using this Feishu app_id` | Only one Devil Agent instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Devil Agent can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
