...
sidebar_position: 12
title: "Google Chat"
description: "Set up Devil Agent Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Devil Agent Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Devil Agent
...
download the file. Save it somewhere only Devil Agent can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — Devil Agent calls
...
- **App name**: whatever you want users to see ("Devil Agent" is reasonable).
...
`ADDED_TO_SPACE` event that Devil Agent uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Devil Agent
...
Install the dependencies the Google Chat adapter needs (no Devil Agent extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Devil Agent is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Devil Agent detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Devil Agent session.
...
3. Download the JSON. Move it onto the host that runs Devil Agent.
4. On the host, register the client with Devil Agent:
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Devil Agent isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: Devil Agent will only attach the SA bearer
