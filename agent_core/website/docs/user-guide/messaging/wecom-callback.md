...
Connect Devil Agent to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Devil Agent supports two WeCom integration modes:
...
3. Devil Agent decrypts the message, queues it for the agent
