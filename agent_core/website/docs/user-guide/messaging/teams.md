...
description: "Set up Devil Agent Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Devil Agent Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Devil Agent strips automatically before processing.
