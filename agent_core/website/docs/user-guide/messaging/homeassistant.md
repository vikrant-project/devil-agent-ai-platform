...
description: Control your smart home with Devil Agent Agent via Home Assistant integration.
...
Devil Agent Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Devil Agent Agent"
5. Copy the token
...
Devil Agent Agent registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Devil Agent Agent".
