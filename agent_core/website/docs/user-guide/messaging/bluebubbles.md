...
Connect Devil Agent to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Devil Agent and the BlueBubbles server
...
When someone messages your iMessage, Devil Agent automatically sends them a pairing code. Approve it with:
```bash
...
Devil Agent will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Devil Agent
Devil Agent → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Devil Agent sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Devil Agent resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
