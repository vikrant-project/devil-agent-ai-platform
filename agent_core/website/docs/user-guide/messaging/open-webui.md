...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to Devil Agent Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With Devil Agent Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
Open WebUI connects to Devil Agent Agent's API server just like it would connect to OpenAI. Devil Agent handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

:::important Runtime location
The API server is a **Devil Agent agent runtime**, not a pure LLM proxy. For each request, Devil Agent creates a server-side `AIAgent` on the API-server host. Tool calls run where that API server is running.

For example, if a laptop points Open WebUI or another OpenAI-compatible client at a Devil Agent API server on a remote machine, `pwd`, file tools, browser tools, local MCP tools, and other workspace tools run on the remote API-server host, not on the laptop.
:::

Open WebUI talks to Devil Agent server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.

## Quick Setup

### One-command local bootstrap (macOS/Linux, no Docker)

If you want Devil Agent + Open WebUI wired together locally with a reusable launcher, run:
...
- restarts the Devil Agent gateway so the API server comes up
...
- Devil Agent API: `http://127.0.0.1:8642/v1`
- Open WebUI: `http://127.0.0.1:8080`
- model name advertised to Open WebUI: `Devil Agent Agent`

Useful overrides:

```bash
OPEN_WEBUI_NAME='My Devil Agent UI' \
OPEN_WEBUI_ENABLE_SIGNUP=true \
HERMES_API_MODEL_NAME='My Devil Agent Agent' \
bash scripts/setup_open_webui.sh
```
...
### 2. Start Devil Agent Agent gateway
...
6. Enter:
   ...
   - **API Key**: the exact same value as `API_SERVER_KEY` in Devil Agent
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and Devil Agent Agent responds accordingly. Each request includes the full conversation history.
...
With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Devil Agent Agent can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, Devil Agent also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. Devil Agent Agent creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
Your agent has access to the same tools and capabilities as that API-server Devil Agent instance. If the API server is remote, those tools are remote too.

If you need tools to run against your **local** workspace today, run Devil Agent locally and point it at a pure LLM provider or pure OpenAI-compatible model proxy (for example vLLM, LiteLLM, Ollama, llama.cpp, OpenAI, OpenRouter, etc.). A future split-runtime mode for "remote brain, local hands" is being tracked in [#18715](https://github.com/NousResearch/hermes-agent/issues/18715); it is not the behavior of the current API server.
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | Devil Agent Agent's API URL (include `/v1`) |
...
- **Empty Ollama backend shadowing the picker**: If you omitted `ENABLE_OLLAMA_API=false`, Open WebUI shows an empty Ollama section above your Devil Agent models. Restart the container with `-e ENABLE_OLLAMA_API=false` or disable Ollama in **Admin Settings → Connections**.
...
Devil Agent Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in Devil Agent Agent.
...
To run separate Devil Agent instances per user — each with their own config, memory, and skills — use [profiles](/docs/user-guide/profiles). Each profile runs its own API server on a different port and automatically advertises the profile name as the model in Open WebUI.
...
The model dropdown will show `alice` and `bob` as distinct models. You can assign models to Open WebUI users via the admin panel, giving each user their own isolated Devil Agent agent.
