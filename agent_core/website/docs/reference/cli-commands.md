...
description: "Authoritative reference for Devil Agent terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which Devil Agent profile to use for this invocation. Overrides the sticky default set by `hermes profile use`. |
...
| `hermes backup` | Back up Devil Agent home directory to a zip file. |
...
| `hermes import` | Restore a Devil Agent backup from a zip file. |
...
| `hermes acp` | Run Devil Agent as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run Devil Agent as an MCP server. |
| `hermes plugins` | Manage Devil Agent Agent plugins (install, enable, disable, remove). |
...
| `hermes profile` | Manage profiles — multiple isolated Devil Agent instances. |
...
| `hermes uninstall` | Remove Devil Agent from the system. |
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Devil Agent chat session.
...
**`hermes model`** (run from your terminal, outside any Devil Agent session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Devil Agent chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Devil Agent session first (`Ctrl+C` or `/quit`), then run `hermes model` from your terminal prompt.
:::
...
| `--name NAME` | `Devil Agent` | Bot display name in Slack. |
...
hermes auth spotify                                      # Authenticate Devil Agent with Spotify via PKCE
```
...
Outputs a compact, plain-text summary of your entire Devil Agent setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Devil Agent version, release date, git commit hash |
...
The report includes system info (OS, Python version, Devil Agent version), recent agent and gateway logs (512 KB limit per file), and redacted API key status. Keys are always redacted — no secrets are uploaded.
...
Create a zip archive of your Devil Agent configuration, skills, sessions, and data. The backup excludes the hermes-agent codebase itself.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Devil Agent is running (WAL-mode safe).
...
Restore a previously created Devil Agent backup into your Devil Agent home directory. All files in the archive overwrite existing files in your Devil Agent home; `--force` only skips the confirmation prompt that fires when the target already has a Devil Agent installation.
...
View, tail, and filter Devil Agent log files. All logs are stored in `~/.hermes/logs/` (or `<profile>/logs/` for non-default profiles).
...
Devil Agent uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
- `--source well-known` lets you point Devil Agent at a site exposing `/.well-known/skills/index.json`.
...
Starts Devil Agent as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run Devil Agent as an MCP server.

| Subcommand | Description |
|------------|-------------|
| `serve [-v\|--verbose]` | Run Devil Agent as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Devil Agent](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Devil Agent Plugin](../guides/build-a-hermes-plugin.md).
...
right now without waiting for the next Devil Agent update.
...
Migrate your OpenClaw setup to Devil Agent. Reads from `~/.openclaw` (or a custom path) and writes to `~/.hermes`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Devil Agent files on conflicts (default: refuse to apply when the plan has conflicts). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Devil Agent equivalents or **archived** for manual review.
...
Manage profiles — multiple isolated Devil Agent instances, each with its own config, sessions, skills, and home directory.
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Devil Agent commands, subcommands, and profile names.
...
- **Legacy `hermes.service` warning.** If Devil Agent detects a pre-rename `hermes.service` systemd unit (instead of the current `hermes-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| `hermes uninstall [--full] [--yes]` | Remove Devil Agent, optionally deleting all config/data. |
