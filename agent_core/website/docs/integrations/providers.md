...
This page covers setting up inference providers for Devil Agent Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Devil Agent.
...
# → done — Devil Agent auto-provisions your free tier on first request
```

Devil Agent ships Google's **public** `gemini-cli` desktop OAuth client by default —
...
- Responses translated back to OpenAI shape so the rest of Devil Agent works unchanged
...
| VPC-SC-protected org | Devil Agent detects `SECURITY_POLICY_VIOLATED` and forces `standard-tier` automatically |
...
experience, use your own API key via the `gemini` provider instead. Devil Agent shows
...
:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Devil Agent stores the resulting credentials in its own auth store under `~/.hermes/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Devil Agent routes these tasks to your **main chat model** — the same model you picked in `hermes model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/docs/user-guide/configuration#auxiliary-models).
:::
...
Devil Agent has **two** model commands that serve different purposes:
...
| **`/model`** | Inside a Devil Agent chat session | Quick switch between **already-configured** providers and models |
...
When you authenticate via `hermes model` → Anthropic OAuth (or via `hermes auth add anthropic --type oauth`), Devil Agent routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Devil Agent — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
# Devil Agent will use Claude Code's credential store directly when available
hermes model
...
When you choose Anthropic OAuth through `hermes model`, Devil Agent prefers Claude Code's own credential store over copying the token into `~/.hermes/.env`. That keeps refreshable Claude credentials refreshable.
...
Devil Agent supports GitHub Copilot as a first-class provider with two modes:
...
:::warning Token types
The Copilot API does **not** support classic Personal Access Tokens (`YOUR_GITHUB_TOKEN*`). Supported token types:
...
If your `gh auth token` returns a `YOUR_GITHUB_TOKEN*` token, use `hermes model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Devil Agent
Devil Agent sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Devil Agent now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Devil Agent therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
When using the Z.AI / GLM provider, Devil Agent automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `hermes model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Devil Agent auto-discovers the available models.
...
Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `hermes model`, sign in through the browser, and Devil Agent persists the refresh token.
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Devil Agent exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `hermes model`, sign in through the browser, and Devil Agent persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
Devil Agent Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Devil Agent at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
`OPENAI_BASE_URL` and `LLM_MODEL` in `.env` are **removed**. Neither is read by any part of Devil Agent — `config.yaml` is the single source of truth for model and endpoint configuration. If you have stale entries in your `.env`, they are automatically cleared on the next `hermes setup` or config migration. Use `hermes model` or edit `config.yaml` directly.
:::
...
**`/model`** (typed inside an active Devil Agent chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.
...
When switching providers, Devil Agent persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Devil Agent.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Devil Agent) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `hermes` (Qwen 2.5, Devil Agent 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.
...
Then configure Devil Agent to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Devil Agent won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.

Native tool calling support (best performance): Llama 3.x, Qwen 2.5 (including Coder), Devil Agent 2/3, Mistral, DeepSeek, Functionary. All other models use a generic handler that works but may be less efficient. See the [llama.cpp function calling docs](https://github.com/ggml-org/llama.cpp/blob/master/docs/function-calling.md) for the full list.
...
Devil Agent will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Devil Agent) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Devil Agent Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Devil Agent config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Devil Agent config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Devil Agent.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Devil Agent inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Devil Agent's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Devil Agent thinks the context is
...
Devil Agent auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Devil Agent uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Devil Agent with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/docs/user-guide/configuration#context-compression) in Devil Agent.
...
Then configure Devil Agent with `hermes model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Devil Agent with `hermes model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Devil Agent uses this to decide when to compress history and to validate API requests.
...
Devil Agent uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Devil Agent uses "no-key-required" for keyless local servers
  - name: work
...
2. Point Devil Agent at your instance (no API key needed):
   ```bash
...
Configure a chain of backup providers Devil Agent tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
