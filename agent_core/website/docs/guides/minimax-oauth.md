...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Devil Agent Agent — no API key required"
---

# MiniMax OAuth

Devil Agent Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Devil Agent automatically refreshes your session.
...
# → Devil Agent opens your browser to the MiniMax authorization page
...
Devil Agent will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Devil Agent implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Devil Agent generates a PKCE verifier / challenge pair and a random state value.
...
4. Devil Agent polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
...
Devil Agent refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Devil Agent marks the session as requiring re-login.

**Fix:** run `hermes auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Devil Agent raises a timeout error.

**Fix:** re-run `hermes auth add minimax-oauth` (or `hermes model`). The flow starts fresh.

### State mismatch (possible CSRF)

Devil Agent detected that the `state` value returned by the authorization server does not match what it sent.
...
Devil Agent prints the URL and code. Open the URL on any device and complete the flow there.
