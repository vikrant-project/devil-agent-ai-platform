...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Devil Agent new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Devil Agent how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Devil Agent installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Devil Agent to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Devil Agent but aren't active by default. Install them explicitly:
...
For how to ship skills in your own plugin, see [Build a Devil Agent Plugin → Bundle skills](/docs/guides/build-a-hermes-plugin#bundle-skills).
...
When a skill with config is first loaded, Devil Agent prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.
...
:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Devil Agent may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Devil Agent will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.
...
**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Devil Agent to update the skill with what you learned. Skills that aren't maintained become liabilities.
