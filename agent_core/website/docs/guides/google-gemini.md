...
sidebar_position: 16
title: "Google Gemini"
description: "Use Devil Agent Agent with Google Gemini — native AI Studio API, API-key setup, OAuth option, tool calling, streaming, and quota guidance"
---

# Google Gemini

Devil Agent Agent supports Google Gemini as a native provider using the **Google AI Studio / Gemini API** — not the OpenAI-compatible endpoint. This lets Devil Agent translate its internal OpenAI-shaped message and tool loop into Gemini's native `generateContent` API while preserving tool calling, streaming, multimodal inputs, and Gemini-specific response metadata.

Devil Agent also supports a separate **Google Gemini (OAuth)** provider that uses the same Cloud Code Assist backend as Google's Gemini CLI. Use the API-key provider (`gemini`) for the lowest-risk official API path.

## Prerequisites

- **Google AI Studio API key** — create one at [aistudio.google.com/apikey](https://aistudio.google.com/apikey)
- **Billing-enabled Google Cloud project** — recommended for agent use. Gemini's free tier is too small for long-running agent sessions because Devil Agent may make several model calls per user turn.
- **Devil Agent installed** — no extra Python package is required for the native Gemini provider.

:::tip API key path
Set `GOOGLE_API_KEY` or `GEMINI_API_KEY`. Devil Agent checks both names for the `gemini` provider.
:::
...
# → Devil Agent checks your key tier and shows Gemini models
# → Select a model
...
Devil Agent detects this endpoint and creates its native Gemini adapter. Internally, Devil Agent still keeps the agent loop in OpenAI-shaped messages, then translates each request to Gemini's native schema:
...
- streaming responses → OpenAI-shaped stream chunks for the Devil Agent loop

:::note Gemini 3 thought signatures
For Gemini 3 tool use, Devil Agent preserves the `thoughtSignature` values attached to function-call parts and replays them on the next tool turn. That covers the validation-critical path for multi-step agent workflows.

Gemini 3 may also attach thought signatures to other response parts. Devil Agent' native adapter is optimized for agent tool loops today, so it does not yet replay every non-tool-call signature with full part-level fidelity.
:::
...
Devil Agent also has a `google-gemini-cli` provider:
...
This uses browser PKCE login and the Cloud Code Assist backend. It can be useful for users who want Gemini CLI-style OAuth, but Devil Agent shows an explicit warning because Google may treat use of the Gemini CLI OAuth client from third-party software as a policy violation. For production or lowest-risk usage, prefer the API-key provider above.

## Available Models

The `hermes model` picker shows Gemini models maintained in Devil Agent' provider registry. Common choices include:
...
Google publishes moving aliases for the Pro and Flash Gemini families. `gemini-pro-latest` and `gemini-flash-latest` are useful when you want Google to advance the model automatically without changing your Devil Agent config.
...
Google also exposes Gemma models through the Gemini API. Devil Agent recognizes these as Google models, but hides very low-throughput Gemma entries from the default model picker so new users do not accidentally select an evaluation-tier model for a long-running agent session.
...
These models are best treated as evaluation options on Gemini API keys. Google's Gemma API pricing is free-tier-only and the usage caps are low compared with production Gemini models, so sustained Devil Agent agent use should normally move to a paid Gemini model, a self-hosted deployment, or another provider with appropriate quota.
...
For OAuth quota usage, run this inside a Devil Agent session:
...
Gemini works with all Devil Agent gateway platforms (Telegram, Discord, Slack, WhatsApp, LINE, Feishu, etc.). Configure Gemini as your provider, then start the gateway normally:
...
Devil Agent could not find a usable API key. Add one of these to `~/.hermes/.env`:
...
Devil Agent probes Gemini API keys during setup. Free-tier quotas can be exhausted after a handful of agent turns because tool use, retries, compression, and auxiliary tasks may require multiple model calls.
...
Devil Agent may hide low-throughput Gemma models from the picker by default. If you intentionally want to evaluate one, set the model ID directly in `~/.hermes/config.yaml`.
...
The `google-gemini-cli` provider uses a Gemini CLI / Cloud Code Assist OAuth flow. Devil Agent warns before starting it because this is distinct from the official AI Studio API-key path. Use `provider: gemini` with `GOOGLE_API_KEY` for the official API-key integration.

### Tool calling fails with schema errors

Upgrade Devil Agent and rerun `hermes model`. The native Gemini adapter sanitizes tool schemas for Gemini's stricter function-declaration format; older builds or custom endpoints may not.
