...
2. **Devil Agent spins up** a fresh agent session with your prompt
...
Devil Agent will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Devil Agent Agent
```
...
Before creating cron jobs, ensure Devil Agent has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Devil Agent what you want:
...
Devil Agent will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Devil Agent to delegate each topic to a sub-agent:
...
Devil Agent will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
