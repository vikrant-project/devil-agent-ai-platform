...
#### Option 1: Devil Agent cron (recommended if you already run the Devil Agent gateway)

Devil Agent ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.hermes/scripts/`. First create the script:
...
```ini
[Unit]
Description=Devil Agent Teams pipeline subscription maintenance
...
```ini
[Unit]
Description=Run Devil Agent Teams pipeline subscription maintenance every 12 hours
...
- [ ] **`maintain-subscriptions` is scheduled** (Devil Agent cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
