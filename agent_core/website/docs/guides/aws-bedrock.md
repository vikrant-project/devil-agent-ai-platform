...
sidebar_position: 14
title: "AWS Bedrock"
description: "Use Devil Agent Agent with Amazon Bedrock — native Converse API, IAM authentication, Guardrails, and cross-region inference"
---

# AWS Bedrock

Devil Agent Agent supports Amazon Bedrock as a native provider using the **Converse API** — not the OpenAI-compatible endpoint. This gives you full access to the Bedrock ecosystem: IAM authentication, Guardrails, cross-region inference profiles, and all foundation models.
...
:::tip EC2 / ECS / Lambda
On AWS compute, attach an IAM role with `AmazonBedrockFullAccess` and you're done. No API keys, no `.env` configuration — Devil Agent detects the instance role automatically.
:::
...
Devil Agent auto-discovers available models via the Bedrock control plane. You can customize discovery:
...
Bedrock works with all Devil Agent gateway platforms (Telegram, Discord, Slack, Feishu, etc.). Configure Bedrock as your provider, then start the gateway normally:
...
Devil Agent checks for credentials in this order:
...
You've hit the Bedrock per-model rate limit. Devil Agent automatically retries with backoff. To increase limits, request a quota increase in the [AWS Service Quotas console](https://console.aws.amazon.com/servicequotas/).
