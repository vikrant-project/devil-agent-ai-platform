---
sidebar_position: 6
title: "Use MCP with Devil Agent"
description: "A practical guide to connecting MCP servers to Devil Agent Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Devil Agent

This guide shows how to actually use MCP with Devil Agent Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Devil Agent tool
- you want Devil Agent to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Devil Agent to internal APIs, databases, or company systems without modifying Devil Agent core

Do not use MCP when:
- a built-in Devil Agent tool already solves the job well
...
- Devil Agent remains the agent
- MCP servers contribute tools
- Devil Agent discovers those tools at startup or reload time
...
If you installed Devil Agent with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Devil Agent banner/status should show MCP integration when configured
- ask Devil Agent what tools it has available
...
## WSL2: bridge Devil Agent in WSL to Windows Chrome

This is the practical setup when:

- Devil Agent runs inside WSL2
...
In this setup, Devil Agent does **not** connect to Chrome directly. Instead:

- Devil Agent runs in WSL
- Devil Agent starts a local stdio MCP server
...
```text
Devil Agent (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Devil Agent stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Devil Agent core browser transport
...
Then start a fresh Devil Agent session or run:
...
Once loaded, Devil Agent can use the MCP-prefixed browser tools directly. For example:
...
If Devil Agent runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Devil Agent from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Devil Agent from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Devil Agent:
...
2. Devil Agent-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Devil Agent will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Devil Agent to reason over a bounded workspace.
...
Now Devil Agent can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Devil Agent core.
...
Because Devil Agent now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
