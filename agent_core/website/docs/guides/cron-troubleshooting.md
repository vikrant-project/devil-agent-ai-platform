...
description: "Diagnose and fix common Devil Agent cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
The `script` path must be an absolute path (or relative to the Devil Agent config directory). Verify:
```bash
...
hermes logs                         # View recent Devil Agent logs
