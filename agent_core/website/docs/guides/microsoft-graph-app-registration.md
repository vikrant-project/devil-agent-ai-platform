...
4. Fill in:
   - **Name:** `Devil Agent Teams Meeting Pipeline` (or any name you'll recognize).
...
```powershell
# Create a policy scoped to the Devil Agent app
New-CsApplicationAccessPolicy `
  -Identity "Devil Agent-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Devil Agent meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Devil Agent-Meeting-Pipeline-Policy" `
...
Devil Agent ships a Graph auth smoke-test. From your Devil Agent install:
