...
description: "Sign in with your SuperGrok subscription to use Grok models in Devil Agent Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok Subscription)

Devil Agent Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using your existing **SuperGrok subscription**. No `XAI_API_KEY` is required — log in once and Devil Agent automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Devil Agent — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Devil Agent opens your browser to accounts.x.ai
...
On servers, containers, or SSH sessions where no browser is available, Devil Agent detects the remote environment and prints the authorization URL instead of opening a browser.
...
1. Devil Agent opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Devil Agent and the tokens are saved to `~/.hermes/auth.json`.
4. From then on, Devil Agent refreshes the access token in the background — you stay signed in until you `hermes auth remove xai-oauth` or revoke access from your xAI account settings.
...
Devil Agent refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Devil Agent surfaces a typed re-auth message instead of crashing.

**Fix:** run `hermes auth add xai-oauth` again to start a fresh login.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Devil Agent raises a timeout error.

**Fix:** re-run `hermes auth add xai-oauth` (or `hermes model`). The flow starts fresh.

### State mismatch (possible CSRF)

Devil Agent detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Devil Agent prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Devil Agent is on a different machine than your browser
