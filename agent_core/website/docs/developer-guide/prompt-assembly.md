...
description: "How Devil Agent builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Devil Agent deliberately separates:
...
You are Devil Agent, an AI assistant created by Nous Research.
...
```
You are Devil Agent Agent, an intelligent AI assistant created by Nous Research.
...
|----------|-------|-------------|-------|
| 1 | `.hermes.md`, `HERMES.md` | CWD up to git root | Devil Agent-native project config |
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Devil Agent already loads, rather than editing Python templates in place.
...
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Devil Agent.
...
- if you want to change how Devil Agent assembles prompts for everyone, change Python and treat it as a code contribution
