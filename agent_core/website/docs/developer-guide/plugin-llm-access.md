...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Devil Agent already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Devil Agent provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Devil Agent provider
        # (must be one Devil Agent already knows about — same names as
        ...
        # plugin sends — Devil Agent does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Devil Agent' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Devil Agent subsystem:
