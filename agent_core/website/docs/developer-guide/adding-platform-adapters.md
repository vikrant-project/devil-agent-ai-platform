...
This guide covers adding a new messaging platform to the Devil Agent gateway. A platform adapter connects Devil Agent to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
The plugin system lets you add a platform adapter without modifying any core Devil Agent code. Your plugin is a directory with two files:
...
def register(ctx):
    """Plugin entry point — called by the Devil Agent plugin system."""
...
version: 1.0.0
description: >
  My Platform gateway adapter for Devil Agent Agent.
author: Your Name
requires_env:
...
:::note
This checklist is for adding a platform directly to the Devil Agent core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
