...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Devil Agent Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Devil Agent Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Devil Agent core? Start with [Build a Devil Agent Plugin](../guides/build-a-hermes-plugin.md)
- Building a new built-in core tool for Devil Agent itself? Start with [Adding Tools](./adding-tools.md)
...
Devil Agent officially supports **Linux, macOS, WSL2, and native Windows (early beta — via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. The native-Windows path is new and moves fast — if you're doing Windows-heavy dev, expect to hit and fix rough edges.
...
Devil Agent has terminal access. Security matters.
...
- Include: OS, Python version, Devil Agent version (`hermes version`), full error traceback
