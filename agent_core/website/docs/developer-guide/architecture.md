...
sidebar_position: 1
title: "Architecture"
description: "Devil Agent Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Devil Agent Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── cli.py                    # Devil AgentCLI — interactive terminal UI (large file)
...
```text
User input → Devil AgentCLI.process_input()
...
Exposes Devil Agent as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
