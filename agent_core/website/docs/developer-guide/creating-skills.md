...
description: "How to create skills for Devil Agent Agent — SKILL.md format, guidelines, and publishing"
---

# Creating Skills

Skills are the preferred way to add new capabilities to Devil Agent Agent. They're easier to create than tools, require no code changes to the agent, and can be shared with the community.
...
Use `required_environment_variables` when a skill needs an API key or token. Missing values do **not** hide the skill from discovery. Instead, Devil Agent prompts for them securely when the skill is loaded in the local CLI.
...
The user can skip setup and keep loading the skill. Devil Agent never exposes the raw secret value to the model. Gateway and messaging sessions show local setup guidance instead of collecting secrets in-band.
...
When loaded, Devil Agent checks if these files exist. Missing files trigger `setup_needed`. Existing files are automatically:
...
Prefer stdlib Python, curl, and existing Devil Agent tools (`web_extract`, `terminal`, `read_file`). If a dependency is needed, document installation steps in the skill.
...
Bundled skills (in `skills/`) ship with every Devil Agent install. They should be **broadly useful to most users**:
...
Devil Agent can now consume third-party skills from multiple external discovery models:
