...
description: "How to add a new inference provider to Devil Agent Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Devil Agent can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:
...
- What env vars should Devil Agent check, and in what priority order?
...
Be careful with API-key precedence. Devil Agent already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.
...
When you add a native provider, double-check that Devil Agent is only sending fields that provider actually understands.
