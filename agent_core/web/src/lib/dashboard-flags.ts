declare global {
  interface Window {
    /** Set true by the server only for `devil dashboard --tui` (or HERMES_DASHBOARD_TUI=1). */
...
/** True only when the dashboard was started with embedded TUI Chat (`devil dashboard --tui`). */
