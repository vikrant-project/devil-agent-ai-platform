...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    ...
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.devil/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.devil/tts/.",
    },
    "discord-settings": {
...
# become ``~/.devil`` — the real Hermes home — not the broken ``~/.Hermes``).
...
def _case_preserving_replacement(replacement: str):
    ...
    ``~/.devil/config.yaml`` (the real Hermes home) instead of the broken
    ``~/.Hermes/config.yaml``.
    """
...
class Migrator:
    ...
    def generate_migration_notes(self) -> None:
        ...
        notes.extend([
            ...
            "- Review `~/.devil/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--target", default=os.environ.get("DEVIL_HOME") or str(Path.home() / ".devil"), help="Hermes home directory")
    parser.add_argument(
        "--workspace-target",
...
def main() -> int:
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.devil/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.devil/config.yaml")
