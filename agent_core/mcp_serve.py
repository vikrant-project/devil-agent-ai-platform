...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using DEVIL_HOME."""
    try:
        from devil_constants import get_devil_home
        return get_devil_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("DEVIL_HOME", Path.home() / ".devil")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from devil_constants import get_devil_home
        directory_file = get_devil_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("DEVIL_HOME", Path.home() / ".devil")
...
class EventBridge:
    ...
    def _poll_once(self, db):
        ...
        try:
            from devil_constants import get_devil_home
            db_file = get_devil_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("DEVIL_HOME", Path.home() / ".devil")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    ...
    mcp = FastMCP(
        "devil",
        instructions=(
            "Devil Agent messaging bridge. Use these tools to interact with "
