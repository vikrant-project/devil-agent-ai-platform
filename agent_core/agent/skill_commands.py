...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``DEVIL_PLATFORM`` env var and
    ``DEVIL_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("DEVIL_PLATFORM")
            or get_session_env("DEVIL_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("DEVIL_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.devil.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_devil_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.devil/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.devil/skills/`` and any ``skills.external_dirs`` so the
