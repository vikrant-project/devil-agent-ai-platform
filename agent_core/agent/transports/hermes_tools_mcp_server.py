...
MCP server (per `~/.codex/config.toml [mcp_servers.devil-tools]`) and
...
Scope (what we expose):
  ...
  - kanban_* (complete/block/comment/    — kanban worker + orchestrator
    ...
    unblock/link)                          write ~/.devil/kanban.db)
...
Run with: python -m agent.transports.devil_tools_mcp_server
...
EXPOSED_TOOLS: tuple[str, ...] = (
    ...
    "skills_list",
    "text_to_speech",
    # Kanban worker handoff tools — gated on DEVIL_KANBAN_TASK env var
    # (set by the kanban dispatcher when spawning a worker). Without these
    # in the callback, a worker spawned with openai_runtime=codex_app_server
    # could do the work but couldn't report completion back to the kernel,
    # making it hang until timeout. Stateless dispatch — they just read
    # the env var and write to ~/.devil/kanban.db.
    "kanban_complete",
    "kanban_block",
    "kanban_comment",
    "kanban_heartbeat",
    ...
    # only — the kanban tool gates them on DEVIL_KANBAN_TASK being unset.
...
def _build_server() -> Any:
    ...
    mcp = FastMCP(
        "hermes-tools",
        instructions=(
            "Devil Agent's tool surface, exposed for use inside a Codex "
...
def main(argv: Optional[list[str]] = None) -> int:
    """Entry point for `python -m agent.transports.devil_tools_mcp_server`."""
    ...
    os.environ.setdefault("DEVIL_QUIET", "1")
    os.environ.setdefault("DEVIL_REDACT_SECRETS", "true")
