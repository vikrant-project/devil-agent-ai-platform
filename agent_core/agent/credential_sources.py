...
Hermes seeds its credential pool from many places:
    ...
    env:<VAR>     — os.environ / ~/.devil/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.devil/.anthropic_oauth.json
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.devil/.env  → clear it
    ...
    """
    from devil_cli.config import get_env_path, remove_env_value
...
def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.devil/.anthropic_oauth.json is ours — delete it outright."""
    from devil_constants import get_devil_home

    result = RemovalResult()
    oauth_file = get_devil_home() / ".anthropic_oauth.json"
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.devil/.anthropic_oauth.json",
    ))
    register(RemovalStep(
