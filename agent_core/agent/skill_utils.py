...
from devil_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``DEVIL_PLATFORM`` or
            ``DEVIL_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.devil/skills/`` are silently skipped.
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against DEVIL_HOME, not cwd
        if not p.is_absolute():
            p = (devil_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.devil/skills/`` first, then external.
