...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - devil_home (str): The active DEVIL_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.devil``.
    ...
    def save_config(self, values: Dict[str, Any], devil_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'hermes memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``devil_home`` is the active DEVIL_HOME directory path.
