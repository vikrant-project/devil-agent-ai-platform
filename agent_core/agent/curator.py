...
def _state_file() -> Path:
    return get_devil_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.devil/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    ...
    "into ~/.devil/skills/.archive/) is the maximum destructive action. "
    ...
    "~/.devil/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.devil/logs/curator/``)
    ...
    authored skill data in ``~/.devil/skills/``.

    ``ensure_devil_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_devil_home() / "logs" / "curator"
    try:
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.devil/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.devil/skills/.archive/` for "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.devil/skills/.archive/`. "
    ...
    lines.append("- All archives live under `~/.devil/skills/.archive/` and are recoverable by `mv`")
