...
``<DEVIL_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
# ``<DEVIL_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def hermes_lsp_bin_dir() -> Path:
    ...
    home = os.environ.get("DEVIL_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".devil")
    p = Path(home) / "lsp" / "bin"
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hermes_lsp_bin_dir().parent  # <DEVIL_HOME>/lsp/
