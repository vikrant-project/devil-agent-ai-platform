...
Nous can attribute usage to Devil Agent and bucket it by client release.
...
The version is sourced live from ``devil_cli.__version__`` so it auto-aligns
...
``devil_cli.__version__`` is the canonical source of truth.
"""
...
def _hermes_version() -> str:
    ...
    Falls back to ``"unknown"`` if ``devil_cli`` cannot be imported (should
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=hermes-agent", devil_client_tag()]
