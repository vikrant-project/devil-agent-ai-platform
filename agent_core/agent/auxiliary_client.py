...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.devil/auth.json active provider)
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``DEVIL_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``DEVIL_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("DEVIL_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("DEVIL_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
# in lockstep with devil_cli.__version__ across every Portal call site
...
def _nous_extra_body() -> dict:
    ...
    Computed at call time so a hot-reloaded ``devil_cli.__version__`` is
...
_AUTH_JSON_PATH = get_devil_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Devil Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``devil_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.devil/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    Falls back to ``devil_cli.auth``'s singleton runtime resolver for older
    auth-store-only logins. Returns ``None`` if the user is not authenticated
    with xAI Grok OAuth.
    """
    try:
        from devil_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from devil_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from devil_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from devil_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from devil_cli.models import get_nous_recommended_aux_model
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    old OPENAI_BASE_URL lingers in ~/.devil/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "OPENAI_BASE_URL from ~/.devil/.env",
...
def resolve_provider_client(
    ...
    try:
        from devil_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("devil_cli.auth not available for provider %s", provider)
        return None, None
