...
as ``kind: backend``) or ``~/.devil/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$DEVIL_HOME/cache/images/``, creating parents as needed."""
    from devil_constants import get_devil_home

    path = get_devil_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$DEVIL_HOME/cache/images/``.
