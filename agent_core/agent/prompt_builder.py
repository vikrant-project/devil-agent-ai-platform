...
from devil_constants import get_devil_home, get_skills_dir, is_wsl
...
_DEVIL_MD_NAMES = (".devil.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.devil.md`` or ``HERMES.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Devil Agent, an intelligent AI assistant created by Nous Research. "
...
DEVIL_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Devil Agent "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.devil/kanban.db`. Your task id is in "
    "`$DEVIL_KANBAN_TASK`; your workspace is `$DEVIL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $DEVIL_KANBAN_WORKSPACE` before "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_devil_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.devil/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("DEVIL_PLATFORM")
        or get_session_env("DEVIL_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Devil Agent itself — its CLI, config, models, providers, tools, "
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from DEVIL_HOME and return its content, or None.
    ...
    """
    try:
        from devil_cli.config import ensure_devil_home
        ensure_devil_home()
    except Exception as e:
        logger.debug("Could not ensure DEVIL_HOME before loading SOUL.md: %s", e)

    soul_path = get_devil_home() / "SOUL.md"
...
def _load_hermes_md(cwd_path: Path) -> str:
    """.devil.md / HERMES.md — walk to git root."""
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".devil.md")
    except Exception as e:
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .devil.md / HERMES.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from DEVIL_HOME is independent and always included when present.
    ...
    # SOUL.md from DEVIL_HOME only — skip when already loaded as identity
    if not skip_soul:
