...
as ``kind: backend``) or ``~/.devil/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _videos_cache_dir() -> Path:
    """Return ``$DEVIL_HOME/cache/videos/``, creating parents as needed."""
    from devil_constants import get_devil_home

    path = get_devil_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$DEVIL_HOME/cache/videos/``.
