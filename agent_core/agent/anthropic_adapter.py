"""Anthropic Messages API adapter for Devil Agent.
...
# Stores credentials in ~/.devil/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_DEVIL_OAUTH_FILE = get_devil_home() / ".anthropic_oauth.json"
...
def read_hermes_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Hermes-managed OAuth credentials from ~/.devil/.anthropic_oauth.json."""
    if _DEVIL_OAUTH_FILE.exists():
        try:
            data = json.loads(_DEVIL_OAUTH_FILE.read_text(encoding="utf-8"))
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Devil Agent", "Claude Code")
