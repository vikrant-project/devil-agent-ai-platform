...
Consent records live under ``~/.devil/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.devil/config.yaml``
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.devil/config.yaml.")
