...
def resolve_hermes_bin() -> Optional[str]:
    ...
    Priority:
      ...
      2. ``shutil.which("devil")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m devil_cli.main``.
    ...
    ``hermes.exe`` on PATH or the ``sys.executable -m devil_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "devil_cli.main"]
...
def relaunch(
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    ``python -m devil_cli.main``) or a ``.cmd`` batch file, and both
