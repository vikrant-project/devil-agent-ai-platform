...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hermes-agent;
   ...
   own discovery paths)
2. **User plugins**   – ``~/.devil/plugins/<name>/``
3. **Project plugins** – ``./.devil/plugins/<name>/`` (opt-in via
   ``DEVIL_ENABLE_PROJECT_PLUGINS``)
...
# ---------------------------------------------------------------------------
#
# Set ``DEVIL_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.devil/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("DEVIL_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When DEVIL_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.
    ...
    logger.debug(
        "DEVIL_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
class PluginManifest:
    ...
    #              in ~/.devil/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
...
class PluginContext:
    ...
    def register_skill(
        ...
        ``~/.devil/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to devil_cli/. Two layouts are
        ...
        # 2. User plugins (~/.devil/plugins/)
        user_dir = get_devil_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.devil/plugins/)
        if _env_enabled("DEVIL_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".devil" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set DEVIL_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
