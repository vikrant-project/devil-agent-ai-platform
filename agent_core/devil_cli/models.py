...
_DEVIL_USER_AGENT = f"hermes-cli/{_DEVIL_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    """
    from devil_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $DEVIL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $DEVIL_HOME/models_dev_cache.json directly (no network) so this
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from devil_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from devil_cli.model_catalog import get_curated_openrouter_models
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from devil_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from devil_constants import AI_GATEWAY_BASE_URL
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from devil_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from devil_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``devil_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         when the env var is set in ``~/.devil/.env``.
    ...
    """
    try:
        from devil_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from devil_cli.auth import read_credential_pool
        from devil_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from devil_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _DEVIL_USER_AGENT}
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _DEVIL_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_devil_home() / "ollama_cloud_models_cache.json"
