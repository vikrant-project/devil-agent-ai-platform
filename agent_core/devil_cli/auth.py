"""
Multi-provider authentication system for Devil Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.devil/auth.json with cross-process file locking.
...
from devil_cli.config import get_devil_home, get_config_path, read_raw_config
from devil_constants import OPENROUTER_BASE_URL
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from devil_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.devil/.env file
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("DEVIL_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.devil/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_devil_home() / "auth.json"
    # Seat belt: if pytest is running and DEVIL_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch DEVIL_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".devil" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set DEVIL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom DEVIL_HOME that is not a profile).
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.devil/auth.json`` even when DEVIL_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hermes_root()`` for a profile-shaped DEVIL_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".devil" / "auth.json"
            try:
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from devil_cli.config import validate_config_structure
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        ...
        "OPENAI_API_KEY, etc.) in ~/.devil/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("DEVIL_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.devil/auth/google_oauth.json (managed by agent.google_oauth).
...
# Spotify auth — PKCE tokens stored in ~/.devil/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("DEVIL_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set DEVIL_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("DEVIL_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("DEVIL_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("DEVIL_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.devil/.env, and return it.
    ...
    save_env_value("DEVIL_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("DEVIL_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved DEVIL_SPOTIFY_CLIENT_ID to ~/.devil/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "DEVIL_SPOTIFY_CLIENT_ID is required".
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.devil/auth.json (not ~/.codex/)
#
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.devil/auth.json).
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.devil/auth.json)."""
...
def resolve_codex_runtime_credentials(
    *,
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DEVIL_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = (
        os.getenv("DEVIL_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.devil/auth.json
# =============================================================================
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.devil/auth.json``. A single MITM during initial
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("DEVIL_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = (
        os.getenv("DEVIL_XAI_BASE_URL", "").strip().rstrip("/")
...
#
# File lives at ${DEVIL_SHARED_AUTH_DIR}/nous_auth.json, defaulting to
# ``<hermes-root>/shared/nous_auth.json`` where ``<hermes-root>`` is what
# ``get_default_hermes_root()`` returns — ``~/.devil`` on Linux/macOS,
# ``%LOCALAPPDATA%\hermes`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's DEVIL_HOME so named profiles (which
...
def _nous_shared_auth_dir() -> Path:
    ...
    Honors ``DEVIL_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hermes-root>/shared/``, where ``<hermes-root>`` is what
    :func:`devil_constants.get_default_hermes_root` returns — so
    Linux/macOS classic installs land at ``~/.devil/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\hermes\\shared\\``, and
    Docker / custom ``DEVIL_HOME`` deployments at
    ``<DEVIL_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("DEVIL_SHARED_AUTH_DIR", "").strip()
    if override:
...
def _nous_shared_store_path() -> Path:
    ...
    # state. Tests must set DEVIL_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                ...
                f"{path}. Set DEVIL_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _nous_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    try:
        ...
        # No DEVIL_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    *,
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.devil/auth.json) called POST /api/oauth/token with Hermes's "
    ...
    *,
    ...
    with _auth_store_lock():
        ...
        portal_base_url = (
            ...
            or os.getenv("DEVIL_PORTAL_BASE_URL")
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("DEVIL_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("DEVIL_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set DEVIL_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _prompt_model_selection(
    ...
    """
    from devil_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from devil_cli.config import save_config, load_config
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.devil/auth.json."""
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("DEVIL_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.devil/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
