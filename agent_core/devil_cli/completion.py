...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Devil Agent bash completion
# Add to ~/.bashrc:
#   eval "$(hermes completion bash)"

_hermes_profiles() {{
    local profiles_dir="$HOME/.devil/profiles"
...
_hermes_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.devil/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.devil/profiles 2>/dev/null)}}")
    fi
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        ...
        "    echo default",
        "    if test -d $HOME/.devil/profiles",
        "        ls $HOME/.devil/profiles 2>/dev/null",
        "    end",
        "end",
        "",
