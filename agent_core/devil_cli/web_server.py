"""
Devil Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m devil_cli.main web          # Start on http://127.0.0.1:9119
    python -m devil_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["DEVIL_WEB_DIST"]) if "DEVIL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Devil Agent", version=__version__)
...
# or DEVIL_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.devil/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_devil_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    Uses the running interpreter's ``devil_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "devil_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "DEVIL_NONINTERACTIVE": "1"},
    }
...
# in devil_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.devil/config.yaml`` — applies to **new** sessions only.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.devil/.anthropic_oauth.json`` — Hermes-managed PKCE flow
    ...
    try:
        ...
        _DEVIL_OAUTH_FILE = None  # type: ignore
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Hermes PKCE ({_DEVIL_OAUTH_FILE})",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from devil_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _DEVIL_OAUTH_FILE
            if _DEVIL_OAUTH_FILE.exists():
                _DEVIL_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from devil_cli.auth import clear_provider_auth
...
#          → persists to ~/.devil/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _DEVIL_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _DEVIL_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from devil_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("DEVIL_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from devil_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from devil_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from devil_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_devil_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from devil_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_devil_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from devil_cli import profiles as profiles_mod
    try:
...
async def get_skills():
    ...
    from devil_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from devil_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from devil_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Session resume is propagated via the ``DEVIL_TUI_RESUME`` env var —
    matching what ``devil_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``DEVIL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from devil_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("DEVIL_TUI_DISABLE_MOUSE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["DEVIL_TUI_RESUME"] = resume

    if sidecar_url:
        env["DEVIL_TUI_SIDECAR_URL"] = sidecar_url
...
# DEVIL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__DEVIL_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        token_script = (
            f'<script>window.__DEVIL_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__DEVIL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__DEVIL_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.devil/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.devil/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_devil_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.devil/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as devil_cli.plugins):
    1. User plugins:    ~/.devil/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.devil/plugins/  (only if DEVIL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from devil_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_devil_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("DEVIL_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".devil" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from devil_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_devil_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from devil_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from devil_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from devil_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from devil_cli.plugins_cmd import dashboard_remove_user_plugin
