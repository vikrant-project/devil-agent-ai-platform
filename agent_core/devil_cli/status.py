...
Shows the status of all Devil Agent components.
"""
...
from devil_cli.auth import AuthError, resolve_provider
from devil_cli.colors import Colors, color
from devil_cli.config import get_env_path, get_env_value, get_devil_home, load_config
from devil_cli.models import provider_label
from devil_cli.nous_subscription import get_nous_subscription_features
from devil_cli.runtime_provider import resolve_requested_provider
...
def show_status(args):
    """Show status of all Devil Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Devil Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if _effective_provider_label() == "LM Studio":
        from devil_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from devil_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_devil_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_devil_home() / "sessions" / "sessions.json"
