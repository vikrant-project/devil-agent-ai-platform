"""``hermes debug`` debug tools for Devil Agent.
...
Currently supports:
    hermes debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.devil/logs/*.log`` are not leaked into
...
def _pending_file() -> Path:
    """Path to ``~/.devil/pastes/pending.json``.
    ...
    """
    return get_devil_home() / "pastes" / "pending.json"
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    The replacement is stateless: we append to ``~/.devil/pastes/pending.json``
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from devil_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_devil_home() / "logs" / filename) if filename else None
