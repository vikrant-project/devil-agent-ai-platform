...
The actual runtime resolution happens in devil_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in ("off", "default", "disable", "devil"):
        return "auto", []
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        try:
            from devil_cli.codex_runtime_plugin_migration import migrate
