...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from devil_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from devil_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from devil_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("DEVIL_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/devil_cli/test_runtime_provider_resolution.py
    ...
    *,
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.devil/.env or run 'hermes model' to configure."
        )
    ...
    *,
    ...
    if pool and pool.has_credentials():
        ...
        if provider == "nous" and entry is not None and pool_api_key:
            min_ttl = max(60, int(os.getenv("DEVIL_NOUS_MIN_KEY_TTL_SECONDS", "1800")))
            nous_state = {
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from devil_cli.auth import resolve_minimax_oauth_runtime_credentials
