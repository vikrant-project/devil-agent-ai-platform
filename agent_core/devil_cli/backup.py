...
`hermes backup` creates a zip archive of the entire ~/.devil/ directory
...
from devil_constants import get_default_hermes_root, get_devil_home, display_devil_home
...
def run_backup(args) -> None:
    ...
    # Collect files
    print(f"Scanning {display_devil_home()} ...")
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.devil/config.yaml` instead of `config.yaml`.
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_devil_home()}")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # devil_cli.profiles might not be available (fresh install)
...
# Critical state files to include in quick snapshots (relative to DEVIL_HOME).
...
def _quick_snapshot_root(devil_home: Optional[Path] = None) -> Path:
    home = devil_home or get_devil_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    devil_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(devil_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(devil_home), keep=keep)
...
def run_quick_backup(args) -> None:
    ...
    if snap_id:
        ...
        print(f"  {len(snaps)} snapshot(s) stored in {display_devil_home()}/state-snapshots/")
...
def _pre_update_backup_dir(devil_home: Optional[Path] = None) -> Path:
    home = devil_home or get_devil_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of DEVIL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<DEVIL_HOME>/backups/pre-update-<timestamp>.zip`` and
    ...
    """
    hermes_root = devil_home or get_default_hermes_root()
    if not hermes_root.is_dir():
        return None
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of DEVIL_HOME under ``backups/`` before a
    ...
    ``<DEVIL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
