...
from devil_cli.config import get_devil_home, get_env_path, get_project_root, load_config
...
def _gateway_status() -> str:
    ...
    try:
        from devil_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(devil_home: Path) -> int:
...
def _cron_summary(devil_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = devil_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"devil_home:      {display_devil_home()}")
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(devil_home)}")
    lines.append(f"  skills:             {_count_skills(devil_home)}")
