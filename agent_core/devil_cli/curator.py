...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.devil/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    print(
        "\nThis will replace the current ~/.devil/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from devil_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.devil/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.devil/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by devil_cli.main fallthrough)."""
