...
Diagnoses issues with Devil Agent setup.
"""
...
from devil_cli.config import get_project_root, get_devil_home, get_env_path
...
_DHH = display_devil_home()  # user-facing display path (e.g. ~/.devil or ~/.devil/profiles/coder)

# Load environment variables from ~/.devil/.env so API key checks work
_env_path = get_env_path()
load_devil_dotenv(devil_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from devil_cli.colors import Colors, color
from devil_cli.models import _DEVIL_USER_AGENT
from devil_cli.vercel_auth import describe_vercel_auth
from devil_constants import OPENROUTER_MODELS_URL
...
from devil_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.devil/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("DEVIL_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("DEVIL_KANBAN_TASK"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from devil_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    os.environ.setdefault("DEVIL_INTERACTIVE", "1")
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.devil/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    # Check ~/.devil/.env (primary location for user config)
    env_path = DEVIL_HOME / '.env'
    if env_path.exists():
    ...
    # Check ~/.devil/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from devil_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from devil_cli.providers import (
            ...
            if runtime_provider and runtime_provider not in {"auto", "custom", "openrouter"}:
                try:
                    from devil_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.devil/.env or run 'hermes setup')",
                            )
                            issues.append(
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from devil_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
        try:
            from devil_cli.config import check_config_version, migrate_config
    ...
    devil_home = DEVIL_HOME
    if devil_home.exists():
        check_ok(f"{_DHH} directory exists")
    elif should_fix:
        devil_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = devil_home / subdir_name
    ...
    memories_dir = devil_home / "memories"
    ...
    state_db_path = devil_home / "state.db"
    ...
    wal_path = devil_home / "state.db-wal"
    if wal_path.exists():
        try:
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "devil"
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = DEVIL_HOME / "config.yaml"
    ...
    # =========================================================================
    try:
        from devil_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
