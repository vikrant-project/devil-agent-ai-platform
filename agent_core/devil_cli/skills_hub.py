...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.devil/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.devil/skills/<name>/)[/]"
        )
    try:
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                f"Files will be at: [cyan]{display_devil_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
...
def do_list(source_filter: str = "all",
    ...
    ``skills.disabled`` list because ``-p`` swaps ``DEVIL_HOME`` at process
    ...
    if target == "github":
        ...
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_devil_home()}/.env or run 'gh auth login'.\n")
            return
...
def skills_command(args) -> None:
    """Router for `hermes skills <subcommand>` — called from devil_cli/main.py."""
