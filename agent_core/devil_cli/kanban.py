...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this DEVIL_HOME and its
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    # DEVIL_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "DEVIL_KANBAN_BOARD env var). Use `hermes kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $DEVIL_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $DEVIL_PROFILE or 'user')")
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.devil/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $DEVIL_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    ...
    # env-var pin for the duration of this call. Using DEVIL_KANBAN_BOARD
    ...
    prev_board_env = os.environ.get("DEVIL_KANBAN_BOARD")
    ...
    def _restore_board_env() -> None:
        ...
        if prev_board_env is None:
            os.environ.pop("DEVIL_KANBAN_BOARD", None)
        else:
            os.environ["DEVIL_KANBAN_BOARD"] = prev_board_env
    if board_override:
        try:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            ...
            return 1
        os.environ["DEVIL_KANBAN_BOARD"] = normed
    ...
    # DEVIL_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("DEVIL_PROFILE_NAME", "DEVIL_PROFILE"):
...
def _dispatch_boards(args: argparse.Namespace) -> int:
    ...
    DB, so a fresh DEVIL_HOME that has never called ``kanban init``
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.devil/profiles/.")
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from devil_cli import kanban_diagnostics as kd
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("DEVIL_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("DEVIL_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from devil_cli import kanban_specify as spec
