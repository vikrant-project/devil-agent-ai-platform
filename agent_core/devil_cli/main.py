...
Usage:
    ...
    hermes uninstall           Uninstall Devil Agent
...
# IMPORTANT: devil_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``devil_bootstrap`` is a
...
# new code references ``devil_bootstrap`` but the editable install's
...
try:
    import devil_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to DEVIL_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
#
# Many modules cache DEVIL_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("DEVIL_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.devil/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set DEVIL_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors devil_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If DEVIL_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.devil/profiles/coder or
    # /opt/data/profiles/coder).  If DEVIL_HOME points to the hermes root
    # instead (e.g. systemd hardcodes DEVIL_HOME=/root/.devil), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    devil_home_env = os.environ.get("DEVIL_HOME", "")
    if profile_name is None and devil_home_env:
        if Path(devil_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the hermes root
    if profile_name is None:
        try:
            from devil_constants import get_default_hermes_root
    ...
    # 3. If we found a profile, resolve and set DEVIL_HOME
    if profile_name is not None:
        try:
            from devil_cli.profiles import resolve_profile_env

            devil_home = resolve_profile_env(profile_name)
            ...
            return
        os.environ["DEVIL_HOME"] = devil_home
...
# Load .env from ~/.devil/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from devil_cli.config import get_devil_home
from devil_cli.env_loader import load_devil_dotenv

load_devil_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → DEVIL_REDACT_SECRETS env
# var BEFORE devil_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "DEVIL_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_devil_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["DEVIL_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
...
try:
    from devil_logging import setup_logging as _setup_logging
...
try:
    from devil_cli.config import load_config as _load_config_early
    from devil_constants import apply_ipv4_preference as _apply_ipv4
...
from devil_cli import __version__, __release_date__
from devil_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from devil_cli.config import get_env_path, get_devil_home, load_config
    from devil_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from devil_cli.config import DEFAULT_CONFIG
    ...
    from devil_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_devil_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from devil_state import SessionDB
...
def _ensure_tui_node() -> None:
    ...
    ``DEVIL_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("DEVIL_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    devil_home = os.environ.get("DEVIL_HOME") or str(Path.home() / ".devil")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "DEVIL_HOME": devil_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(devil_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(devil_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if devil_cli_dir is None:
        devil_cli_dir = Path(__file__).parent
    bundled = devil_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (DEVIL_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("DEVIL_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from devil_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("DEVIL_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with DEVIL_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset DEVIL_TUI_DIR (e.g. `unset DEVIL_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("DEVIL_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from devil_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env["DEVIL_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["DEVIL_PYTHON_SRC_ROOT"] = os.environ.get(
        "DEVIL_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("DEVIL_PYTHON", sys.executable)
    env.setdefault("DEVIL_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["DEVIL_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["DEVIL_MODEL"] = model
        env["DEVIL_INFERENCE_MODEL"] = model
    if provider:
        env["DEVIL_TUI_PROVIDER"] = provider
        env["DEVIL_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["DEVIL_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["DEVIL_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["DEVIL_TUI_SKILLS"] = value
    if query:
        env["DEVIL_TUI_QUERY"] = query
    if image:
        env["DEVIL_TUI_IMAGE"] = image
    if checkpoints:
        env["DEVIL_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["DEVIL_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["DEVIL_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["DEVIL_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["DEVIL_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["DEVIL_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["DEVIL_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``DEVIL_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("DEVIL_KANBAN_BOARD"):
        return
    try:
        from devil_cli.kanban_db import get_current_board

        os.environ["DEVIL_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("DEVIL_TUI") == "1"
    ...
    try:
        from devil_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["DEVIL_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.devil/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["DEVIL_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["DEVIL_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["DEVIL_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from devil_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from devil_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from devil_cli.config import get_env_value, save_env_value
    ...
    session_dir = get_devil_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Devil Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_setup(args):
    ...
    from devil_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    print("⚕ Devil post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    persistence.
    """
    from devil_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from devil_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from devil_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("DEVIL_INFERENCE_PROVIDER") or "auto"
    )
    ...
    from devil_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from devil_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.devil/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.devil/.env if the active provider is not 'custom'.
    ...
    """
    from devil_cli.config import get_env_value, save_env_value, load_config
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from devil_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from devil_cli.config import load_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    """
    from devil_cli.config import load_config
    from devil_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from devil_cli.auth import _prompt_model_selection
    from devil_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from devil_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from devil_cli.config import get_env_value

    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.devil/.env by hand. The
    ...
    from devil_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.devil/.env by hand.
    ...
    from devil_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from devil_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from devil_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from devil_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from devil_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model=""):
    ...
    from devil_cli.models import _PROVIDER_MODELS
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from devil_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from devil_cli.auth import _save_model_choice, deactivate_provider
    from devil_cli.config import get_env_value, load_config, save_config
    ...
    from devil_cli.models import probe_api_models
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from devil_cli.runtime_provider import _detect_api_mode_for_url
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from devil_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from devil_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from devil_cli import azure_detect
    import getpass
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from devil_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from devil_cli.config import save_env_value, load_config, save_config
    from devil_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from devil_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from devil_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set DEVIL_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    recover from a malformed paste without editing ``~/.devil/.env`` by hand.
    ...
    from devil_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from devil_cli.config import save_env_value
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from devil_cli.auth import AuthError
        from devil_cli.models import fetch_lmstudio_models
        ...
        from devil_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from devil_constants import display_devil_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from devil_cli.auth import get_anthropic_key
...
def cmd_login(args):
    ...
    from devil_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from devil_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from devil_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from devil_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from devil_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from devil_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from devil_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from devil_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from devil_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from devil_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from devil_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from devil_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from devil_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from devil_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from devil_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from devil_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Devil Agent v{__version__} ({__release_date__})")
    ...
    try:
        from devil_cli.banner import check_for_updates
        from devil_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Devil Agent."""
    _require_tty("uninstall")
    from devil_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    from devil_constants import get_devil_home
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``devil_cli/web_dist/`` (per vite.config.ts
    outDir: "../devil_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "devil_cli" / "web_dist"
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    # build path usable on a stock `py -m devil_cli.main web` invocation.
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    patterns = [
        "hermes dashboard",
        "devil_cli.main dashboard",
        "devil_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # with `devil_cli.gateway._scan_gateway_pids` and avoids the
...
def _update_via_zip(args):
    """Update Devil Agent by downloading a ZIP archive.
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_devil_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_devil_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
class _UpdateOutputStream:
    ...
    * Every write is also mirrored to an append-only log file
      (``~/.devil/logs/update.log``) that users can inspect after the
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.devil/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    try:
        ...
        # devil_cli.config.get_devil_home to simulate setup failure.
        from devil_cli.config import get_devil_home as _get_devil_home

        logs_dir = _get_devil_home() / "logs"
...
def _cmd_update_check():
    ...
    from devil_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from devil_cli.config import recommended_update_command
        from devil_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# Devil Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of DEVIL_HOME before running the update.
    ...
    DEVIL_HOME directories.  The ``--backup`` flag on ``hermes update``
    ...
    try:
        from devil_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_devil_home so the user sees ~/.devil/...
    try:
        from devil_constants import get_devil_home, display_devil_home

        home = get_devil_home()
        try:
            display_path = f"{display_devil_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def cmd_update(args):
    """Update Devil Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from devil_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Devil Agent")
        return
...
def _cmd_update_pip(args):
    ...
    from devil_cli import __version__
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_devil_home added to devil_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload devil_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_devil_home() added since the last release.
        try:
            import importlib
            import devil_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit DEVIL_HOME so
        # it is not affected by sync_skills()'s module-level DEVIL_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's DEVIL_HOME env var points at the default or a named profile.
        try:
            from devil_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from devil_cli.tools_config import install_cua_driver
        ...
        if gateway_mode:
            _exit_code_path = get_devil_home() / ".update_exit_code"
            try:
...
def cmd_profile(args):
    ...
    from devil_constants import display_devil_home
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.devil)")
            else:
        ...
        from devil_cli.profiles import profile_exists
        ...
        from devil_cli.profiles import rename_profile
        ...
        from devil_cli.profiles import export_profile
        ...
        from devil_cli.profiles import import_profile
        ...
        import tempfile
        from devil_cli.profile_distribution import (
            plan_install,
        ...
        from devil_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        from devil_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from devil_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.devil_requires:
        print(f"  Requires: Hermes {mf.devil_requires}")
...
def cmd_dashboard(args):
    ...
    if "DEVIL_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["DEVIL_WEB_DIST"])
            if "DEVIL_WEB_DIST" in os.environ
            else PROJECT_ROOT / "devil_cli" / "web_dist"
        )
    ...
    from devil_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("DEVIL_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from devil_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from devil_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from devil_cli.models import CANONICAL_PROVIDERS as _cp
...
# the top-level flags declared in ``devil_cli/_parser.py``.
#
...
def main():
    ...
    try:
        from devil_cli.stdio import configure_windows_stdio
    ...
    from devil_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from devil_cli.fallback_cmd import cmd_fallback
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Devil Agent with an interactive wizard. "
    ...
    slack_manifest.add_argument(
        ...
        "writes to $DEVIL_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Devil Agent")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set DEVIL_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Devil Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.devil/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.devil/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from devil_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.devil/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.devil/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Devil Agent setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Devil Agent. Use 'hermes debug share' to "
...
""",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.devil/checkpoints/",
    ...
    from devil_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Devil Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from devil_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.devil/skills/.bundled_manifest) "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from devil_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from devil_cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from devil_cli.plugins_cmd import plugins_command
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from devil_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from devil_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_devil_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_devil_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_devil_home()}/memories/\n")
        else:
            from devil_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from devil_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from devil_cli.tools_config import tools_command
    ...
    def cmd_mcp(args):
        from devil_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_devil_home() / "sessions"
            ...
            from devil_cli.relaunch import relaunch
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.devil/ (by default a "
        "single restore-point archive is written to ~/.devil/backups/ "
    ...
    def cmd_claw(args):
        from devil_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Devil Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Devil Agent",
        description="Remove Devil Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Devil Agent as an ACP (Agent Client Protocol) server",
        description="Start Devil Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.devil/node/ "
    ...
    def cmd_acp(args):
        """Launch Devil Agent as an ACP server."""
        try:
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Devil Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        help=(
            ...
            "Alternatively set DEVIL_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
...
""",
    ...
    from devil_cli.config import get_container_exec_info
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from devil_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from devil_cli.oneshot import run_oneshot
