"""
Devil Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.devil/ (configs, sessions, logs)
"""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Devil Agent" comment and following line
                if '# Devil Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains devil_cli reference)
                content = wrapper.read_text()
                if 'devil_cli' in content or 'hermes-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    try:
        from devil_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from devil_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``DEVIL_HOME`` and ``DEVIL_GIT_BASH_PATH``
...
def remove_path_from_windows_registry(devil_home: Path) -> list[str]:
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _hermes_path_markers(devil_home)
            kept: list[str] = []
...
def remove_hermes_env_vars_windows() -> list[str]:
    """Delete DEVIL_HOME and DEVIL_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("DEVIL_HOME", "DEVIL_GIT_BASH_PATH"):
                try:
...
def remove_portable_tooling_windows(devil_home: Path) -> list[Path]:
...
def _is_default_devil_home(devil_home: Path) -> bool:
    """Return True when ``devil_home`` points at the default (non-profile) root."""
    try:
        from devil_constants import get_default_hermes_root
        return devil_home.resolve() == get_default_hermes_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its DEVIL_HOME directory.
    ...
    current DEVIL_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m devil_cli.main` so we don't depend on a `hermes`
    #    wrapper that may be half-removed mid-uninstall.
    hermes_invocation = [_sys.executable, "-m", "devil_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's DEVIL_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.devil/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.devil/ for future reinstall
    """
    project_root = get_project_root()
    devil_home = get_devil_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie DEVIL_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_devil_home(devil_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Devil Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {devil_home / 'config.yaml'}")
    print(f"  Secrets: {devil_home / '.env'}")
    print(f"  Data:    {devil_home / 'cron/'}, {devil_home / 'sessions/'}, {devil_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their DEVIL_HOME dirs. Otherwise
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in devil_home so the marker matching is
        ...
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(devil_home))))
        ...
        log_info("Removing DEVIL_HOME / DEVIL_GIT_BASH_PATH User env vars...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.devil/, just remove the hermes-agent subdir
            if devil_home in project_root.parents or project_root.parent == devil_home:
    ...
    #     under DEVIL_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(devil_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(devil_home)
    ...
    # 5. Optionally remove ~/.devil/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile DEVIL_HOME dirs live under
        ...
        try:
            if devil_home.exists():
                shutil.rmtree(devil_home)
                log_success(f"Removed {devil_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {devil_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {devil_home}")
    ...
    print()
    print("Thank you for using Devil Agent! ⚕")
    print()
