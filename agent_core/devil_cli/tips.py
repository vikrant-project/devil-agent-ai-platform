...
TIPS = [
    ...
    "DEVIL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    "The plan skill saves markdown plans under .devil/plans/ in the active workspace.",
    ...
    "Context files (.devil.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.devil/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Hermes loads project context from .devil.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "Checkpoints use shadow repos in ~/.devil/checkpoints/ — your project's .git is never touched.",
    ...
    "DEVIL_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    ...
    "Cron scripts live in ~/.devil/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.devil/SOUL.md to customize.",
    ...
    "The skills audit log at ~/.devil/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at DEVIL_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "DEVIL_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in DEVIL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.devil/interrupt_debug.log with timestamps.",
    ...
    "DEVIL_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hermes tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.devil/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.devil/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.devil/.env into the running session — pick up new API keys without restarting.',
    ...
    "DEVIL_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.devil/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.devil/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    # --- TUI & Dashboard ---
    'DEVIL_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "DEVIL_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'Drop a YAML in ~/.devil/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.devil/dashboard-plugins/ — no npm build required.',
    ...
    'DEVIL_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'DEVIL_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'DEVIL_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'DEVIL_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.devil/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.devil/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hermes chat --ignore-user-config skips ~/.devil/config.yaml — reproducible bug reports and CI runs.',
    ...
    'DEVIL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'DEVIL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.devil/plugins/ — trust-gated by design.',
    "DEVIL_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'DEVIL_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'DEVIL_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'DEVIL_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'DEVIL_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'DEVIL_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'DEVIL_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'DEVIL_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'DEVIL_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'DEVIL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'DEVIL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.devil/dashboard-plugins/.',
]
