"""
Interactive setup wizard for Devil Agent.
...
from devil_cli.nous_subscription import get_nous_subscription_features
...
# display_devil_home imported lazily at call sites (stale-module safety during hermes update)
...
from devil_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Devil Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from devil_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from devil_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, devil_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from devil_cli.plugins import _ensure_plugins_discovered
    ...
    try:
        ...
        from devil_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    try:
        from devil_cli.auth import get_provider_auth_state
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {devil_home}/cron/, sessions/, logs/"
    )
    print()
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from devil_cli.main import select_provider_and_model
    try:
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from devil_cli.auth_commands import auth_add_command
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    """
    try:
        from devil_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.devil/.env."
            )
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into DEVIL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("DEVIL_MAX_ITERATIONS")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # DEVIL_MAX_ITERATIONS from agent.max_turns at startup.
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under DEVIL_HOME, and print
    ...
    """
    try:
        from devil_cli.slack_cli import _build_full_manifest
        from devil_constants import get_devil_home

        manifest = _build_full_manifest(
            bot_name="Devil Agent",
            bot_description="Your Hermes agent on Slack",
        )
        target = Path(get_devil_home()) / "slack-manifest.json"
...
def _setup_qqbot():
    ...
    from devil_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from devil_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``devil_cli.auth`` — lists every supported
    ...
    """
    try:
        from devil_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from devil_cli.config
    ...
    if section_key == "model":
        ...
        from devil_cli.gateway import _all_platforms, _platform_status
...
def _offer_openclaw_migration(devil_home: Path) -> bool:
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=devil_home.resolve(),
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Devil Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Devil Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Devil Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, devil_home)
            return
        ...
        migration_ran = _offer_openclaw_migration(devil_home)
        if migration_ran:
            config = load_config()
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, devil_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {devil_home}")
    ...
    _print_setup_summary(config, devil_home)


def _run_first_time_quick_setup(config: dict, devil_home, is_existing: bool):
...
def _run_quick_setup(config: dict, devil_home):
