...
Each profile is a fully independent DEVIL_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.devil/profiles/<name>/`` by default.

The "default" profile is ``~/.devil`` itself — backward compatible,
...
_PROFILE_DIRS = [
    ...
    # See devil_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.devil``).  Named profiles never contain these
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.devil``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_devil_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.devil) profile.
...
_RESERVED_NAMES = frozenset({
    "devil", "default", "test", "tmp", "root", "sudo",
})

# Hermes subcommands that cannot be used as profile names/aliases
_DEVIL_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the hermes root, NOT to the current DEVIL_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where DEVIL_HOME points outside
    ``~/.devil``, profiles live under ``DEVIL_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_devil_home() / "profiles"


def _get_default_devil_home() -> Path:
    """Return the default (pre-profile) DEVIL_HOME path.

    In standard deployments this is ``~/.devil``.
    In Docker/custom deployments where DEVIL_HOME is outside ``~/.devil``
    (e.g. ``/opt/data``), returns DEVIL_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_devil_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DEVIL_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_devil_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    if canon in _DEVIL_SUBCOMMANDS:
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_devil_home()
...
def create_profile(
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.devil)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.devil/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DEVIL_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DEVIL_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.devil).\n"
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set DEVIL_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("DEVIL_HOME")
    try:
        os.environ["DEVIL_HOME"] = str(profile_dir)
        from devil_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["DEVIL_HOME"] = old_home
        elif "DEVIL_HOME" in os.environ:
            del os.environ["DEVIL_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.devil/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DEVIL_HOME.

    Returns ``"default"`` if DEVIL_HOME is not set or points to ``~/.devil``.
    Returns the profile name if DEVIL_HOME points into ``~/.devil/profiles/<name>``.
    Returns ``"custom"`` if DEVIL_HOME is set to an unrecognized path.
    """
    from devil_constants import get_devil_home
    devil_home = get_devil_home()
    resolved = devil_home.resolve()

    default_resolved = _get_default_devil_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.devil itself — its parent is ~/ and its
        # directory name is ".devil", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    # dir.  Importing as "default" would target ~/.devil itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.devil). "
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_devil_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a DEVIL_HOME path string.

    Called early in the CLI entry point, before any hermes modules
    are imported, to set the DEVIL_HOME environment variable.
    """
