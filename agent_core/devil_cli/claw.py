...
from devil_cli.config import get_devil_home, get_config_path, load_config, save_config
...
def _cmd_migrate(args):
    ...
    print_info(f"Target:      {devil_home}")
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.devil/. "
    ...
    # Delegates to devil_cli.backup.create_pre_migration_backup(), which
    ...
    if not no_backup:
        try:
            from devil_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(devil_home=devil_home)
            if backup_archive:
