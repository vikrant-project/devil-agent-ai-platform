...
configuration in ~/.devil/config.yaml under the ``mcp_servers`` key.
"""
...
from devil_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_devil_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from devil_cli.cli_output import prompt as _shared_prompt
...
def cmd_mcp_add(args):
    ...
    # devil_cli/main.py for why the dest is renamed.
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_devil_home()}/.env as {env_key}")
    ...
    print()
    _success(f"Saved '{name}' to {display_devil_home()}/config.yaml ({tool_count}/{total} tools enabled)")
