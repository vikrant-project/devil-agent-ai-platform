...
class DistributionManifest:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.devil_requires:
            out["hermes_requires"] = self.devil_requires
        if self.author:
...
def plan_install(
    ...
    check_hermes_requires(manifest.devil_requires, hermes_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.devil).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
