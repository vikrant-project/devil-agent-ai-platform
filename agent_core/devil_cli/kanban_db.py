...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``DEVIL_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``DEVIL_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
In standard installs ``<root>`` is ``~/.devil``. In Docker / custom
deployments where ``DEVIL_HOME`` points outside ``~/.devil`` (e.g.
``/opt/hermes``), ``<root>`` is ``DEVIL_HOME``. Legacy env-var
overrides still work:

* ``DEVIL_KANBAN_DB`` — pin the database file path directly.
* ``DEVIL_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``DEVIL_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``DEVIL_KANBAN_DB``,
``DEVIL_KANBAN_WORKSPACES_ROOT``, and ``DEVIL_KANBAN_BOARD`` into
...
def kanban_home() -> Path:
    ...
    1. ``DEVIL_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hermes_root()``, which already returns ``<root>``
       when ``DEVIL_HOME`` is ``<root>/profiles/<name>``, and returns
       ``DEVIL_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``DEVIL_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("DEVIL_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    ...
    1. ``DEVIL_KANBAN_BOARD`` env var (set by the dispatcher on worker
    ...
    """
    env = os.environ.get("DEVIL_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``DEVIL_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("DEVIL_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``DEVIL_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("DEVIL_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``DEVIL_KANBAN_DB`` env → ``DEVIL_KANBAN_BOARD`` env →
    ...
    # See devil_state._WAL_INCOMPAT_MARKERS for detection logic.
    from devil_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from devil_cli.profiles import normalize_profile_name
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from devil_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _resolve_hermes_argv() -> list[str]:
    ...
    1. ``shutil.which("devil")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m devil_cli.main`` — fallback for setups where
    ...
    local (not imported from gateway) because ``devil_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    hermes_bin = shutil.which("devil")
    if hermes_bin:
        return [hermes_bin]
    # Fallback to the module form. ``devil_cli.main`` is the actual
...
def _default_spawn(
    ...
    *,
    ...
    ``DEVIL_KANBAN_DB`` / ``DEVIL_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject DEVIL_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `hermes -p <name>` the
    # _apply_profile_override() runs *before* devil_constants is imported.
    # If DEVIL_HOME is absent from the child's env, get_devil_home() falls
    # back to Path.home() / ".devil" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from devil_cli.profiles import resolve_profile_env
    try:
        env["DEVIL_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via DEVIL_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # DEVIL_HOME never had profiles created.
        pass
    if task.tenant:
        env["DEVIL_TENANT"] = task.tenant
    env["DEVIL_KANBAN_TASK"] = task.id
    env["DEVIL_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["DEVIL_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["DEVIL_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`hermes -p <name>`
    # rewrites DEVIL_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hermes_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["DEVIL_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["DEVIL_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["DEVIL_KANBAN_BOARD"] = resolved_board
    # DEVIL_PROFILE is the author the kanban_comment tool defaults to.
    # `hermes -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["DEVIL_PROFILE"] = profile_arg
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Devil Agent or activate its venv before running the kanban dispatcher."
        )
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled DEVIL_PROFILE values like "hermes-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    ``devil_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from devil_constants import get_default_hermes_root
