"""
Unified tool configuration for Devil Agent.

`hermes tools` and `hermes setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.devil/config.yaml under
...
from devil_cli.nous_subscription import (
...
from devil_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _DEVIL_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from devil_cli.plugins import discover_plugins, get_plugin_toolsets
...
from devil_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. DEVIL_CUA_DRIVER_VERSION is an
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "DEVIL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "DEVIL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "DEVIL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "DEVIL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "DEVIL_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_devil_home()}/hermes-agent && npm install")
                if result.stderr:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.devil/config.yaml")
        ...
        # to ~/.devil/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from devil_cli.auth import login_spotify_command
        except Exception as exc:
        ...
        try:
            from devil_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
...
def _get_platform_tools(
    ...
    *,
    ...
    # "hermes-cli" (which include all _DEVIL_CORE_TOOLS) cause disabled
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Devil Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from devil_constants import display_devil_home
    print(color(f"  Tool configuration saved to {display_devil_home()}/config.yaml", Colors.DIM))
