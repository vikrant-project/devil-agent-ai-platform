...
# display_devil_home is imported lazily at call sites to avoid ImportError
# when devil_constants is cached from a pre-update version during `hermes update`.
from devil_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from devil_cli.colors import Colors, color
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "devil_cli.main gateway",
        "devil_cli.main --profile",
        "devil_cli.main -p",
        "devil_cli/main.py gateway",
        "devil_cli/main.py --profile",
        "devil_cli/main.py -p",
        "hermes gateway",
        "gateway/run.py",
    ]
    current_home = str(get_devil_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"DEVIL_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. DEVIL_HOME
        ...
        # DEVIL_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "DEVIL_HOME=" in command and f"DEVIL_HOME={current_home}" not in command:
            return False
        return True
...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from devil_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "devil_cli.main"]
    if profile != "default":
...
def _sync_devil_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``DEVIL_HOME``.

    Under ``sudo``, ``DEVIL_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_devil_home` falls back to ``/root/.devil`` — the wrong
    profile. The unit file pins ``DEVIL_HOME`` for the actual gateway
    ...
    unit_home = env.get("DEVIL_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("DEVIL_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["DEVIL_HOME"] = unit_home
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from devil_constants import is_container
...
def _gateway_list() -> None:
    ...
    """
    try:
        from devil_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (DEVIL_HOME-scoped).
...
from devil_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    ``DEVIL_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("DEVIL_GATEWAY_DETACHED", "").strip().lower()
...
SERVICE_DESCRIPTION = "Devil Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current DEVIL_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (DEVIL_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from devil_constants import get_default_hermes_root
    home = get_devil_home().resolve()
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary DEVIL_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(devil_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when DEVIL_HOME is a named profile.

    For ``~/.devil/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        devil_home: Optional explicit DEVIL_HOME path. Defaults to the current
            ``get_devil_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from devil_constants import get_default_hermes_root
    home = Path(devil_home or str(get_devil_home())).resolve()
    default = get_default_hermes_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this DEVIL_HOME.

    Default ``~/.devil`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.devil/profiles/coder`` returns ``hermes-gateway-coder``.
    Any other DEVIL_HOME appends a short hash for uniqueness.
    """
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.devil`` → ``ai.devil.gateway.plist`` (backward compatible).
    Profile ``~/.devil/profiles/coder`` → ``ai.devil.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.devil.gateway-{suffix}" if suffix else "ai.devil.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.devil/hermes-agent  -> /home/alice/.devil/hermes-agent
...
def _devil_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current DEVIL_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_devil_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.devil                    → /home/alice/.devil
      /root/.devil/profiles/coder     → /home/alice/.devil/profiles/coder
      /opt/custom-hermes               → /opt/custom-hermes  (kept as-is)
    """
    current_hermes = get_devil_home().resolve()
    current_default = (Path.home() / ".devil").resolve()
    target_default = Path(target_home_dir) / ".devil"

    # Default ~/.devil → remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.devil → preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.devil) — keep as-is
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    devil_home = get_devil_home()
    hermes_node = devil_home / "node" / "bin"
    if hermes_node.is_dir():
        candidates.append(str(hermes_node))
    hermes_nm = devil_home / "node_modules" / ".bin"
    if hermes_nm.is_dir():
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        devil_home = _devil_home_for_target_user(home_dir)
        profile_arg = _profile_arg(devil_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m devil_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="DEVIL_HOME={devil_home}"
Restart=always
RestartSec=5
...
"""
    ...
    devil_home = str(get_devil_home().resolve())
    profile_arg = _profile_arg(devil_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only DEVIL_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp DEVIL_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../hermes_test`` path into
    # ``Environment="DEVIL_HOME=..."``. Writing that to the developer's
...
def systemd_stop(system: bool = False):
    ...
    _sync_devil_home_from_systemd_unit(system=system)
    try:
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.devil.gateway-{suffix}" if suffix else "ai.devil.gateway"
...
def launchd_install(force: bool = False):
    ...
    from devil_constants import display_devil_home as _dhh
...
def launchd_status(deep: bool = False):
    ...
    if deep:
        log_file = get_devil_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("DEVIL_ALLOW_ROOT_GATEWAY")):
        return
    ...
    print(
        ...
        "$DEVIL_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set DEVIL_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # Service-style launchers set DEVIL_GATEWAY_DETACHED=1; older wrappers
    ...
    # DEVIL_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("DEVIL_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from devil_constants import get_devil_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
def _all_platforms() -> list[dict]:
    ...
    # User-installed platform plugins under ~/.devil/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_devil_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_whatsapp():
    ...
    from devil_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from devil_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from devil_cli import gateway_windows
...
def _setup_weixin():
    ...
    print_info("  3. Hermes will store the returned account_id/token in ~/.devil/.env.")
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_devil_home())))
    except KeyboardInterrupt:
        print()
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``devil_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from devil_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.devil/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.devil/.env: {', '.join(required)}")
    else:
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            ...
            print("  nohup hermes gateway run > ~/.devil/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    print("  nohup hermes gateway run > ~/.devil/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s hermes 'hermes gateway run'         # persistent via tmux")
                    print("  nohup hermes gateway run > ~/.devil/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
