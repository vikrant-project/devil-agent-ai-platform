...
- ``devil_cli.providers``        -- canonical provider identity + overlays
- ``devil_cli.model_normalize``  -- per-provider name formatting
...
_DEVIL_MODEL_WARNING = (
    "Nous Research Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with Devil Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Hermes 3 / Hermes 4 chat families.
# The previous substring check (`"devil" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hermes-brain:qwen3-14b-ctx16k`` that just
# happen to carry "devil" in their tag but are fully tool-capable.
#
...
_NOUS_DEVIL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hermes[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_devil_non_agentic(model_name: str) -> bool:
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from devil_cli.model_switch import
...
def switch_model(
    ...
    # Mirrors the same logic in devil_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from devil_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from devil_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (DEVIL_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from devil_cli.auth import _load_auth_store
    ...
    from devil_cli.providers import DEVIL_OVERLAYS
    from devil_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hermes provider ID.
    # DEVIL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hermes IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in DEVIL_OVERLAYS.items():
    ...
    # in PROVIDER_TO_MODELS_DEV or DEVIL_OVERLAYS (keeps /model in sync
    # with `hermes model`).
    try:
        from devil_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # (see devil_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            # run_agent.py, devil_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`devil_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from devil_cli.models import fetch_openrouter_models
