"""
Configuration management for Devil Agent.

Config files are stored in ~/.devil/ for easy access:
- ~/.devil/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.devil/.env         - API keys and secrets
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.devil/config.yaml`` causes ``load_config()``
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "DEVIL_LANGFUSE_ENV",
    "DEVIL_LANGFUSE_RELEASE",
    "DEVIL_LANGFUSE_SAMPLE_RATE",
    "DEVIL_LANGFUSE_MAX_CHARS",
    "DEVIL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from devil_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("DEVIL_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_devil_home() / ".managed"
...
def is_managed() -> bool:
    ...
    Two signals: the DEVIL_MANAGED env var (set by the systemd service),
    or a .managed marker file in DEVIL_HOME (set by the NixOS activation
...
def format_managed_message(action: str = "modify this Hermes installation") -> str:
    ...
    raw = os.getenv("DEVIL_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hermes installation is managed by NixOS "
            f"(DEVIL_MANAGED={env_hint}).\n"
            "Edit services.devil-agent.settings in your configuration.nix and run:\n"
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from DEVIL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or DEVIL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("DEVIL_DEV") == "1":
        return None
    ...
    container_mode_file = get_devil_home() / ".container-mode"
    ...
    exec_user = info.get("exec_user", "devil")
...
# Re-export from devil_constants — canonical definition lives there.
from devil_constants import get_devil_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_devil_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_devil_home() / ".env"
...
def _secure_dir(path):
    ...
    The mode can be overridden via the DEVIL_HOME_MODE environment variable
    (e.g. DEVIL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse DEVIL_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("DEVIL_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    """
    # Explicit opt-out
    if os.environ.get("DEVIL_CONTAINER") or os.environ.get("DEVIL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set DEVIL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into DEVIL_HOME if the user doesn't have one yet."""
...
def ensure_devil_home():
    """Ensure ~/.devil directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_devil_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_devil_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"DEVIL_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "terminal": {
        ...
        #  "/home/user/.devil/cache/documents:/output"]
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.devil/checkpoints/`` size (MB).  When
    ...
    "display": {
        ...
        # users aren't surprised.  DEVIL_TUI_RESUME=<id> always wins.
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.devil/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.devil/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.devil/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${DEVIL_SKILL_DIR} and ${DEVIL_SESSION_ID} in SKILL.md
    ...
    "curator": {
        ...
        # skipped), snapshot ~/.devil/skills/ into
        # ~/.devil/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Devil Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (DEVIL_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.devil/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or DEVIL_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # ``devil_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # Also overridable via DEVIL_CRON_MAX_PARALLEL env var.
    ...
    # Logging — controls file logging to ~/.devil/logs/.
    ...
    # Session storage — controls automatic cleanup of ~/.devil/state.db.
    ...
    "updates": {
        # Run a full ``hermes backup``-style zip of DEVIL_HOME before every
        # ``hermes update``.  Backups land in ``<DEVIL_HOME>/backups/`` and
        # can be restored with ``hermes import <path>``.  Off by default —
        # on large DEVIL_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<DEVIL_HOME>/lsp/bin/`` on first use.
...
OPTIONAL_ENV_VARS = {
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. DEVIL_BOT)",
    ...
    # DEVIL_TOOL_PROGRESS and DEVIL_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.devil.config`` entries, then checks
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``devil_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("DEVIL_HOME", "~/.devil")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("DEVIL_TOOL_PROGRESS")
            old_mode = get_env_value("DEVIL_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from DEVIL_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from DEVIL_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("DEVIL_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from DEVIL_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$DEVIL_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_devil_home() / "plugins"
    ...
    #   3. Creates `~/.devil/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_devil_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_devil_home() / "logs" / "curator"
    ...
    # metadata.devil.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _devil_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.devil/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.devil/config.yaml.
    ...
    (which change ``DEVIL_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_devil_home()
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.devil/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.devil/.env.
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.devil/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.devil/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.devil/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.devil/.env into os.environ. Returns count of vars updated.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.devil/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Devil Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
