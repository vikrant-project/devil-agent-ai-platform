...
def _translate_one_server(
    ...
    for key in hermes_cfg:
        ...
        elif key not in _KNOWN_DEVIL_KEYS:
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (DEVIL_HOME, PYTHONPATH) could
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a DEVIL_HOME pointing
    ...
    ``~/.devil`` symlink that happens to live under ``/private/var/folders``
...
def _build_hermes_tools_mcp_entry() -> dict:
    ...
    DEVIL_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # DEVIL_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_devil_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever DEVIL_HOME its launcher sets
    ...
    # — that would override the launcher's DEVIL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("DEVIL_HOME", tmp_path) would
    ...
    devil_home = os.environ.get("DEVIL_HOME") or ""
    if devil_home and _looks_like_test_tempdir(devil_home):
        devil_home = ""
    if devil_home:
        env["DEVIL_HOME"] = devil_home
    ...
    env["DEVIL_QUIET"] = "1"
    env["DEVIL_REDACT_SECRETS"] = env.get("DEVIL_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.devil_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    ...
    *,
    ...
    Args:
        hermes_config: full ~/.devil/config.yaml dict
