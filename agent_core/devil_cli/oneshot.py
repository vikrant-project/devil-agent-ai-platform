...
Approvals = auto-bypassed (DEVIL_YOLO_MODE=1 is set for the call).
...
Env var fallbacks (used when the corresponding arg is not passed):
    - DEVIL_INFERENCE_MODEL
    - DEVIL_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to DEVIL_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            DEVIL_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = os.getenv("DEVIL_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "hermes -z: --provider requires --model (or DEVIL_INFERENCE_MODEL). "
    ...
    os.environ["DEVIL_ACCEPT_HOOKS"] = "1"
...
def _run_agent(
    ...
    from devil_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("DEVIL_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from devil_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("DEVIL_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                DEVIL_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via DEVIL_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via DEVIL_YOLO_MODE=1
