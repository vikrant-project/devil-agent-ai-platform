...
Uses a file-based lock (~/.devil/cron/.tick.lock) so only one tick
...
# the module) fail with ModuleNotFoundError for devil_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from devil_constants import get_devil_home
from devil_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    if per_job:
        return per_job
    try:
        from devil_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_devil_home: Path | None = None


def _get_devil_home() -> Path:
    """Resolve Hermes home dynamically while preserving test monkeypatch hooks."""
    return _devil_home or get_devil_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    devil_home = _get_devil_home()
    lock_dir = devil_home / "cron"
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within DEVIL_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against DEVIL_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    scripts_dir = _get_devil_home() / "scripts"
    ...
    # escape — scripts MUST reside within DEVIL_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_devil_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_devil_home() / ".env"), override=True, encoding="latin-1")
        ...
        try:
            import yaml
            _cfg_path = str(_get_devil_home() / "config.yaml")
        ...
        agent = AIAgent(
            ...
            # DEVIL_HOME. When a workdir is configured, also inject project
