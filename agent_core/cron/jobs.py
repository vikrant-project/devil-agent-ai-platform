...
Jobs are stored in ~/.devil/cron/jobs.json
Output is saved to ~/.devil/cron/output/{job_id}/{timestamp}.md
"""
...
HERMES_DIR = get_devil_home().resolve()
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.devil/scripts/; ``.sh`` / ``.bash`` files run via bash,
