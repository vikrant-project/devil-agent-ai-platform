#!/usr/bin/env python3
"""
Devil Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Devil Agent, inspired by Claude Code.
...
# IMPORTANT: devil_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See devil_bootstrap.py for full rationale.
try:
    import devil_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when devil_bootstrap isn't registered in the venv
...
os.environ["DEVIL_QUIET"] = "1"  # Our own modules
...
try:
    from devil_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from devil_cli.banner import _format_context_length, format_banner_version_label
...
from devil_cli.browser_connect import (
...
_devil_home = get_devil_home()
_project_env = Path(__file__).parent / '.env'
load_devil_dotenv(devil_home=_devil_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.devil/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from devil_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.devil/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If DEVIL_IGNORE_USER_CONFIG=1 is set (via ``devil chat --ignore-user-config``),
    the user config at ``~/.devil/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({DEVIL_HOME}/config.yaml)
    user_config_path = _devil_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("DEVIL_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Devil, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Devil - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
    ...
    if config_path.exists():
        try:
            ...
            # location is model.provider (written by `devil model`).
    ...
    from devil_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (devil_cli/config.py) and all
    ...
    # UNLESS we're inside a gateway process (detected by _DEVIL_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_DEVIL_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["DEVIL_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.devil/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from devil_logging import setup_logging
...
try:
    from devil_cli.config import print_config_warnings
...
try:
    from devil_cli.skin_engine import init_skin_from_config
...
from devil_cli.banner import build_welcome_banner
from devil_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from devil_cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from devil_cli.plugins import invoke_hook as _invoke_hook
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run devil -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"devil-{short_id}"
    branch_name = f"devil/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`devil_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from devil_cli.config import load_config as _load_full_config
        from devil_constants import get_devil_home as _get_devil_home
        _devil_home_maint = _get_devil_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_devil_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`devil_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("devil-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    ...
    orphaned = [
        ...
        and (b.startswith("devil/devil-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. DEVIL_LIGHT / DEVIL_TUI_LIGHT env (true/false) — explicit override
#   2. DEVIL_TUI_THEME=light|dark — explicit theme
#   3. DEVIL_TUI_BACKGROUND=#RRGGBB — explicit bg hint
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("DEVIL_LIGHT", "DEVIL_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("DEVIL_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("DEVIL_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from devil_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from devil_cli.skin_engine import get_active_skin
...
from devil_constants import is_termux as _is_termux_environment
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "⚕ DEVIL AGENT - AI Agent Framework"
        tiny_line = "⚕ DEVIL AGENT"
    else:
        agent_name = _skin.get_branding("agent_name", "Devil Agent") if _skin else "Devil Agent"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from devil_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.devil/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.devil/config.yaml on first use)
...
class HermesCLI:
    """
    Interactive CLI for the Devil Agent.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from devil_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("DEVIL_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("DEVIL_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("DEVIL_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `devil chat --ignore-rules` in devil_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("DEVIL_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("DEVIL_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Hermes processes for this DEVIL_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.devil/checkpoints/.  Opt-in via
        ...
        self._history_file = _devil_home / ".devil_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from devil_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("DEVIL_YOLO_MODE"))
            if width < 52:
            ...
            return f"⚕ {self.model if getattr(self, 'model', None) else 'Devil'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from devil_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from devil_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from devil_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from devil_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚕ Devil")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from devil_cli.runtime_provider import (
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: devil setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: devil setup")
            return False
        ...
        # `devil chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `devil auth add openai-codex`
        # without `devil model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from devil_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from devil_cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (devil sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``devil doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``devil doctor`` so the banner stays
        small.
        """
        try:
            from devil_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Devil needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        from devil_cli.model_switch import is_nous_devil_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_devil_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Devil 3 & 4 models are NOT agentic and are not "
                "designed for use with Devil Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Devil: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Devil: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.devil/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from devil_cli.clipboard import save_clipboard_image

        img_dir = get_devil_home() / "images"
    ...
    def _handle_snapshot_command(self, command: str):
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_devil_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from devil_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run devil chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'devil setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from devil_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from devil_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from devil_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Devil!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in devil_cli/colors.py
        ...
        from devil_cli.tools_config import _get_platform_tools
    ...
    def _handle_profile_command(self):
        ...
        from devil_constants import display_devil_home
        from devil_cli.profiles import get_active_profile_name

        display = display_devil_home()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from devil_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("DEVIL_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from devil_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `devil gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `devil sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from devil_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `devil sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.devil/sessions/saved/.
        ...
        saved_dir = get_devil_home() / "sessions" / "saved"
        try:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from devil_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from devil_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `devil model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from devil_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from devil_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from devil_cli.model_switch import switch_model, parse_model_flags
        from devil_cli.providers import get_label
        ...
        # clobber disk config.
        from devil_cli.inventory import build_models_payload, load_picker_context
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        """
        from devil_cli import codex_runtime_switch as crs
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from devil_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to devil_cli.curator so the CLI and the `hermes curator`
        ...
        try:
            from devil_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from devil_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to devil_cli.skills_hub."""
        from devil_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_devil_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_devil_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in devil_cli/commands.py instead of touching every dispatch site.
        from devil_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from devil_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from devil_cli.skin_engine import get_active_skin
            ...
            from devil_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from devil_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_devil_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from devil_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from devil_cli.goals import GoalManager
            from devil_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Devil keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from devil_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_devil_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from devil_cli.config import load_config
        from devil_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("DEVIL_YOLO_MODE"))
        if current:
            os.environ.pop("DEVIL_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Devil is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Devil is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Devil is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from devil_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from devil_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by devil_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from devil_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.devil/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_devil_home / "config.yaml", TOOL_PROGRESS_FLAG)
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _devil_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            try:
                from devil_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from devil_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from devil_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Devil Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Devil Agent! Type your message or /help for commands."
        ...
        try:
            _redact_raw = os.getenv("DEVIL_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(DEVIL_REDACT_SECRETS={_redact_raw}). "
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from devil_cli.config import get_config_path as _get_cfg_path_resid
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_devil_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from devil_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Devil Agent")
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Devil needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Devil needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Devil needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``DEVIL_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("DEVIL_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: devil setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Devil Agent CLI - Interactive AI Assistant
    ...
    os.environ["DEVIL_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Devil Gateway (messaging platforms)...")
    ...
    # so main unwinds normally.  DEVIL_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Devil Agent" box, no tool-gen
            ...
            # Single-query mode (`devil chat -q "…"`): skip the welcome
