...
Resolution order:
  1. ``DEVIL_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.devil/config.yaml``
...
from devil_constants import get_config_path
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("DEVIL_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
