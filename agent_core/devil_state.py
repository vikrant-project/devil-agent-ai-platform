#!/usr/bin/env python3
"""
SQLite State Store for Devil Agent.
...
DEFAULT_DB_PATH = get_devil_home() / "state.db"
...
# devil_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``devil_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see devil_cli/kanban_db.py) would
...
class SessionDB:
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``devil_state._set_last_init_error(None)`` explicitly.
