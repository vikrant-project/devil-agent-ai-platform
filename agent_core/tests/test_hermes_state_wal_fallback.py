...
Without this fallback, users on NFS-mounted ``DEVIL_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    devil_state._set_last_init_error(None)
    yield
    devil_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    devil_state._wal_fallback_warned_paths.clear()
    yield
    devil_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see devil_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        devil_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("devil_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        devil_state._set_last_init_error("OperationalError: generic SQLite error")
