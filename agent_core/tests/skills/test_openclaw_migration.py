...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    #                      tell the user to *review* ~/.devil/config.yaml;
...
def test_rebrand_text_replaces_openclaw_variants():
    ...
    # real filesystem path ``~/.devil`` (Hermes home) when rebranding
...
def test_rebrand_text_preserves_filesystem_path_casing():
    ...
    rewrite to lowercase ``.devil`` (the real Hermes home), not the broken
    ``.Hermes``.
    ...
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.devil/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .devil directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.devil'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "hermes config path: ~/.devil/"
