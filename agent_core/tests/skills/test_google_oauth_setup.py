...
class TestHermesConstantsFallback:
    """Tests for _devil_home.py fallback when devil_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_devil_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _devil_home.py with devil_constants blocked."""
        monkeypatch.setitem(sys.modules, "devil_constants", None)
        spec = importlib.util.spec_from_file_location("_devil_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_devil_home_env_var(self, monkeypatch, tmp_path):
        """When devil_constants is missing, DEVIL_HOME comes from env var."""
        monkeypatch.setenv("DEVIL_HOME", str(tmp_path / "custom-hermes"))
        module = self._load_helper(monkeypatch)
        assert module.get_devil_home() == tmp_path / "custom-hermes"

    def test_fallback_defaults_to_dot_hermes(self, monkeypatch):
        """When devil_constants is missing and DEVIL_HOME unset, default to ~/.devil."""
        monkeypatch.delenv("DEVIL_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_devil_home() == Path.home() / ".devil"

    def test_fallback_ignores_empty_devil_home(self, monkeypatch):
        """Empty/whitespace DEVIL_HOME is treated as unset."""
        monkeypatch.setenv("DEVIL_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_devil_home() == Path.home() / ".devil"

    def test_fallback_display_devil_home_shortens_path(self, monkeypatch):
        """Fallback display_devil_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("DEVIL_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_devil_home() == "~/.devil"

    def test_fallback_display_devil_home_profile_path(self, monkeypatch):
        """Fallback display_devil_home() handles profile paths under ~/."""
        monkeypatch.setenv("DEVIL_HOME", str(Path.home() / ".devil/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_devil_home() == "~/.devil/profiles/coder"

    def test_fallback_display_devil_home_custom_path(self, monkeypatch):
        """Fallback display_devil_home() returns full path for non-home locations."""
        monkeypatch.setenv("DEVIL_HOME", "/opt/hermes-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_devil_home() == "/opt/hermes-custom"

    def test_delegates_to_devil_constants_when_available(self):
        """When devil_constants IS importable, _devil_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_devil_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import devil_constants
        assert module.get_devil_home is devil_constants.get_devil_home
        assert module.display_devil_home is devil_constants.display_devil_home
