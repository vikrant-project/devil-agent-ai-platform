...
def server():
    with patch.dict("sys.modules", {
        "devil_constants": MagicMock(get_devil_home=MagicMock(return_value="/tmp/hermes_test")),
        "devil_cli.env_loader": MagicMock(),
        "devil_cli.banner": MagicMock(),
        "devil_state": MagicMock(),
    }):
        import importlib
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "devil_cli.plugins.get_plugin_command_handler",
