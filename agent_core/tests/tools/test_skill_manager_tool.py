...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.devil/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("devil_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("devil_cli.config.load_config",
    ...
    def test_guard_flag_handles_config_error(self):
        ...
        with patch("devil_cli.config.load_config", side_effect=RuntimeError("boom")):
            assert _guard_agent_created_enabled() is False
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.devil/skills/ as a
    workaround.
    """
