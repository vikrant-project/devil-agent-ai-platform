...
class TestWriteDenyExactPaths:
    ...
    def test_hermes_env(self):
        # ``.env`` under the active DEVIL_HOME (profile-aware, not just
        # ``~/.devil``) must be write-denied. The hermetic test conftest
        # points DEVIL_HOME at a tempdir — resolve via get_devil_home()
        # to match the denylist.
        from devil_constants import get_devil_home
        path = str(get_devil_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_hermes_config_not_env(self):
        path = os.path.join(str(Path.home()), ".devil", "config.yaml")
