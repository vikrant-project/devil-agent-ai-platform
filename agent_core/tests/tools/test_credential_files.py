...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        devil_home.mkdir()
        (devil_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"DEVIL_HOME": str(devil_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(devil_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.devil/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.devil/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        devil_home = tmp_path / ".devil"
        (devil_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"DEVIL_HOME": str(devil_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.devil")

        assert mounts[0]["container_path"] == "/home/user/.devil/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.devil/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.devil/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape DEVIL_HOME."""
        monkeypatch.setenv("DEVIL_HOME", str(tmp_path / ".devil"))
        (tmp_path / ".devil").mkdir()

        # Create a sensitive file one level above devil_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside devil_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside DEVIL_HOME must still be registered."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        (devil_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_devil_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of DEVIL_HOME must be allowed."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        subdir = devil_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside DEVIL_HOME pointing outside must be rejected."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))

        # Create a sensitive file outside devil_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside devil_home pointing outside
        symlink = devil_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes DEVIL_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, devil_home: Path, cred_files: list):
        import yaml
        config_path = devil_home / "config.yaml"
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape DEVIL_HOME."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        ...
        sensitive.write_text("{}")
        self._write_config(devil_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(devil_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside DEVIL_HOME via config must still mount."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))

        (devil_home / "oauth.json").write_text("{}")
        self._write_config(devil_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / "cache" / "documents").mkdir(parents=True)
        (devil_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.devil/cache/documents" in paths
        assert "/root/.devil/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.devil/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        (devil_home / "document_cache").mkdir()
        (devil_home / "image_cache").mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(devil_home / "document_cache") in host_paths
        assert str(devil_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.devil/cache/documents" in container_paths
        assert "/root/.devil/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        doc_dir = devil_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        ss_dir = devil_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.devil/cache/screenshots/session_abc/screen1.png"
