...
class TestFileSync:
    def test_initial_sync_uploads_managed_files_under_remote_home(
        ...
        monkeypatch.setattr(
            ...
            lambda: [
                {
                    "host_path": str(src),
                    "container_path": "/root/.devil/credentials/token.txt",
                }
            ],
        )
        monkeypatch.setattr("tools.credential_files.iter_skills_files", lambda **kwargs: [])
        ...
        assert uploaded == [
            {
                "path": "/home/vercel/.devil/credentials/token.txt",
    ...
    def test_cleanup_syncs_back_snapshots_closes_and_is_idempotent(
        ...
        vercel_sdk.current.download_file_content = _tar_bytes(
            {
                "home/vercel/.devil/credentials/token.txt": b"remote-token",
                "home/vercel/.devil/credentials/new.txt": b"new-remote",
                "home/vercel/.devil/unmapped/skip.txt": b"skip",
            }
        )
