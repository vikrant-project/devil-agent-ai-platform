...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in DEVIL_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
# ---------------------------------------------------------------------------
# DEVIL_HOME isolation
# ---------------------------------------------------------------------------

class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_devil_home(self):
        """_hermes_bin_dir must use DEVIL_HOME, not hardcoded ~/.devil."""
        from tools.tirith_security import _hermes_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"DEVIL_HOME": tmpdir}):
            result = _hermes_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_devil_home(self):
        """_failure_marker_path must use DEVIL_HOME, not hardcoded ~/.devil."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"DEVIL_HOME": "/custom/hermes"}):
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets DEVIL_HOME; verify it's active."""
        devil_home = os.getenv("DEVIL_HOME")
        assert devil_home is not None, "DEVIL_HOME should be set by conftest"
        assert "hermes_test" in devil_home, "Should point to test temp dir"

    def test_get_devil_home_fallback(self):
        """Without DEVIL_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_devil_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove DEVIL_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".devil")
            result = _get_devil_home()
