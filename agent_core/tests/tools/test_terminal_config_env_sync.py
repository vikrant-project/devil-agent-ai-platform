...
at startup, by THREE separate code paths:
  ...
  2. gateway/run.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
  3. devil_cli/config.py:save_config_value
...
dicts) keeps the test independent of the user's ~/.devil/config.yaml and
mirrors the pattern used in tests/devil_cli/test_config_drift.py.
"""
...
def _save_config_env_sync_keys() -> set[str]:
    ...
    from devil_cli import config as hc_config
...
def test_save_config_set_supports_critical_bridged_keys():
    ...
    assert not missing, (
        ...
        f"in devil_cli/config.py:set_config_value."
    )
