...
def _restore_tool_modules():
    original_devil_home = os.environ.get("DEVIL_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_devil_home is None:
            os.environ.pop("DEVIL_HOME", None)
        else:
            os.environ["DEVIL_HOME"] = original_devil_home
        _reset_modules(("tools", "devil_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    devil_cli = types.ModuleType("devil_cli")
    devil_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["devil_cli"] = devil_cli
    devil_home = tmp_path / "hermes-home"
    os.environ["DEVIL_HOME"] = str(devil_home)
    sys.modules["devil_cli.config"] = types.SimpleNamespace(
        get_devil_home=lambda: devil_home,
    )
    ...
    return {
        "snapshot_store": devil_home / "modal_snapshots.json",
