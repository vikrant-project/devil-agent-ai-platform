...
def _isolate_devil_home(tmp_path, monkeypatch):
    """Redirect DEVIL_HOME to a temp directory."""
    monkeypatch.setenv("DEVIL_HOME", str(tmp_path))
    try:
        import devil_constants
        monkeypatch.setattr(devil_constants, "get_devil_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking devil_state schema."""
