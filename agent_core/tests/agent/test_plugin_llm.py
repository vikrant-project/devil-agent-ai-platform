...
""",
        ...
        from devil_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        from devil_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from devil_cli.plugins import PluginContext, PluginManifest, PluginManager
