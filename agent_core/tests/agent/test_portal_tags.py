...
def test_devil_client_tag_includes_current_version():
    """The client tag must reflect devil_cli.__version__ verbatim."""
    from devil_cli import __version__
    from agent.portal_tags import devil_client_tag

    assert devil_client_tag() == f"client=hermes-client-v{__version__}"


def test_devil_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import devil_client_tag

    tag = devil_client_tag()
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import devil_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=hermes-agent" in tags
    assert devil_client_tag() in tags
    assert len(tags) == 2
