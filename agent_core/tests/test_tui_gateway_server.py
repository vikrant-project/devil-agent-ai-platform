...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    import devil_cli.config as config_mod
...
def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.models.detect_static_provider_for_model",
...
def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "devil_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "devil_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_statusbar_survives_non_dict_display(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_devil_home", tmp_path)
...
def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("devil_cli.main._has_any_provider_configured", lambda: False)
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    monkeypatch.setattr("devil_cli.config.save_config", lambda cfg: saved.update(cfg))
...
def test_config_set_model_syncs_inference_provider_env(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_syncs_tui_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.model_switch.switch_model", fake_switch_model)
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("devil_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "devil_cli.commands.SlashCommandCompleter",
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("devil_state")
    ...
    monkeypatch.setitem(sys.modules, "devil_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_devil_home
    # fixture pins DEVIL_HOME to a temp dir; the handler should append
    # /sessions to it.
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    with patch(
        "devil_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("devil_cli.models.provider_model_ids") as live_fetch:
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"devil_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "devil_cli.browser_connect.try_launch_chrome_debug", return_value=False
            ),
            patch(
                "devil_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "devil_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_devil_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.devil/.env
    ...
    with patch.dict(sys.modules, {"devil_cli.config": fake}):
...
def _setup_make_agent_mocks(monkeypatch, cfg):
    ...
    monkeypatch.setattr(
        "devil_cli.runtime_provider.resolve_runtime_provider",
