"""Tests for devil_constants module."""
...
class TestGetDefaultHermesRoot:
    ...
    def test_no_devil_home_returns_native(self, tmp_path, monkeypatch):
        """When DEVIL_HOME is not set, returns ~/.devil."""
        monkeypatch.delenv("DEVIL_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_hermes_root() == tmp_path / ".devil"

    def test_devil_home_is_native(self, tmp_path, monkeypatch):
        """When DEVIL_HOME = ~/.devil, returns ~/.devil."""
        native = tmp_path / ".devil"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(native))
        assert get_default_hermes_root() == native

    def test_devil_home_is_profile(self, tmp_path, monkeypatch):
        """When DEVIL_HOME is a profile under ~/.devil, returns ~/.devil."""
        native = tmp_path / ".devil"
    ...
    def test_devil_home_is_docker(self, tmp_path, monkeypatch):
        """When DEVIL_HOME points outside ~/.devil (Docker), returns DEVIL_HOME."""
    ...
    def test_devil_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any DEVIL_HOME outside ~/.devil is treated as the root."""
    ...
    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (DEVIL_HOME=<root>/profiles/<name>),
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(devil_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(devil_constants, "_container_detected", True)
        assert is_container() is True
