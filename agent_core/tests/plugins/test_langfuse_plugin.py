...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        # Isolated DEVIL_HOME so we don't read the developer's config.yaml.
        home = tmp_path / ".devil"
        home.mkdir()
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_does_not_import_hermes_config(self, monkeypatch):
        ...
        # Drop any cached import of devil_cli.config.
        sys.modules.pop("devil_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "devil_cli.config" not in sys.modules, (
            "langfuse plugin imported devil_cli.config — regression toward "
