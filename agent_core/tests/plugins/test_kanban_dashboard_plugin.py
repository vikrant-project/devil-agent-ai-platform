...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "devil_cli.web_server", stub)
    monkeypatch.setattr(devil_cli, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.kanban._check_dispatcher_presence",
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see devil_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
