...
def test_registry_defaults_to_devil_home(tmp_path, monkeypatch):
    from plugins.google_meet.node.registry import NodeRegistry

    # _isolate_home already set DEVIL_HOME to tmp_path/.devil; the
    # registry default path must live inside that tree.
    r = NodeRegistry()
    r.add("x", "ws://x", "t")
    expected = Path(tmp_path) / ".devil" / "workspace" / "meetings" / "nodes.json"
