...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate DEVIL_HOME for each test.

    The global hermetic fixture already redirects DEVIL_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    DEVIL_HOME here for clarity.
    """
...
class TestIsSafePath:
    def test_accepts_path_under_devil_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_devil_home(self, _isolate_env):
        dg = _load_lib()
...
class TestBundledDiscovery:
    def _write_enabled_config(self, devil_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from devil_cli import plugins as pmod
