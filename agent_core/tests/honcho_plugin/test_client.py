...
from devil_cli.profiles import _get_default_devil_home
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.devil/honcho.json
        monkeypatch.setenv("DEVIL_HOME", str(tmp_path / "isolated"))
...
class TestResolveConfigPath:
    def test_prefers_devil_home_when_exists(self, tmp_path):
        devil_home = tmp_path / "hermes"
        devil_home.mkdir()
        local_cfg = devil_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: DEVIL_HOME points at ~/.devil/profiles/<name>, so
        # _get_default_devil_home() must resolve back to ~/.devil — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".devil"
        ...
        assert _get_default_devil_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_devil_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("DEVIL_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".devil".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".devil"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    ...
    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("devil_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "hermes.coder"

    def test_default_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("devil_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "hermes"

    def test_custom_profile_returns_hermes(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            with patch("devil_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "hermes"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("HERMES_HONCHO_HOST", None)
            # Temporarily remove devil_cli.profiles to simulate import failure
            saved = sys.modules.get("devil_cli.profiles")
            sys.modules["devil_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "hermes"
            finally:
                if saved is not None:
                    sys.modules["devil_cli.profiles"] = saved
                else:
                    sys.modules.pop("devil_cli.profiles", None)
...
class TestGetHonchoClient:
    ...
    def test_hermes_config_timeout_override_used_when_config_timeout_missing(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("devil_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_defaults_to_30s_when_no_timeout_configured(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("devil_cli.config.load_config", return_value={}):
            client = get_honcho_client(cfg)
    ...
    def test_hermes_request_timeout_alias_used(self):
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("devil_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)
