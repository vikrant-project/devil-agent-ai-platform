...
2. **Isolated DEVIL_HOME.** DEVIL_HOME points to a per-test tempdir so
   code reading ``~/.devil/*`` via ``get_devil_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".devil"`` instead of the canonical
   ``get_devil_home()`` is a bug to fix at the callsite.)
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.devil/kanban.db instead of the per-test DEVIL_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and DEVIL_HOME to per-test tempdirs so code that
    reads ``~/.devil/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 3. Redirect DEVIL_HOME to a per-test tempdir. Code that reads
    #    ``~/.devil/*`` via ``get_devil_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.devil/*`` via
    #    ``Path.home() / ".devil"`` instead of ``get_devil_home()``
    #    is a bug to fix at the callsite.
    fake_devil_home = tmp_path / "hermes_test"
    fake_devil_home.mkdir()
    (fake_devil_home / "sessions").mkdir()
    (fake_devil_home / "cron").mkdir()
    (fake_devil_home / "memories").mkdir()
    (fake_devil_home / "skills").mkdir()
    monkeypatch.setenv("DEVIL_HOME", str(fake_devil_home))
    ...
    #    ~/.devil/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import devil_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_devil_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "devil_cli"):
...
# correlated with a ``tests/devil_cli/`` pytest run starting up.
#
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                ...
                # whose cmdline contains "python -m devil_cli.main".
                if (
                    "hermes" in low
                    or "gateway" in low
