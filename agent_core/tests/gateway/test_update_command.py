...
class TestHandleUpdateCommand:
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _devil_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._devil_home", tmp_path), \
    ...
    async def test_no_hermes_binary(self, tmp_path):
        """Returns error when hermes is not on PATH and devil_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m devil_cli.main when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m devil_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "devil_cli.main" in joined or "bash" in call_args[0]
    ...
    async def test_resolve_hermes_bin_fallback(self):
        ...
        assert result == [sys.executable, "-m", "devil_cli.main"]
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        assert not (devil_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((devil_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (devil_home / ".update_pending.json").exists()
        assert not (devil_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (devil_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = devil_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (devil_home / ".update_pending.json").write_text(json.dumps(pending))
        (devil_home / ".update_output.txt").write_text("x" * 5000)
        (devil_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (devil_home / ".update_pending.json").write_text(json.dumps(pending))
        (devil_home / ".update_output.txt").write_text("Traceback: boom")
