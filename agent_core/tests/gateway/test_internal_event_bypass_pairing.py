...
def _build_runner(monkeypatch, tmp_path) -> GatewayRunner:
    ...
    monkeypatch.setattr(gateway_run, "_devil_home", tmp_path)
...
async def test_non_internal_event_without_user_triggers_pairing(monkeypatch, tmp_path):
    ...
    # import time from whichever DEVIL_HOME was set then. Per-test
    # DEVIL_HOME redirection in conftest doesn't retroactively move it.
    ...
    # module-level dotenv in gateway/run.py from the real ~/.devil/.env).
