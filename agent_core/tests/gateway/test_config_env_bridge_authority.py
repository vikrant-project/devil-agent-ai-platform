...
def _run_gateway_import(devil_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
...
@pytest.fixture
def devil_home(tmp_path: Path) -> Path:
    home = tmp_path / ".devil"
    home.mkdir()
    return home


def test_config_max_turns_wins_over_stale_env(devil_home: Path) -> None:
    """Regression: config.yaml:agent.max_turns=500 must beat .env=60."""
    _write_config(devil_home, agent_cfg={"max_turns": 500})
    _write_env(devil_home, {"HERMES_MAX_ITERATIONS": "60"})

    env = _run_gateway_import(devil_home, initial_env={})
...
def test_config_gateway_timeout_wins_over_stale_env(devil_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(devil_home, agent_cfg={
        "gateway_timeout": 1800,
        "gateway_timeout_warning": 900,
    })
    _write_env(devil_home, {
...
def test_config_display_busy_input_mode_wins_over_stale_env(devil_home: Path) -> None:
    _write_config(devil_home, display_cfg={"busy_input_mode": "interrupt"})
    _write_env(devil_home, {"HERMES_GATEWAY_BUSY_INPUT_MODE": "queue"})
...
def test_config_timezone_wins_over_stale_env(devil_home: Path) -> None:
    _write_config(devil_home, timezone="America/Los_Angeles")
    _write_env(devil_home, {"HERMES_TIMEZONE": "UTC"})
...
def test_env_value_survives_when_config_omits_key(devil_home: Path) -> None:
    ...
    """
    _write_config(devil_home, agent_cfg={})  # no max_turns
    _write_env(devil_home, {"HERMES_MAX_ITERATIONS": "123"})
