"""Tests for _setup_feishu() in devil_cli/gateway.py.
...
def _run_setup_feishu(
    *,
    ...
    with patch("devil_cli.gateway.save_env_value", side_effect=mock_save), \
         patch("devil_cli.gateway.get_env_value", side_effect=mock_get), \
         patch("devil_cli.gateway.prompt_yes_no", side_effect=prompt_yes_no_responses), \
         patch("devil_cli.gateway.prompt_choice", side_effect=prompt_choice_responses), \
         patch("devil_cli.gateway.prompt", side_effect=prompt_responses), \
         patch("devil_cli.gateway.print_info"), \
         patch("devil_cli.gateway.print_success"), \
         patch("devil_cli.gateway.print_warning"), \
         patch("devil_cli.gateway.print_error"), \
         patch("devil_cli.gateway.color", side_effect=lambda t, c: t), \
         patch("gateway.platforms.feishu.qr_register", return_value=qr_result):

        from devil_cli.gateway import _setup_feishu
        _setup_feishu()
