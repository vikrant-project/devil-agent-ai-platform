...
def adapter(tmp_path):
    ...
    ~/.devil/google_chat_thread_counts.json.
    """
    ...
    # Replace the production store (which would write to ~/.devil/...)
...
class TestGoogleChatInteractiveSetup:
    def test_interactive_setup_uses_shared_cli_prompt_helpers(self, monkeypatch):
        ...
        monkeypatch.setattr("devil_cli.config.get_env_value", fake_get_env_value)
        monkeypatch.setattr("devil_cli.config.save_env_value", fake_save_env_value)
        monkeypatch.setattr("devil_cli.cli_output.prompt", fake_prompt)
        monkeypatch.setattr(
            "devil_cli.cli_output.prompt_yes_no", lambda *_a, **_kw: True
        )
        monkeypatch.setattr(
            "devil_cli.cli_output.print_info", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "devil_cli.cli_output.print_success", lambda *_a, **_kw: None
        )
        monkeypatch.setattr(
            "devil_cli.cli_output.print_warning", lambda *_a, **_kw: None
        )
