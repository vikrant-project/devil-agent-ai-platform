"""Tests for devil_cli.tools_config platform tool persistence."""
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("devil_cli.tools_config.save_config"):
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("devil_cli.tools_config.managed_nous_tools_enabled", lambda: True)
    config = {"model": {"provider": "nous"}}

    monkeypatch.setattr(
        "devil_cli.nous_subscription.get_nous_auth_status",
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("devil_cli.tools_config.managed_nous_tools_enabled", lambda: False)
    config = {"model": {"provider": "nous"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("devil_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "devil_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config: configured.append(ts_key),
    )
    monkeypatch.setattr("devil_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "devil_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "devil_cli.nous_subscription.get_nous_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "devil_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from devil_cli.tools_config import PLATFORMS
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from devil_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from devil_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from devil_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from devil_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from devil_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("devil_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("devil_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("devil_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    with mock_patch("devil_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from devil_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from devil_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    """
    from devil_cli.tools_config import _get_effective_configurable_toolsets
