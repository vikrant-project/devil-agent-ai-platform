...
- Profile-scoped reset (uses DEVIL_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake DEVIL_HOME with memory files."""
    devil_home = tmp_path / ".devil"
    memories = devil_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.devil/hermes-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_all_with_yes_flag(self, memory_env):
        ...
        devil_home, memories = memory_env
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        (devil_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's DEVIL_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))

        # The memories dir won't exist; get_devil_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
