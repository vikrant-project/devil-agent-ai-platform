...
# ---------------------------------------------------------------------------
# Shared fixture — isolate DEVIL_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".devil" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from devil_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from devil_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    def test_list_empty(self, isolated_home, capsys):
        ...
        from devil_cli.fallback_cmd import cmd_fallback_list
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("devil_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("devil_cli.main._require_tty"):
            from devil_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from devil_cli.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        with patch("devil_cli.setup._curses_prompt_choice", return_value=1):
            from devil_cli.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from devil_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "devil_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
