...
``devil_cli.main`` skips eager plugin discovery at argparse-setup time
...
def _live_subcommand_names() -> set[str]:
    ...
    built-in-only set.
    """
    from devil_cli import main as _main
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    assert not missing_from_declaration, (
        ...
        f"devil_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
