...
* ``--ignore-user-config`` → skip ``~/.devil/config.yaml`` in ``load_cli_config()``
...
class TestIgnoreUserConfigEnvGate:
    ...
    When the env var is set, user config at ``<devil_home>/config.yaml`` is
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._devil_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_devil_home", tmp_path)
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``devil_cli/main.py`` must set
