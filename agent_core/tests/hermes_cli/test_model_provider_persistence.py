...
def config_home(tmp_path, monkeypatch):
    """Isolated DEVIL_HOME with a minimal string-format config."""
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from devil_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_update_config_for_provider_uses_atomic_yaml_write(self, config_home):
        ...
        from devil_cli.auth import _update_config_for_provider
    ...
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from devil_cli.main import _model_flow_api_key_provider
        ...
        with patch("devil_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("devil_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from devil_cli.main import _model_flow_copilot
        from devil_cli.config import load_config

        with patch(
            "devil_cli.auth.resolve_api_key_provider_credentials",
            return_value={
                "provider": "copilot",
            ...
            "devil_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "devil_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "devil_cli.auth.deactivate_provider",
        ):
    ...
    def test_named_custom_provider_preserves_explicit_api_mode(self, config_home):
        ...
        from devil_cli.main import _model_flow_named_custom
        ...
        with patch("devil_cli.auth._save_model_choice"), \
             patch("devil_cli.auth.deactivate_provider"), \
             patch("devil_cli.models.fetch_api_models", return_value=["gpt-5.4"]), \
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from devil_cli.main import _model_flow_copilot_acp
        from devil_cli.config import load_config

        with patch(
            "devil_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "devil_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("devil_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("devil_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("devil_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("devil_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("devil_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("devil_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        from devil_cli.main import _model_flow_api_key_provider
        from devil_cli.config import load_config, get_env_value

        # User types a shell command instead of a URL at the base URL prompt
        with patch("devil_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("devil_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.devil/.env"):
