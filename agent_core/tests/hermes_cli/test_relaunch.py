"""Tests for devil_cli.relaunch — unified self-relaunch utility."""

import sys

import pytest

from devil_cli import relaunch as relaunch_mod
...
class TestBuildRelaunchArgv:
    ...
    def test_falls_back_to_python_module(self, monkeypatch):
        ...
        argv = relaunch_mod.build_relaunch_argv(["--resume", "abc"])
        assert argv == [sys.executable, "-m", "devil_cli.main", "--resume", "abc"]
