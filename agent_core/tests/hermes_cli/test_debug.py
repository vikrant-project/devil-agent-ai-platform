...
@pytest.fixture
def devil_home(tmp_path, monkeypatch):
    """Set up an isolated DEVIL_HOME with minimal logs."""
    home = tmp_path / ".devil"
    home.mkdir()
    monkeypatch.setenv("DEVIL_HOME", str(home))
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from devil_cli.debug import _upload_paste_rs
        ...
        with patch("devil_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "devil_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from devil_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from devil_cli.debug import upload_to_pastebin

        with patch("devil_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("devil_cli.debug._upload_paste_rs",
             ...
             patch("devil_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, devil_home):
        from devil_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, devil_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (devil_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, devil_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = devil_home / "logs" / "agent.log"
        from devil_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, devil_home):
        ...
        (devil_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, devil_home):
    ...
    def test_unknown_log_returns_none(self, devil_home):
        from devil_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, devil_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from devil_cli.debug import _capture_log_snapshot

        logs_dir = devil_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, devil_home):
    ...
    def test_falls_back_when_primary_empty(self, devil_home):
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def devil_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated DEVIL_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".devil"
        home.mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(home))
    ...
    def test_default_redacts_tail_and_full_text(self, devil_home_with_secret):
        from devil_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, devil_home_with_secret):
        from devil_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, devil_home_with_secret, monkeypatch
    ):
    ...
    def test_capture_default_log_snapshots_threads_redact(
        self, devil_home_with_secret
    ):
        from devil_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, devil_home):
        from devil_cli.debug import collect_debug_report

        with patch("devil_cli.dump.run_dump") as mock_dump:
    ...
    def test_report_includes_agent_log(self, devil_home):
        from devil_cli.debug import collect_debug_report

        with patch("devil_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, devil_home):
        from devil_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, devil_home):
        from devil_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, devil_home, capsys):
        ...
        with patch("devil_cli.dump.run_dump"), \
             patch("devil_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("devil_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, devil_home, capsys):
        ...
        with patch("devil_cli.dump.run_dump"), \
             patch(
                 "devil_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, devil_home, capsys):
    ...
    def test_share_uploads_three_pastes(self, devil_home, capsys):
    ...
    def test_share_keeps_report_and_full_log_on_same_snapshot(self, devil_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from devil_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("devil_cli.dump.run_dump"), \
             patch("devil_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("devil_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, devil_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, devil_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, devil_home_with_secret, capsys
    ):
        ...
        with patch("devil_cli.dump.run_dump"), \
             patch("devil_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("devil_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    ...
    def test_share_subcommand_routes(self, devil_home):
        from devil_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from devil_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from devil_cli.debug import delete_paste
        ...
        with patch("devil_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.devil/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``hermes debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, devil_home):
        ...
        import ast
        import inspect
        from devil_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, devil_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from devil_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, devil_home):
    ...
    def test_merges_with_existing_pending(self, devil_home):
        """Subsequent calls merge into existing pending.json."""
        from devil_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, devil_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, devil_home):
    ...
    def test_sweep_deletes_expired_entries(self, devil_home):
        ...
        with patch("devil_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, devil_home):
        from devil_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("devil_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, devil_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, devil_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, devil_home):
        from devil_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("devil_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, devil_home, capsys):
...
class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from devil_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("devil_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("devil_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, devil_home, capsys):
        from devil_cli.debug import run_debug_share
        ...
        with patch("devil_cli.dump.run_dump"), \
             ...
             patch("devil_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, devil_home, capsys):
        from devil_cli.debug import run_debug_share
        ...
        with patch("devil_cli.dump.run_dump"), \
             patch("devil_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("devil_cli.debug._schedule_auto_delete"):
            run_debug_share(args)

        out = capsys.readouterr().out
        assert "public paste service" in out

    def test_local_no_privacy_notice(self, devil_home, capsys):
        from devil_cli.debug import run_debug_share
