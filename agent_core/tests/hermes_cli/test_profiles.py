"""Comprehensive tests for devil_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and DEVIL_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.devil/profiles)
    * DEVIL_HOME  -> tmp_path/.devil  (so get_devil_home() agrees)
    * Creates the bare-minimum ~/.devil directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".devil"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("DEVIL_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_devil_home(self, profile_env):
    ...
    def test_named_profile_returns_profiles_subdir(self, profile_env):
        ...
        assert result == tmp_path / ".devil" / "profiles" / "coder"

    def test_named_profile_matching_is_case_insensitive(self, profile_env):
        tmp_path = profile_env
        assert get_profile_dir("Coder") == tmp_path / ".devil" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.devil must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".devil"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("devil_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".devil" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_devil_home_returns_default(self, profile_env):
        # DEVIL_HOME points to tmp_path/.devil which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom DEVIL_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("DEVIL_HOME", str(custom))
        # With Docker-aware roots, a custom DEVIL_HOME is the default —
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".devil_history",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.devil_history",
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_devil_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".devil" / "profiles"

    def test_default_devil_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_devil_home()
        assert home == tmp_path / ".devil"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (DEVIL_HOME outside ~/.devil), profiles go under DEVIL_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_devil_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_devil_home() returns DEVIL_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (DEVIL_HOME under ~/.devil), profiles root is still ~/.devil/profiles."""
        native = tmp_path / ".devil"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under DEVIL_HOME."""
        from devil_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under DEVIL_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".devil" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from devil_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".devil"
