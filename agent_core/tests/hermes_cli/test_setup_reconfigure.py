...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("devil_cli.setup.ensure_devil_home", {}),
        ("devil_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("devil_cli.config.is_managed", {"return_value": False}),
        ("devil_cli.setup.load_config", {"return_value": {}}),
        ("devil_cli.setup.save_config", {}),
        ("devil_cli.setup.get_env_value", {"return_value": None}),
        ("devil_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("devil_cli.setup._print_setup_summary", {}),
        ("devil_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("devil_cli.setup.load_config", {"return_value": {}}),
        ("devil_cli.setup.save_config", {}),
        ("devil_cli.auth.get_active_provider", {"return_value": None}),
        ("devil_cli.setup.get_env_value", {"return_value": None}),
        ("devil_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="devil_cli.setup.prompt_choice",
                quick="devil_cli.setup._run_quick_setup",
                model="devil_cli.setup.setup_model_provider",
                terminal="devil_cli.setup.setup_terminal_backend",
                agent="devil_cli.setup.setup_agent_settings",
                gateway="devil_cli.setup.setup_gateway",
                tools="devil_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("devil_cli.setup.prompt_choice", {"return_value": 0}),
                first="devil_cli.setup._run_first_time_quick_setup",
            )
            from devil_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "devil_cli.setup.run_setup_wizard",
