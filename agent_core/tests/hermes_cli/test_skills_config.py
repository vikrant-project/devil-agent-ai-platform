"""Tests for devil_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("devil_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from devil_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("devil_cli.config.load_config")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from devil_cli.skills_config import _get_categories
        skills = [
