"""Tests for Codex auth — tokens stored in Hermes auth store (~/.devil/auth.json)."""
...
def _setup_hermes_auth(devil_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_hermes_auth(devil_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))
    ...
    monkeypatch.setattr("devil_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    devil_home = tmp_path / "hermes"
    _setup_hermes_auth(devil_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "devil_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("devil_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("devil_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
