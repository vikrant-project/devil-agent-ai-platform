...
def test_load_env_caches_on_repeat_calls():
    ...
    from devil_cli.config import invalidate_env_cache, load_env
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from devil_cli import config as config_mod
    from devil_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_devil_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    try:
        with patch("devil_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}
