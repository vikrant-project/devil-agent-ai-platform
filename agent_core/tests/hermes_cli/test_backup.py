...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.devil directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "devil_state.db").write_bytes(b"fake-state")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        from devil_cli.backup import _should_exclude
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        _make_hermes_tree(devil_home)

        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / "config.yaml").write_text("model: test\n")
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from devil_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only devil_state.db (old wrong name) is rejected."""
        from devil_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["devil_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        ...
        assert (devil_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (devil_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (devil_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (devil_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_hermes_prefix(self, tmp_path, monkeypatch):
        """Import strips .devil/ prefix if all entries share it."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".devil/config.yaml": "model: test\n",
            ".devil/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (devil_home / "config.yaml").read_text() == "model: test\n"
        assert (devil_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (devil_home / "config.yaml").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (devil_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (devil_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (devil_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (devil_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".devil"
        src_home.mkdir(parents=True)
        _make_hermes_tree(src_home)

        monkeypatch.setenv("DEVIL_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from devil_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".devil"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("DEVIL_HOME", str(dst_home))
...
class TestFormatSize:
    def test_bytes(self):
        from devil_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        """Detects .devil/ prefix wrapping all entries."""
        import io
        from devil_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".devil/config.yaml", "test")
            zf.writestr(".devil/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".devil/"
...
class TestBackupEdgeCases:
    def test_nonexistent_devil_home(self, tmp_path, monkeypatch):
        """Backup exits when hermes home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".devil"
        monkeypatch.setenv("DEVIL_HOME", str(fake_home))
    ...
    def test_empty_devil_home(self, tmp_path, monkeypatch):
        """Backup handles empty hermes home (no files to back up)."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        # Only excluded dirs, no actual files
        (devil_home / "__pycache__").mkdir()
        (devil_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = devil_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        (devil_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (devil_home / "config.yaml").exists()
        assert (devil_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (devil_home / "profiles" / "coder" / "config.yaml").exists()
        assert (devil_home / "profiles" / "researcher" / "config.yaml").exists()
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import devil_cli.backup as backup_mod
        original_import = __builtins__.__import__ if hasattr(__builtins__, '__import__') else __import__

        def fake_import(name, *a, **kw):
            if name == "devil_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from devil_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def devil_home(self, tmp_path):
        """Create a fake DEVIL_HOME with critical state files."""
        home = tmp_path / ".devil"
        home.mkdir()
    ...
    def test_creates_snapshot(self, devil_home):
        from devil_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(devil_home=devil_home)
        assert snap_id is not None
        snap_dir = devil_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, devil_home):
        from devil_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", devil_home=devil_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, devil_home):
        from devil_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(devil_home=devil_home)
        db_copy = devil_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, devil_home):
        from devil_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(devil_home=devil_home)
        assert (devil_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_missing_files_skipped(self, devil_home):
        from devil_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(devil_home=devil_home)
        with open(devil_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(devil_home=empty) is None

    def test_list_snapshots(self, devil_home):
        from devil_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", devil_home=devil_home)
        id2 = create_quick_snapshot(label="second", devil_home=devil_home)

        snaps = list_quick_snapshots(devil_home=devil_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, devil_home):
        from devil_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", devil_home=devil_home)
        snaps = list_quick_snapshots(limit=3, devil_home=devil_home)
        assert len(snaps) == 3

    def test_restore_config(self, devil_home):
        from devil_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(devil_home=devil_home)

        (devil_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (devil_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, devil_home=devil_home)
        assert result is True
        assert "openrouter" in (devil_home / "config.yaml").read_text()

    def test_restore_state_db(self, devil_home):
        from devil_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(devil_home=devil_home)

        conn = sqlite3.connect(str(devil_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, devil_home=devil_home)
    ...
    def test_restore_nonexistent(self, devil_home):
        from devil_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", devil_home=devil_home) is False

    def test_auto_prune(self, devil_home):
        from devil_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", devil_home=devil_home)
        snaps = list_quick_snapshots(limit=100, devil_home=devil_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, devil_home):
        from devil_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", devil_home=devil_home)
        deleted = prune_quick_snapshots(keep=3, devil_home=devil_home)
        assert deleted == 7
        assert len(list_quick_snapshots(devil_home=devil_home)) == 3

    def test_snapshot_includes_pairing_directories(self, devil_home):
        ...
        (devil_home / "platforms" / "pairing").mkdir(parents=True)
        (devil_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (devil_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (devil_home / "pairing").mkdir()
        (devil_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (devil_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, devil_home):
        ...
        assert restore_quick_snapshot(snap_id, devil_home=devil_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, devil_home):
...
class TestPreUpdateBackup:
    ...
    def test_creates_backup_under_backups_dir(self, devil_home):
        from devil_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(devil_home=devil_home)
        assert out is not None
        assert out.exists()
        assert out.parent == devil_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, devil_home):
    ...
    def test_does_not_recurse_into_prior_backups(self, devil_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from devil_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(devil_home=devil_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(devil_home=devil_home)
    ...
    def test_rotation_keeps_only_n(self, devil_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(devil_home=devil_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (devil_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, devil_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from devil_cli.backup import create_pre_update_backup

        (devil_home / "backups").mkdir(exist_ok=True)
        manual = devil_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(devil_home=devil_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from devil_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(devil_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, devil_home):
        ...
        """
        from devil_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(devil_home=devil_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, devil_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, devil_home):
        ...
        first = create_pre_update_backup(devil_home=devil_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(devil_home=devil_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(devil_home=devil_home, keep=0)
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def devil_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".devil"
        root.mkdir()
        _make_hermes_tree(root)
        # Point DEVIL_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("DEVIL_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for devil_cli.config + devil_constants so they pick up DEVIL_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("devil_cli.config") or mod == "devil_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, devil_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from devil_cli.main import _run_pre_update_backup
        ...
        backups = list((devil_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, devil_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (devil_home / "backups").exists() or not list(
            (devil_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, devil_home, capsys):
        from devil_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, devil_home, capsys):
        ...
        import yaml
        (devil_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("devil_cli.config"):
    ...
    def test_config_disabled_is_silent(self, devil_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((devil_home / "backups").glob("pre-update-*.zip")) \
            if (devil_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, devil_home, capsys):
...
class TestPreMigrationBackup:
    ...
    ``hermes claw migrate`` runs before mutating ~/.devil/."""
    ...
    def test_creates_backup_under_backups_dir(self, devil_home):
        from devil_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(devil_home=devil_home)
        assert out is not None
        assert out.exists()
    ...
    def test_backup_uses_shared_exclusion_rules(self, devil_home):
    ...
    def test_restorable_with_hermes_import(self, devil_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Hermes
        backup — `hermes import` should accept it."""
        from devil_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(devil_home=devil_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, devil_home):
        from devil_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(devil_home=devil_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(devil_home=devil_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, devil_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(devil_home=devil_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((devil_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_devil_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.devil yet — nothing to back up."""
        from devil_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(devil_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, devil_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from devil_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(devil_home=devil_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(devil_home=devil_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
