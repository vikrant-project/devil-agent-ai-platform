...
from devil_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
...
def test_setup_wizard_codex_import_resolves():
    ...
    # This mirrors the exact import used in devil_cli/setup.py line 873.
    # A prior bug had 'get_codex_models' (wrong) instead of 'get_codex_model_ids'.
    from devil_cli.codex_models import get_codex_model_ids as setup_import
...
def test_get_codex_model_ids_adds_forward_compat_models_from_templates(monkeypatch):
    monkeypatch.setattr(
        "devil_cli.codex_models._fetch_models_from_api",
...
def test_fetch_from_api_keeps_supported_in_api_false_models(monkeypatch):
    ...
    """
    import sys
    from devil_cli import codex_models
...
def test_model_command_uses_runtime_access_token_for_codex_list(monkeypatch):
    from devil_cli.main import _model_flow_openai_codex
    ...
    monkeypatch.setattr(
        "devil_cli.auth.get_codex_auth_status",
...
def test_model_command_prompts_to_reuse_or_reauthenticate_codex_session(monkeypatch, capsys):
    ...
    monkeypatch.setattr("devil_cli.auth._login_openai_codex", _fake_login)
    monkeypatch.setattr(
        "devil_cli.codex_models.get_codex_model_ids",
...
def test_model_command_uses_existing_codex_session_without_relogin(monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.auth._login_openai_codex",
