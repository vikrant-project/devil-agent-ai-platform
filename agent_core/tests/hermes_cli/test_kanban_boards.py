...
def fresh_home(tmp_path, monkeypatch):
    """Isolated DEVIL_HOME with no prior kanban state.

    The autouse hermetic conftest already nukes credentials + TZ; this
    fixture layers a per-test DEVIL_HOME plus a path-init cache reset
    so each test sees a truly empty board set.
    """
    home = tmp_path / "devil_home"
    home.mkdir()
    ...
    # Also reset devil_constants cache so get_default_hermes_root() re-reads.
    try:
        import devil_constants
        devil_constants._cached_default_hermes_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, "-m", "devil_cli.main", "kanban"] + args,
        env=env,
        capture_output=True,
        text=True,
        cwd=str(_WORKTREE),
