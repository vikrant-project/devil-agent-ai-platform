...
from devil_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from devil_cli.auth import _resolve_verify
...
def _setup_nous_auth(
    ...
    *,
    ...
    (devil_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from devil_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    devil_home = tmp_path / "hermes"
    _setup_nous_auth(devil_home, access_token="at-123")
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    devil_home = tmp_path / "hermes"
    _setup_nous_auth(devil_home, access_token="at-fresh")
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    devil_home = tmp_path / "hermes"
    _setup_nous_auth(devil_home, refresh_token="refresh-old")
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))
    ...
    monkeypatch.setattr("devil_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("devil_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = devil_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return devil_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import devil_cli.auth as auth_mod
        import devil_cli.models as models_mod
        import devil_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from devil_cli.auth import PROVIDER_REGISTRY, _login_nous

        devil_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    """
    from devil_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    """
    from devil_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    """
    from devil_cli.auth import _refresh_access_token
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    writing to the user's real ``~/.devil/shared/`` across CI runs.
    """
    from devil_cli.auth import _nous_shared_store_path
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    ...
    from devil_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from devil_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from devil_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from devil_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    devil_home = tmp_path / "hermes"
    devil_home.mkdir(parents=True, exist_ok=True)
    (devil_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from devil_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_returns_none_on_refresh_failure(
    ...
    """
    from devil_cli import auth as auth_mod
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different DEVIL_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("DEVIL_HOME", str(profile_a))
    ...
    # Profile B: fresh DEVIL_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("DEVIL_HOME", str(profile_b))
