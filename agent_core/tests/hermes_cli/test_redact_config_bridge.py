...
`HERMES_REDACT_SECRETS` at module-import time. `devil_cli/main.py` at
...
env var in `devil_cli/main.py` BEFORE the `setup_logging()` call.
"""
import os
import subprocess
import sys
...
def test_redact_secrets_false_in_config_yaml_is_honored(tmp_path):
    ...
    # Spawn a fresh Python process that imports devil_cli.main and checks
    ...
    probe = textwrap.dedent(
        ...
        sys.path.insert(0, %r)
        import devil_cli.main  # triggers the bridge + setup_logging
        import agent.redact
...
def test_dotenv_redact_secrets_beats_config_yaml(tmp_path):
    ...
    (devil_home / ".env").write_text("HERMES_REDACT_SECRETS=true\n")
