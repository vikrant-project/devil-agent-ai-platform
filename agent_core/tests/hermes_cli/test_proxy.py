...
from devil_cli.proxy.adapters import ADAPTERS, get_adapter
from devil_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from devil_cli.proxy.adapters.nous_portal import NousPortalAdapter
...
def _write_auth_store(devil_home: Path, nous_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given nous state into a hermetic DEVIL_HOME."""
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # DEVIL_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_get_credential_refreshes_and_persists(tmp_path, monkeypatch):
    ...
    with patch(
        "devil_cli.proxy.adapters.nous_portal.refresh_nous_oauth_from_state",
...
from devil_cli.proxy.server import create_app  # noqa: E402
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("DEVIL_HOME", str(tmp_path))
    from devil_cli.proxy.cli import cmd_proxy_status
...
def test_cmd_proxy_providers_runs(capsys):
    from devil_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from devil_cli.proxy.cli import cmd_proxy_start
