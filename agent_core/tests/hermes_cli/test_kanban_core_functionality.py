...
Complements tests/devil_cli/test_kanban_db.py (schema + CAS atomicity)
and tests/devil_cli/test_kanban_cli.py (end-to-end run_slash).  The
...
def test_list_profiles_on_disk(tmp_path, monkeypatch):
    ...
    named profiles under ~/.devil/profiles/ that contain a config.yaml."""
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.delenv("DEVIL_HOME", raising=False)
    profiles = tmp_path / ".devil" / "profiles"
...
def test_list_profiles_on_disk_custom_root(tmp_path, monkeypatch):
    """list_profiles_on_disk respects a custom DEVIL_HOME root."""
...
def test_parse_duration_accepts_formats():
    from devil_cli.kanban import _parse_duration
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    # Note: devil_cli.main doesn't propagate sub-command exit codes
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    """First CLI action on an empty DEVIL_HOME must not error with
    ...
    env = {**os.environ, "DEVIL_HOME": str(home),
           "PYTHONPATH": str(worktree_root)}
    r = _sp.run(
        [_sys.executable, "-m", "devil_cli.main", "kanban",
...
def test_connect_auto_inits_fresh_db(tmp_path, monkeypatch):
    """Calling connect() on a fresh DEVIL_HOME must create the
...
def test_check_dispatcher_presence_silent_when_gateway_running(monkeypatch):
    from devil_cli import kanban as kb_cli
...
def test_gateway_dispatcher_watcher_respects_config_flag_off(monkeypatch):
    ...
    import devil_cli.config as _cfg_mod
