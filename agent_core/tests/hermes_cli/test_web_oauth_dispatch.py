"""Regression tests for the OAuth dispatcher in devil_cli.web_server.
...
from devil_cli.web_server import _SESSION_TOKEN, app
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "devil_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "devil_cli.auth._minimax_pkce_pair",
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    from devil_cli import web_server as ws
    ...
    try:
        with patch(
            "devil_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "devil_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "devil_cli.web_server._start_anthropic_pkce",
