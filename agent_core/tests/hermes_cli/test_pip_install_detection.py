...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("devil_cli.config.get_managed_system", return_value=None):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("devil_cli.config.get_managed_system", return_value="NixOS"):
        from devil_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from devil_cli.config import recommended_update_command_for_method
