...
def _write_auth_store(tmp_path, payload: dict) -> None:
    devil_home = tmp_path / "hermes"
    devil_home.mkdir(parents=True, exist_ok=True)
    (devil_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("DEVIL_HOME", str(tmp_path / "hermes"))
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from devil_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from devil_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    ...
    (devil_home / "config.yaml").write_text(
        "model:\n"
    ...
    config_text = (devil_home / "config.yaml").read_text()
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((devil_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from devil_cli.auth import _reset_config_provider
    ...
    with patch("devil_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from devil_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "devil_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("devil_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("devil_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("devil_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "devil_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (devil_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from devil_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((devil_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from devil_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from devil_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    ...
    (devil_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((devil_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((devil_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    ...
    by the shell (not written into ~/.devil/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.devil/.env (not the shell), the
    ...
    (devil_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (devil_home / ".env").read_text().strip() == ""
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from devil_cli.auth import is_source_suppressed
    from devil_cli.auth_commands import auth_add_command
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import devil_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hermes_pkce must not re-seed from ~/.devil/.anthropic_oauth.json when suppressed."""
    devil_home = tmp_path / "hermes"
    devil_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))

    import yaml
    (devil_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (devil_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (devil_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
