...
import devil_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``DEVIL_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../hermes_test`` DEVIL_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir DEVIL_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="DEVIL_HOME=/tmp/pytest-of-alice/pytest-42/'
...
class TestGeneratedSystemdUnits:
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_devil_home_for_target_user", lambda home: "/home/alice/.devil")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.devil.gateway.plist"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.devil.gateway"])
            ),
        )
...
class TestSystemUnitHermesHome:
    """DEVIL_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'DEVIL_HOME=/home/alice/.devil' in unit
        assert '/root/.devil' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: DEVIL_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("DEVIL_HOME", "/root/.devil/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'DEVIL_HOME=/home/alice/.devil/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_devil_home(self, monkeypatch):
        # Custom DEVIL_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("DEVIL_HOME", "/opt/hermes-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'DEVIL_HOME=/opt/hermes-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's DEVIL_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        devil_home = str(gateway_cli.get_devil_home().resolve())
        assert f'DEVIL_HOME={devil_home}' in unit


class TestHermesHomeForTargetUser:
    """Unit tests for _devil_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("DEVIL_HOME", raising=False)

        result = gateway_cli._devil_home_for_target_user("/home/alice")
        assert result == "/home/alice/.devil"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._devil_home_for_target_user("/home/alice")
        assert result == "/home/alice/.devil/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("DEVIL_HOME", "/opt/hermes")
...
class TestProfileArg:
    ...
    def test_default_devil_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.devil should not produce a --profile flag."""
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        result = gateway_cli._profile_arg(str(devil_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.devil/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".devil" / "profiles" / "mybot"
        profile_dir.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("DEVIL_HOME", str(tmp_path / ".devil"))
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile DEVIL_HOME should return empty string."""
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.devil/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".devil" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".devil" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_devil_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".devil" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.devil.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".devil" / "hermes-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".devil" / "hermes-agent")
    ...
    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        ...
        original = str(tmp_path / "alice" / ".devil" / "hermes-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        ...
        root_home.mkdir()
        project = root_home / ".devil" / "hermes-agent"
        ...
        monkeypatch.setenv("DEVIL_HOME", str(root_home / ".devil"))
        monkeypatch.setattr(gateway_cli, "get_devil_home", lambda: root_home / ".devil")
        ...
        assert "WorkingDirectory=/home/alice/.devil/hermes-agent" in unit
...
class TestLegacyHermesUnitDetection:
    ...
    _OUR_UNIT_TEXT = (
        ...
        "ExecStart=/usr/bin/python -m devil_cli.main gateway run --replace\n"
    )
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m devil_cli.main gateway run
          - python path/to/devil_cli/main.py gateway run
          - hermes gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m devil_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/hermes/devil_cli/main.py gateway run",
...
class TestMigrateLegacyCommand:
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import devil_cli.main as cli_main
        ...
        if project_root is None:
            import devil_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "devil_cli.main", "gateway", "--help"],
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "devil_cli.main", "gateway", "status", "-l", "--help"],
