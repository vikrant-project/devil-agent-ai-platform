...
import pytest
from devil_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from devil_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("devil_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("devil_cli.setup.ensure_devil_home"),
            patch("devil_cli.setup.load_config", return_value={}),
            patch("devil_cli.setup.get_devil_home", return_value="/tmp/.devil"),
            patch("devil_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        ...
        from devil_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("devil_cli.main._has_any_provider_configured", return_value=False),
            patch("devil_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
