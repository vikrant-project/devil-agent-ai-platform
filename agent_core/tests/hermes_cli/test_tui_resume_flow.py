...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from devil_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from devil_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "devil_state", mod("devil_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "devil_cli.config",
        mod("devil_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "devil_cli.models",
        mod("devil_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "devil_cli.runtime_provider",
        mod(
            "devil_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "devil_cli.tools_config",
        mod("devil_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import devil_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "devil_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
