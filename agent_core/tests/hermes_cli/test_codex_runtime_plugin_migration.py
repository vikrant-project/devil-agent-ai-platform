...
from devil_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
...
class TestMigrate:
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from devil_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from devil_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_expose_hermes_tools_writes_callback_mcp_entry(self, tmp_path):
        ...
        [mcp_servers.devil-tools] entry is written so codex calls back
        ...
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.devil-tools]" in text
    ...
    def test_expose_hermes_tools_disabled_skips_entry(self, tmp_path):
        ...
        migrate({}, codex_home=tmp_path,
                ...
                expose_hermes_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.devil-tools]" not in text
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        assert "[mcp_servers.devil-mcp]" in final
...
class TestStripUnmanagedPluginTables:
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "devil_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
...
# ---- Bug C: DEVIL_HOME tempdir leak into ~/.codex/config.toml ----


class TestHermesHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_hermes_tools_mcp_entry()`` read ``DEVIL_HOME``
    ...
    def test_tempdir_detector_accepts_real_devil_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.devil")
        assert not _looks_like_test_tempdir("/home/bob/.devil")
    ...
    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when DEVIL_HOME points at a pytest
        ...
        env = entry.get("env", {})
        assert "DEVIL_HOME" not in env, (
            f"pytest-tempdir DEVIL_HOME leaked into codex MCP entry: "
            f"{env.get('DEVIL_HOME')!r}"
        )

    def test_real_devil_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate DEVIL_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.devil"
        monkeypatch.setenv("DEVIL_HOME", real_path)
        entry = _build_hermes_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("DEVIL_HOME") == real_path

    def test_unset_devil_home_omits_env_key(self, monkeypatch):
        """When DEVIL_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever DEVIL_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "DEVIL_HOME" not in env, (
            f"DEVIL_HOME should not be set when env var is unset, got: "
            f"{env.get('DEVIL_HOME')!r}"
        )
