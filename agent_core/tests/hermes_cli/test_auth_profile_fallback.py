...
When ``DEVIL_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.devil/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.devil            (has its own auth.json fixture)
    * Profile     -> tmp_path/.devil/profiles/coder   (active, DEVIL_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (DEVIL_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.devil as the "real home" — which is NOT equal
    # to the DEVIL_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    devil_home = tmp_path / "classic"
    devil_home.mkdir()
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))

    _write(devil_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from devil_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: DEVIL_HOME is set to a custom path that is NOT under
    # ~/.devil/profiles/ — get_default_hermes_root() returns DEVIL_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from devil_cli.auth import read_credential_pool, write_credential_pool
