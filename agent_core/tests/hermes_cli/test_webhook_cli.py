"""Tests for devil_cli/webhook.py — webhook subscription CLI."""
...
def _isolate(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "devil_cli.webhook._is_webhook_enabled", lambda: True
    )
...
class TestWebhookEnabledGate:
    def test_blocks_when_disabled(self, capsys, monkeypatch):
        monkeypatch.setattr("devil_cli.webhook._is_webhook_enabled", lambda: False)
    ...
    def test_real_check_disabled(self, monkeypatch):
        monkeypatch.setattr(
            "devil_cli.webhook._get_webhook_config",
            lambda: {},
        )
        monkeypatch.setattr(
            "devil_cli.webhook._is_webhook_enabled",
            lambda: bool({}.get("enabled")),
        )
        import devil_cli.webhook as wh_mod
