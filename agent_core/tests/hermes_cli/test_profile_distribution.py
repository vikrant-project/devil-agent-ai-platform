"""Tests for devil_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/devil_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestInstall:
    ...
    def test_install_enforces_hermes_requires(self, profile_env, monkeypatch):
        ...
        import devil_cli
        monkeypatch.setattr(devil_cli, "__version__", "0.1.0", raising=False)
...
class TestUpdate:
    ...
    def test_update_missing_manifest_errors(self, profile_env):
        ...
        from devil_cli.profiles import create_profile
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        from devil_cli.profiles import get_profile_dir
        ...
        monkeypatch.setattr(
            "devil_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from devil_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from devil_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from devil_cli.profiles import create_profile, list_profiles, get_profile_dir
