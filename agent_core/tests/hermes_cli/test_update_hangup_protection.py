...
``_finalize_update_output`` in ``devil_cli/main.py``.  These exist so
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_devil_home if present
        import devil_cli.config as _cfg
        if hasattr(_cfg, "_DEVIL_HOME_CACHE"):
            _cfg._DEVIL_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_devil_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "devil_cli.config.get_devil_home", _boom, raising=True
        )
