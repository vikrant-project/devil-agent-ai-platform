...
from devil_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """devil_cli/main.py uses time.sleep(3) after systemctl restart to
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>DEVIL_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_restarts_profile_manual_gateways(
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "ai.devil.gateway.plist",
        )
        ...
        process = gateway_cli.ProfileGatewayProcess(
            profile="coder",
            path=tmp_path / ".devil" / "profiles" / "coder",
            pid=12345,
        )
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "devil_cli.gateway._graceful_restart_via_sigusr1",
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.devil.gateway")
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.devil/hermes-agent/venv/bin/python -m devil_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.devil/hermes-agent/venv/bin/python -m devil_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr("os.getpid", lambda: 999)
        monkeypatch.setattr(gateway_cli, "_get_service_pids", lambda: set())
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda devil_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point DEVIL_HOME at a temp dir so the marker file lands there
        devil_home = tmp_path / ".devil"
        devil_home.mkdir()
        monkeypatch.setenv("DEVIL_HOME", str(devil_home))
        import devil_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_devil_home", lambda: devil_home)
        # Also patch the module-level ref used by cmd_update
        import devil_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_devil_home", lambda: devil_home)
