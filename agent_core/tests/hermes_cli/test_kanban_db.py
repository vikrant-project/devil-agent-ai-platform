"""Tests for the Kanban DB layer (devil_cli.kanban_db)."""
...
from devil_cli import kanban_db as kb


@pytest.fixture
def kanban_home(tmp_path, monkeypatch):
    """Isolated DEVIL_HOME with an empty kanban DB."""
    home = tmp_path / ".devil"
    home.mkdir()
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import devil_cli.kanban_db as _kb
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from devil_cli import profiles
...
def test_scratch_workspace_created_under_devil_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# where `kanban_db_path()` resolved to the active profile's DEVIL_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's DEVIL_HOME."""

    def _set_home(self, monkeypatch, tmp_path, devil_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: DEVIL_HOME == ~/.devil, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.devil/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.devil/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.devil/kanban.db.
        default_home = tmp_path / ".devil"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # DEVIL_HOME and confirm equality. This is the property the
        ...
        monkeypatch.setenv("DEVIL_HOME", str(profile_home))
    ...
    def test_docker_custom_devil_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: DEVIL_HOME points outside ~/.devil.
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: DEVIL_HOME=/opt/hermes/profiles/coder;
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # DEVIL_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile DEVIL_HOME and reads.
        monkeypatch.setenv("DEVIL_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # `-p <profile>` flag rewrites DEVIL_HOME.
        default_home = tmp_path / ".devil"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see devil_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("devil_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="devil_state"):
            conn = kb.connect()
...
def test_resolve_hermes_argv_prefers_path_shim(monkeypatch):
    ...
    import shutil
    import devil_cli.kanban_db as kb
...
def test_resolve_hermes_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m devil_cli.main`.
    ...
    argv = kb._resolve_hermes_argv()
    assert argv == [sys.executable, "-m", "devil_cli.main"]
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    sufficient — if `devil_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m devil_cli.main --version`
