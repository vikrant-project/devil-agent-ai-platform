"""Tests for devil_bootstrap — Windows UTF-8 stdio shim.
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hermes entry point imports devil_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported devil_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("devil_bootstrap", None)
    import devil_bootstrap  # noqa: WPS433
    return devil_bootstrap
...
class TestEntryPointsImportBootstrap:
    """Every Hermes entry point must import devil_bootstrap as its
    ...
    # import devil_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "devil_cli/main.py",   # hermes CLI (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import devil_bootstrap' and that
        ...
        ``devil_bootstrap`` as a top-level module) leaves hermes
        ...
        # at tests/test_devil_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hermes start even when devil_bootstrap hasn't been
        ...
        assert first_import_name == "devil_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'devil_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import devil_bootstrap' line to be the first import."
        )
