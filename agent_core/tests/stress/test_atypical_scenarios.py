...
normal tests assume away:
  ...
  - Filesystem: DEVIL_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own DEVIL_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            ...
            os.environ["DEVIL_HOME"] = home
            os.environ["HOME"] = home
            for m in list(sys.modules.keys()):
                if m.startswith(("devil_cli", "plugins", "gateway")):
                    del sys.modules[m]
            sys.path.insert(0, str(WT))
            from devil_cli import kanban_db as kb  # noqa: F401
            print(f"\n═══ {name} ═══")
            try:
                fn(home, kb)
                print(f"  ✔ {name}")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "DEVIL_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
    ...
    for bad in bad_metas:
        r = subprocess.run(
            [sys.executable, "-m", "devil_cli.main", "kanban",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from devil_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["DEVIL_HOME"]).resolve())
            if not resolved_abs.startswith(home_abs) and resolved_abs.startswith("/tmp"):
                ...
                print(f"  ⚠ workspace resolved OUTSIDE devil_home: {resolved}")
...
@scenario("devil_home_with_spaces")
def _(home, kb):
    """DEVIL_HOME at a path with spaces — should work but catches
    ...
    try:
        ...
        print(f"  DEVIL_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("devil_home_with_unicode")
def _(home, kb):
    """DEVIL_HOME with non-ASCII chars."""
    ...
    try:
        ...
        print(f"  DEVIL_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)


@scenario("devil_home_via_symlink")
def _(home, kb):
    """DEVIL_HOME is a symlink to the real dir. _INITIALIZED_PATHS
    ...
    try:
        ...
        conn2.close()
        print("  symlinks to same DEVIL_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from devil_cli import web_server as ws  # noqa
    except Exception:
        pass
