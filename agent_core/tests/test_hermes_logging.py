"""Tests for devil_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    devil_logging._logging_initialized = False
    ...
    devil_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    devil_logging._logging_initialized = False
    devil_logging.clear_session_context()


@pytest.fixture
def devil_home(tmp_path, monkeypatch):
    """Provide an isolated DEVIL_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_devil_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["DEVIL_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, devil_home):
        log_dir = devil_logging.setup_logging(devil_home=devil_home)
        assert log_dir == devil_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)
        devil_logging.setup_logging(devil_home=devil_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        devil_logging.setup_logging(devil_home=devil_home, force=True)
    ...
    def test_custom_log_level(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, devil_home):
        devil_logging.setup_logging(
            devil_home=devil_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)
    ...
    def test_writes_to_agent_log(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)

        test_logger = logging.getLogger("test_devil_logging.write_test")
        ...
        agent_log = devil_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)

        test_logger = logging.getLogger("test_devil_logging.warning_test")
        ...
        agent_log = devil_home / "logs" / "agent.log"
        errors_log = devil_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)

        test_logger = logging.getLogger("test_devil_logging.info_test")
    ...
    def test_explicit_params_override_config(self, devil_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (devil_home / "config.yaml").write_text(yaml.dump(config))

        devil_logging.setup_logging(devil_home=devil_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, devil_home):
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, devil_home):
        ...
        gw_log = devil_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, devil_home):
    ...
    def test_gateway_log_receives_gateway_records(self, devil_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, devil_home):
    ...
    def test_agent_log_still_receives_all(self, devil_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, devil_home):
        """When session context is set, log lines include [session_id]."""
        devil_logging.setup_logging(devil_home=devil_home)
        devil_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, devil_home):
    ...
    def test_clear_session_context(self, devil_home):
        """After clearing, session tag disappears."""
        devil_logging.setup_logging(devil_home=devil_home)
        devil_logging.set_session_context("xyz789")
        devil_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, devil_home):
        ...
        def thread_a():
            devil_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            devil_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        devil_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        devil_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = devil_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = devil_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in devil_logging.COMPONENT_PREFIXES
        assert ("gateway",) == devil_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = devil_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == devil_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = devil_logging.COMPONENT_PREFIXES["cli"]
        assert "devil_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == devil_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)
        devil_logging.setup_verbose_logging()
    ...
    def test_idempotent(self, devil_home):
        devil_logging.setup_logging(devil_home=devil_home)
        devil_logging.setup_verbose_logging()
        devil_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        devil_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = devil_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        devil_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("devil_cli.config.is_managed", return_value=True):
                devil_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, devil_home):
        level, max_size, backup = devil_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, devil_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, devil_home):
        import yaml
        config = {"model": "test"}
        (devil_home / "config.yaml").write_text(yaml.dump(config))
