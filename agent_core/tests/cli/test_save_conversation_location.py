...
The fix writes snapshots under ``~/.devil/sessions/saved/`` and prints
...
def devil_home(tmp_path, monkeypatch):
    ...
    # Clear any cached devil_home computation
    import devil_constants
    if hasattr(devil_constants, "_devil_home_cache"):
        devil_constants._devil_home_cache = None
    return home
...
def test_save_conversation_writes_under_devil_home(devil_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.devil/sessions/saved/, not CWD."""
    # Change CWD to a different directory to prove the file does NOT go there.
    work = tmp_path / "somewhere-else"
    work.mkdir()
    monkeypatch.chdir(work)

    # Import fresh to pick up the DEVIL_HOME fixture
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "devil_constants"]:
    ...
    # File MUST be under ~/.devil/sessions/saved/
    saved_dir = devil_home / "sessions" / "saved"
...
def test_save_conversation_empty_history_does_nothing(devil_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "devil_constants"]:
        sys.modules.pop(mod, None)
    import cli
