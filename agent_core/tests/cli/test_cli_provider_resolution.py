...
from devil_cli import main as hermes_main
...
def test_devil_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}
    ...
    monkeypatch.setattr("devil_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("devil_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("devil_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "devil_cli.codex_models.get_codex_model_ids",
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setattr("devil_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
    ...
    monkeypatch.setattr(
        "devil_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "devil_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "devil_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("devil_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("devil_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("devil_cli.auth._update_config_for_provider", lambda provider, url: None)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "devil_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("devil_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("devil_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("devil_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("devil_cli.auth.resolve_provider", _resolve_provider)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "devil_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("devil_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("devil_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("devil_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("devil_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "devil_cli.models.probe_api_models",
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("devil_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "devil_cli.main._save_custom_provider",
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    ...
    monkeypatch.setattr("devil_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("devil_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("devil_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("devil_cli.auth.get_provider_auth_state", lambda provider_id: None)
    ...
    monkeypatch.setattr("devil_cli.auth._login_nous", _fake_login)
...
def test_auto_provider_name_localhost():
    from devil_cli.main import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from devil_cli.main import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "devil_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("devil_cli.config.save_config", _save)
