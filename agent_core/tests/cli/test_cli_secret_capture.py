...
def test_secret_capture_callback_can_be_completed_from_cli_state_machine():
    ...
    with patch("devil_cli.callbacks.save_env_value_secure") as save_secret:
...
def test_secret_capture_uses_getpass_without_tui():
    ...
    with patch("devil_cli.callbacks.getpass.getpass", return_value="secret-value"), patch(
        "devil_cli.callbacks.save_env_value_secure"
    ) as save_secret:
        save_secret.return_value = {
            "success": True,
            "stored_as": "TENOR_API_KEY",
...
def test_secret_capture_timeout_clears_hidden_input_buffer():
    ...
    with patch("devil_cli.callbacks.queue.Queue.get", side_effect=queue.Empty), patch(
        "devil_cli.callbacks._time.monotonic",
        side_effect=[0, 121],
    ):
        result = prompt_for_secret(cli, "TENOR_API_KEY", "Tenor API key")
