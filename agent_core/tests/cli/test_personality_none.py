...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._devil_home", tmp_path), \
             patch("devil_constants.display_devil_home", return_value="~/.devil/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.devil/profiles/coder/config.yaml`"
