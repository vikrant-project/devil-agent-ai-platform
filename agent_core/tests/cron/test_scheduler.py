...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._devil_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("dotenv.load_dotenv"), \
             patch("devil_state.SessionDB", return_value=fake_db), \
             patch(
    ...
    def _make_run_job_patches(self, tmp_path):
        ...
        fake_db = MagicMock()
        return fake_db, [
            patch("cron.scheduler._devil_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("dotenv.load_dotenv"),
            patch("devil_state.SessionDB", return_value=fake_db),
            patch(
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        with patches[0], patches[1], patches[2], patches[3], patches[4], \
             patch("run_agent.AIAgent") as mock_agent_cls, \
             patch(
                 "devil_cli.tools_config._get_platform_tools",
...
class TestRunJobSkillBacked:
    ...
    def test_run_job_preserves_credential_file_passthrough_into_worker_thread(self, tmp_path):
        ...
        with patch("cron.scheduler._devil_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("tools.credential_files._resolve_devil_home", return_value=tmp_path), \
             patch("dotenv.load_dotenv"), \
             patch("devil_state.SessionDB", return_value=fake_db), \
             patch(
                 "devil_cli.runtime_provider.resolve_runtime_provider",
