...
- Path resolution (absolute, relative to DEVIL_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp DEVIL_HOME."""
    devil_home = tmp_path / ".devil"
    devil_home.mkdir()
    (devil_home / "cron").mkdir()
    (devil_home / "cron" / "output").mkdir()
    (devil_home / "scripts").mkdir()
    monkeypatch.setenv("DEVIL_HOME", str(devil_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "HERMES_DIR", devil_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", devil_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", devil_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", devil_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.devil/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
