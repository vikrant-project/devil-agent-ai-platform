...
def _load_supermemory_config(devil_home: str) -> dict:
    config = _default_config()
    config_path = Path(devil_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, devil_home: str) -> None:
    config_path = Path(devil_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        ...
        # All other options are documented for $DEVIL_HOME/supermemory.json
    ...
    def save_config(self, values, devil_home):
        ...
        _save_supermemory_config(sanitized, devil_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from devil_constants import get_devil_home
        self._devil_home = kwargs.get("devil_home") or str(get_devil_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._devil_home)
