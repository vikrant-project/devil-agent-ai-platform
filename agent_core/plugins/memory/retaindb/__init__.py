...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        if explicit:
            ...
            devil_home = str(kwargs.get("devil_home", ""))
            profile_name = os.path.basename(devil_home) if devil_home else ""
            project = f"hermes-{profile_name}" if (profile_name and profile_name not in {"", ".devil"}) else "default"
        ...
        devil_home_path = get_devil_home()
        db_path = devil_home_path / "retaindb_queue.db"
        self._queue = _WriteQueue(self._client, db_path)

        # Seed agent identity from SOUL.md in background
        soul_path = devil_home_path / "SOUL.md"
        if soul_path.exists():
