...
Resolution order for config file:
  1. $DEVIL_HOME/honcho.json  (instance-local, enables isolated Hermes instances)
...
def resolve_active_host() -> str:
    ...
    Resolution order:
      1. DEVIL_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``hermes.<profile>``
      3. Fallback: ``"hermes"`` (default profile)
    """
    explicit = os.environ.get("DEVIL_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $DEVIL_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.devil/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_devil_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_devil_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # True when Honcho was explicitly configured for this host (hosts.devil
    ...
    def from_global_config(
        ...
        Resolution: $DEVIL_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        ...
        # A hosts.devil block or explicit enabled flag means the user
