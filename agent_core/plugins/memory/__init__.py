...
2. User-installed providers: ``$DEVIL_HOME/plugins/<name>/``
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$DEVIL_HOME/plugins/`` or None if unavailable."""
    try:
        from devil_constants import get_devil_home
        d = get_devil_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$DEVIL_HOME/plugins/<name>/``) directories.  Bundled takes
