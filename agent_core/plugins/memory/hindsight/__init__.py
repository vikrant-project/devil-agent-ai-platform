...
Or via $DEVIL_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $DEVIL_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_devil_home() / "hindsight" / "config.json"
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Devil Agent and the User"
    ...
    def save_config(self, values, devil_home):
        """Write config to $DEVIL_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(devil_home) / "hindsight"
    ...
    def post_setup(self, devil_home: str, config: dict) -> None:
        ...
        from devil_cli.memory_setup import _curses_select
        ...
        self.save_config(provider_config, devil_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(devil_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(devil_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Devil Agent and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Devil Agent and the User")
