...
``devil_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__DEVIL_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``devil_cli/web_server.py``.
...
from devil_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from devil_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``devil_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from devil_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``DEVIL_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from devil_cli.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See devil_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    prev_env = os.environ.get("DEVIL_KANBAN_BOARD")
    try:
        os.environ["DEVIL_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from devil_cli import kanban_specify  # noqa: WPS433 (intentional)
        ...
        if prev_env is None:
            ...
            os.environ["DEVIL_KANBAN_BOARD"] = prev_env
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.devil/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.devil/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in devil_cli/web_server.py.
