...
Required env vars (set via ``hermes tools`` or ~/.devil/.env):
  DEVIL_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  DEVIL_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  DEVIL_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  DEVIL_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  DEVIL_LANGFUSE_RELEASE     - release/version tag
  DEVIL_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  DEVIL_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  DEVIL_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
_LANGFUSE_KEY_PREFIXES: Dict[str, str] = {
    "DEVIL_LANGFUSE_PUBLIC_KEY": "pk-lf-",
    "DEVIL_LANGFUSE_SECRET_KEY": "sk-lf-",
}
...
def _debug_enabled() -> bool:
    return _env_bool("DEVIL_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    public_key = _env("DEVIL_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("DEVIL_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("DEVIL_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("DEVIL_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset DEVIL_LANGFUSE_PUBLIC_KEY / DEVIL_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("DEVIL_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("DEVIL_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("DEVIL_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("DEVIL_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid DEVIL_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("DEVIL_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
