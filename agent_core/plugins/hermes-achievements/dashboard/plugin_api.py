...
try:
    ...
    import os as _os
    def get_devil_home() -> Path:  # type: ignore[misc]
        val = (_os.environ.get("DEVIL_HOME") or "").strip()
...
def state_path() -> Path:
    return get_devil_home() / "plugins" / "hermes-achievements" / "state.json"


def snapshot_path() -> Path:
    return get_devil_home() / "plugins" / "hermes-achievements" / "scan_snapshot.json"


def checkpoint_path() -> Path:
    return get_devil_home() / "plugins" / "hermes-achievements" / "scan_checkpoint.json"
...
def analyze_messages(session_id: str, title: str, messages: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "plugin_events": len(re.findall(r"plugin|dashboard-plugins|__DEVIL_PLUGIN|manifest\.json", full_text, re.I)),
