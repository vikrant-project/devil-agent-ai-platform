"""disk_cleanup — ephemeral file cleanup for Devil Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under DEVIL_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly DEVIL_HOME and /tmp/hermes-*
Never touches: ~/.devil/logs/ or any system directory.
"""
...
try:
    ...
    def get_devil_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("DEVIL_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".devil").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$DEVIL_HOME/logs/``."""
    return get_devil_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$DEVIL_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under DEVIL_HOME or ``/tmp/hermes-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    devil_home = get_devil_home()
    try:
        path.resolve().relative_to(devil_home)
        return True
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside DEVIL_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under DEVIL_HOME (but leave DEVIL_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(devil_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == devil_home:
                continue
            try:
                rel_parts = dirpath.relative_to(devil_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    devil_home = get_devil_home()
    try:
        rel = path.resolve().relative_to(devil_home)
        ...
        # Path isn't under DEVIL_HOME (e.g. /tmp/hermes-*) — fall through.
        pass
