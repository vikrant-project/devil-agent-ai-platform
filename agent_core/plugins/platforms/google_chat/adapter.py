...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${DEVIL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _devil_home = _get_devil_home()
        except (ModuleNotFoundError, ImportError):
            _devil_home = _Path.home() / ".devil"
        self._thread_count_store = _ThreadCountStore(
            _devil_home / "google_chat_thread_counts.json"
        )
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("DEVIL_HOME", str(_Path.home() / ".devil"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${DEVIL_HOME}/google_chat_user_tokens/<sanitized>.json
...
def interactive_setup() -> None:
    ...
    The setup wizard at ``devil_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.devil/.env`` so the next
    gateway restart picks them up.
    """
    from devil_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print()
    print_success("Google Chat configuration saved to ~/.devil/.env")
