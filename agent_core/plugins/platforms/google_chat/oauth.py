...
- Per-user tokens (keyed by sender email):
    ``${DEVIL_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${DEVIL_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${DEVIL_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${DEVIL_HOME}/google_chat_user_oauth_pending.json``
- Shared OAuth client (one per host):
    ``${DEVIL_HOME}/google_chat_user_client_secret.json``
"""
...
# Use the project's DEVIL_HOME helper so the token follows the user's
# profile (e.g. tests can override via DEVIL_HOME=/tmp/...).
try:
    from devil_constants import display_devil_home, get_devil_home
except (ModuleNotFoundError, ImportError):
    # Fallback for environments where devil_constants isn't importable
...
def _devil_home() -> Path:
    """Resolve DEVIL_HOME at call time (NOT module import).

    Tests and ``DEVIL_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.devil/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _devil_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _devil_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _devil_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _devil_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _devil_home() / "google_chat_user_client_secret.json"
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into DEVIL_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_devil_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_devil_home()}/google_chat_user_token.json"
    )
