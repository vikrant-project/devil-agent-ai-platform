"""
IRC Platform Adapter for Devil Agent.
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Devil Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Devil Agent shutting down")
...
def interactive_setup() -> None:
    ...
    Lazy-imports ``devil_cli.setup`` helpers so the plugin stays importable
    ...
    print()
    print_success("IRC configuration saved to ~/.devil/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Devil Agent (cron)")
