"""
LINE Messaging API platform adapter for Devil Agent.
...
to Devil Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``DEVIL_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            devil_home = Path.home().joinpath(".devil").resolve()
...
def interactive_setup() -> None:
    ...
    one optional public URL. Writes to ``~/.devil/.env`` via ``devil_cli.config``.
    """
    print()
    ...
    try:
        from devil_cli.config import get_env_var, set_env_var
    except ImportError:
        print("devil_cli.config not available; set LINE_* vars manually in ~/.devil/.env")
        return
