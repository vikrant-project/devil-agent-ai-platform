...
session-scoped state file under ``$DEVIL_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $DEVIL_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_devil_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    env = os.environ.copy()
    env["DEVIL_MEET_URL"] = url
    env["DEVIL_MEET_OUT_DIR"] = str(out)
    env["DEVIL_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["DEVIL_MEET_HEADED"] = "1"
    if auth_state:
        env["DEVIL_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["DEVIL_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if DEVIL_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["DEVIL_MEET_MODE"] = mode
    if realtime_model:
        env["DEVIL_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["DEVIL_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["DEVIL_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["DEVIL_MEET_REALTIME_KEY"] = realtime_api_key
