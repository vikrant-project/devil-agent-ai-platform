"""
Lazy dependency installer for opt-in Devil Agent backends.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("DEVIL_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``devil_cli.tools_config._pip_install`` but
