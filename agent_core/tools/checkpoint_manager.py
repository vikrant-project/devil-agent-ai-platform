...
CHECKPOINT_BASE = get_devil_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("DEVIL_CHECKPOINT_TIMEOUT", "30"))))
...
# look for ``HEAD``/``info/exclude``/``DEVIL_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at DEVIL_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "DEVIL_WORKDIR").write_text(
