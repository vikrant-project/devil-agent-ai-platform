...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from devil_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("DEVIL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("DEVIL_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set DEVIL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind DEVIL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds DEVIL_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("DEVIL_CRON_SESSION"):
        return False
...
# via shell expansions like $HOME or $DEVIL_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_DEVIL_ENV_PATH = (
    r'(?:~\/\.devil/|'
    r'(?:\$home|\$\{home\})/\.devil/|'
    r'(?:\$devil_home|\$\{devil_home\})/)'
    r'\.env\b'
)
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["DEVIL_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "DEVIL_SPINNER_PAUSE" in os.environ:
            del os.environ["DEVIL_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("DEVIL_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("DEVIL_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("DEVIL_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("DEVIL_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("DEVIL_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("DEVIL_EXEC_ASK")
