    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Devil Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses DEVIL_HOME so each profile gets its own OAuth tokens.
    Layout: ``DEVIL_HOME/mcp-tokens/``
    """
    try:
        from devil_constants import get_devil_home
        base = Path(get_devil_home())
    except ImportError:
        base = Path(os.environ.get("DEVIL_HOME", str(Path.home() / ".devil")))
...
class HermesTokenStorage:
    ...
    File layout::
        ...
        DEVIL_HOME/mcp-tokens/<server_name>.json         -- tokens
        DEVIL_HOME/mcp-tokens/<server_name>.client.json   -- client info
        DEVIL_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Devil Agent")
    scope = cfg.get("scope")
