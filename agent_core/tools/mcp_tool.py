...
Configuration is read from ~/.devil/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.devil/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            devil_home = os.path.expanduser(
                os.getenv(
                    "DEVIL_HOME", os.path.join(os.path.expanduser("~"), ".devil")
                )
            )
            candidates = [
                os.path.join(devil_home, "node", "bin", resolved_command),
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"file under ~/.devil/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    ``os.environ`` (which includes ``~/.devil/.env`` loaded at startup).
    """
    try:
