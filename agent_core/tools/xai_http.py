...
try:
    from devil_cli.config import get_env_value as _hermes_get_env_value
except Exception:
    _hermes_get_env_value = None


def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.devil/.env`` first, then ``os.environ``.

    Wraps :func:`devil_cli.config.get_env_value` so tests can patch
...
def resolve_xai_http_credentials() -> Dict[str, str]:
    ...
    to ``XAI_API_KEY`` resolved via ``devil_cli.config.get_env_value`` so keys
    stored in ``~/.devil/.env`` (the standard Hermes location) are honored —
