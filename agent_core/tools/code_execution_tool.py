...
def _connect():
    ...
    DEVIL_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["DEVIL_RPC_SOCKET"]
...
_RPC_DIR = os.environ.get("DEVIL_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hermes_rpc")
_seq = 0
...
def _execute_remote(
    ...
    try:
        ...
        env_prefix = (
            f"DEVIL_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("DEVIL_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    # selector from DEVIL_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    try:
        ...
        #   connect.  DEVIL_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["DEVIL_RPC_SOCKET"] = rpc_endpoint
        ...
        # the staging tmpdir so ``from devil_tools import ...`` resolves even
        ...
        # DEVIL_TIMEZONE is an internal Hermes setting and must not leak
        # into child processes.
        _tz_name = os.getenv("DEVIL_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("DEVIL_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {DEVIL_HOME}/home/ when that directory exists.
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.devil/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    if mode == "strict":
        cwd_note = (
            ...
            "(os.path.expanduser('~/.devil/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        ...
        f"Available via `from devil_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from devil_tools import {import_str}` "
