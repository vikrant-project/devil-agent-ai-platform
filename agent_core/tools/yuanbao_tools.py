...
async def send_sticker(
    ...
    Args:
        ...
        chat_id:   目标会话；缺省时使用当前会话上下文（DEVIL_SESSION_CHAT_ID）。
    ...
    target = (chat_id or "").strip() or get_session_env("DEVIL_SESSION_CHAT_ID", "")
    if not target:
        return {
            "success": False,
...
def _check_yuanbao():
    ...
    try:
        from gateway.session_context import get_session_env
        if get_session_env("DEVIL_SESSION_PLATFORM", "") == "yuanbao":
            return True
    except Exception:
        pass
...
async def _handle_yb_send_dm(args, **kw):
    ...
    if not group_code:
        try:
            from gateway.session_context import get_session_env
            chat_id = get_session_env("DEVIL_SESSION_CHAT_ID", "")
