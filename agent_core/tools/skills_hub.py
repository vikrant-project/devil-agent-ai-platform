...
Used by devil_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.devil/skills/ during setup.  They are discoverable via the Skills Hub
...
DEVIL_INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json"
DEVIL_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "hermes-index.json"
DEVIL_INDEX_TTL = 6 * 3600  # 6 hours
...
def _load_hermes_index() -> Optional[dict]:
    ...
    We cache it locally for DEVIL_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if DEVIL_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - DEVIL_INDEX_CACHE_FILE.stat().st_mtime
            if age < DEVIL_INDEX_TTL:
                return json.loads(DEVIL_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(DEVIL_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
    ...
    # Cache locally
    try:
        DEVIL_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        DEVIL_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
