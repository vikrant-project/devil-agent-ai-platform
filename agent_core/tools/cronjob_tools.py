"""
Cron job management tools for Devil Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    ...
    origin_platform = get_session_env("DEVIL_SESSION_PLATFORM")
    origin_chat_id = get_session_env("DEVIL_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        thread_id = get_session_env("DEVIL_SESSION_THREAD_ID") or None
        if thread_id:
            logger.debug(
        ...
        return {
            ...
            "chat_name": get_session_env("DEVIL_SESSION_CHAT_NAME") or None,
            "thread_id": thread_id,
        }
    return None
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within DEVIL_HOME/scripts/.
    ...
    # Only relative paths within ~/.devil/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.devil/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.devil/scripts/ and use just the filename."
        )

    # Validate containment after resolution
    from tools.path_security import validate_within_dir

    scripts_dir = get_devil_home() / "scripts"
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": f"Optional path to a script that runs each tick. In the default mode its stdout is injected into the agent's prompt as context (data-collection / change-detection pattern). With no_agent=True, the script IS the job and its stdout is delivered verbatim (classic watchdog pattern). Relative paths resolve under {display_devil_home()}/scripts/. ``.sh``/``.bash`` extensions run via bash, everything else via Python. On update, pass empty string to clear."
            },
            "no_agent": {
                "type": "boolean",
                "default": False,
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("DEVIL_INTERACTIVE")
        or env_var_enabled("DEVIL_GATEWAY_SESSION")
        or env_var_enabled("DEVIL_EXEC_ASK")
    )
