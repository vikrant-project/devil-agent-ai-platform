...
running under the dispatcher (env var ``DEVIL_KANBAN_TASK`` set). A
...
1. **Backend portability.** A worker whose terminal tool points at Docker
   ...
   always reach ``~/.devil/kanban.db`` regardless of terminal backend.
...
def _check_kanban_mode() -> bool:
    ...
    1. ``DEVIL_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("DEVIL_KANBAN_TASK")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``DEVIL_KANBAN_TASK`` set
    ...
    ``DEVIL_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set DEVIL_KANBAN_TASK in the env)"
        )
    try:
        kb, conn = _connect()
        try:
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # DEVIL_KANBAN_CLAIM_LOCK in the worker env at spawn time
            # (see _default_spawn in kanban_db.py); falling back to the
            # default _claimer_id() covers locally-driven workers that
            # never went through the dispatcher path.
            claim_lock = os.environ.get("DEVIL_KANBAN_CLAIM_LOCK")
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = os.environ.get("DEVIL_PROFILE") or "worker"
    try:
        kb, conn = _connect()
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("DEVIL_TENANT")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                skills=skills,
                created_by=os.environ.get("DEVIL_PROFILE") or "worker",
            )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to DEVIL_KANBAN_TASK from the env "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to DEVIL_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
