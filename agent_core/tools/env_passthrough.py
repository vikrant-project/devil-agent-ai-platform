...
def _is_hermes_provider_credential(name: str) -> bool:
    ...
    token, or similar) per ``_DEVIL_PROVIDER_ENV_BLOCKLIST``.
    ...
    try:
        ...
        return False
    return name in _DEVIL_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    ``_DEVIL_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    ...
    for name in var_names:
        ...
        if _is_hermes_provider_credential(name):
            logger.warning(
                ...
                "credential %r (blocked by _DEVIL_PROVIDER_ENV_BLOCKLIST). "
