...
_FORCE_SYNC_ENV = "DEVIL_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.devil") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.devil to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``DEVIL_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, devil_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.devil/`` directory as a tar archive,
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.devil/skills/a.md`` →
        ``~/.devil/skills/a.md``, a new remote file at
        ``/root/.devil/skills/b.md`` maps to ``~/.devil/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
