...
_DEVIL_PROVIDER_ENV_FORCE_PREFIX = "_DEVIL_FORCE_"
...
_DEVIL_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_DEVIL_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _DEVIL_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_DEVIL_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_DEVIL_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _DEVIL_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    # Per-profile HOME isolation for background processes (same as _make_run_env).
    from devil_constants import get_subprocess_home
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("DEVIL_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. Devil Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set DEVIL_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_DEVIL_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_DEVIL_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _DEVIL_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # npm …) into {DEVIL_HOME}/home/ when that directory exists.  Only the
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``DEVIL_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under DEVIL_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_devil_home() / "cache" / "terminal"
            except Exception:
