...
from tools.environments.local import _DEVIL_PROVIDER_ENV_BLOCKLIST
...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.devil/.env values without failing Docker command execution."""
    try:
        from devil_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``DEVIL_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("DEVIL_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using DEVIL_DOCKER_BINARY override: %s", override)
        return override
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.devil/sandboxes/). Non-persistent
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _DEVIL_PROVIDER_ENV_BLOCKLIST)
