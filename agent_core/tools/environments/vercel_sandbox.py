...
backend stores task-scoped snapshot metadata under ``DEVIL_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_devil_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.devil"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.devil"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_hermes = (
            "/.devil"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.devil"
        )
        archive_member = remote_hermes.lstrip("/")
        remote_tar = f"/tmp/.devil_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
