...
* ``XAI_API_KEY`` is set in ``~/.devil/.env`` or the process environment
  (paid xAI API key), OR
...
from tools.xai_http import devil_xai_user_agent, resolve_xai_http_credentials
...
def check_x_search_requirements() -> bool:
    ...
    :func:`devil_cli.auth.resolve_xai_oauth_runtime_credentials` which
