...
it is automatically downloaded from GitHub releases to $DEVIL_HOME/bin/tirith.
...
def _get_devil_home() -> str:
    """Return the Hermes home directory, respecting DEVIL_HOME env var."""
    return str(get_devil_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_devil_home(), ".tirith-install-failed")
...
def _hermes_bin_dir() -> str:
    """Return $DEVIL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_devil_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $DEVIL_HOME/bin/tirith.
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $DEVIL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $DEVIL_HOME/bin/tirith
