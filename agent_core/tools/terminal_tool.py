...
# display_devil_home imported lazily at call site (stale-module safety during hermes update)
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("DEVIL_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("DEVIL_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("DEVIL_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (DEVIL_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (DEVIL_INTERACTIVE=1):
    ...
    if not has_configured_password and not sudo_password and env_var_enabled("DEVIL_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.devil/.env or environment variables."
        )
        ...
        if background:
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    _gw_platform = _gse("DEVIL_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("DEVIL_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("DEVIL_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("DEVIL_SESSION_USER_ID", "")
                        _gw_user_name = _gse("DEVIL_SESSION_USER_NAME", "")
