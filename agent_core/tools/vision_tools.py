...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("DEVIL_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from devil_cli.config import cfg_get, load_config
        cfg = load_config()
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $DEVIL_HOME/cache/vision/ and cleaned up
