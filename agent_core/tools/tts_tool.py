...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.devil/config.yaml``. Hermes
...
Configuration is loaded from ~/.devil/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``devil_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from devil_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import devil_xai_user_agent
...
def _get_default_output_dir() -> str:
    from devil_constants import get_hermes_dir
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.devil/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.devil/config.yaml.
    ...
    try:
        ...
        logger.debug("devil_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__DEVIL_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _get_piper_voices_dir() -> Path:
    ...
    Resolves to ``~/.devil/cache/piper-voices/`` under the active
    DEVIL_HOME so voice downloads follow profile boundaries.
    """
    from devil_constants import get_hermes_dir
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.devil/config.yaml (tts: section).
    ...
    platform = get_session_env("DEVIL_SESSION_PLATFORM", "").lower()
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_devil_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
