...
LOCAL_STT_COMMAND_ENV = "DEVIL_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "DEVIL_LOCAL_STT_LANGUAGE"
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set DEVIL_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_xai(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    language = str(
        ...
        or os.getenv("DEVIL_LOCAL_STT_LANGUAGE")
        or DEFAULT_LOCAL_STT_LANGUAGE
    ).strip()
