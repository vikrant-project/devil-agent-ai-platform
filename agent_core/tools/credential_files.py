...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.devil",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``DEVIL_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside DEVIL_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    devil_home = _resolve_devil_home()

    # Reject absolute paths — they bypass the DEVIL_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to DEVIL_HOME)",
            relative_path,
        )
        return False

    host_path = devil_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape DEVIL_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, devil_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.devil/{rel}"
                        result.append({
...
# Each tuple is (new_subpath, old_name) matching devil_constants.get_hermes_dir().
